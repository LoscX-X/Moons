package com.blanoir.moons.ysm.internal.geckolib3.core.util;
...
public class MathUtil {
    private static final float DEGREES_TO_RADIANS = ScalarMath.DEG_TO_RAD;
    private static final float RADIANS_TO_DEGREES = ScalarMath.RAD_TO_DEG;
    ...
    public static void nlerpEulerAngles(
    ...
    public static Vector3f getEulerAnglesZYX(Quaternionf quaternionf, Vector3f eulerAngles) {
        ...
        eulerAngles.y =
                Math.safeAsin(
                        (-2.0f)
                                * ((quaternionf.x * quaternionf.z)
                                        - (quaternionf.w * quaternionf.y)));
        eulerAngles.z =
                Math.atan2(
    ...
    public static void lerpValues(
            float percentCompleted, Vector3f begin, Vector3f end, Vector3f outResult) {
        outResult.set(
                lerpValues(percentCompleted, begin.x(), end.x()),
                lerpValues(percentCompleted, begin.y(), end.y()),
                lerpValues(percentCompleted, begin.z(), end.z()));
    ...
    public static Vector3f catmullRom(
            float percentCompleted, Vector3f left, Vector3f begin, Vector3f end, Vector3f right) {
        return new Vector3f(
    ...
    public static Vector3f normalizeAngles(Vector3f angles) {
        return new Vector3f(
                normalizeAngle(angles.x), normalizeAngle(angles.y), normalizeAngle(angles.z));
    }
    ...
    public static Vector3f lerpAngles(Vector3f targetAngles, float t) {
        return new Vector3f(
                lerpAngle(targetAngles.x, t),
                lerpAngle(targetAngles.y, t),
                lerpAngle(targetAngles.z, t));
    }
