...
public final class NoSlow {
        ...
        synchronized (LOCK) {
            ...
            GameAccess.withPredictionSequence(
                    ...
                    sequence -> {
                        ...
                        // Normal send: NoSlow PRE -> other PRE listeners -> actual write -> all
                        // POST listeners.
    ...
    private static void onReceiveApply(PacketReceiveEvent.Apply event) {
        if (!RESET_ON_APPLY_PACKETS.remove(event.packet()) && !isResetPacket(event.packet()))
            return;
        synchronized (LOCK) {
            resetCycle();
        }
    ...
    public static int setEnabled(Minecraft client, boolean value) {
        ...
        ClientChat.send(client, "NoSlow " + (value ? "enabled" : "disabled") + ". Mode: Grim2371.");
        return 1;
    }
    ...
    private static BooleanSetting booleanSetting(String key, boolean defaultValue) {
        return new BooleanSetting.Builder().name(key).defaultValue(defaultValue).build();
    }
