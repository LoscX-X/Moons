...
public final class AimJitter {
    ...
    public static Vec3 insideHitbox(
        ...
        return insideHitbox(eye, base, box, seconds,
                Integer.toUnsignedLong(targetId) * 0x9E3779B97F4A7C15L, strength, speed);
    }

    /** A caller-owned session seed prevents entity IDs from defining the motion profile. */
    public static Vec3 insideHitbox(Vec3 eye, Vec3 base, AABB box,
                                   double seconds, long seed, double strength, double speed) {
        ...
        double depth = AimMotionNoise.sample(time * 1.37D,
                seed ^ 0x94D049BB133111EBL) * lateralAmplitude * 0.34D;
        double vertical = AimMotionNoise.sample(
