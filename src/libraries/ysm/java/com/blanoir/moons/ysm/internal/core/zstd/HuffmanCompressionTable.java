...
import static com.blanoir.moons.ysm.internal.core.zstd.Huffman.MAX_FSE_TABLE_LOG;
...
import static com.blanoir.moons.ysm.internal.core.zstd.Huffman.MAX_TABLE_LOG;
import static com.blanoir.moons.ysm.internal.core.zstd.Huffman.MIN_TABLE_LOG;
...
import static com.blanoir.moons.ysm.internal.core.zstd.Util.minTableLog;
...
final class HuffmanCompressionTable {
    ...
    public int write(
        ...
        int size =
                compressWeights(
                        outputBase, output + 1, outputSize - 1, weights, maxSymbol, workspace);
        ...
        if (size != 0 && size != 1 && size < maxSymbol / 2) {
            ...
            //   - the compressed size is <= 127 bytes, which is the most that the encoding can hold
            // for FSE-compressed weights (see RFC 8478 section 4.2.1.1). This is implied
            ...
            // #entries = #symbols - 1 since last symbol is implicit. Thus, #entries = (maxSymbol +
            // 1) - 1 = maxSymbol
            int entryCount = maxSymbol;

            size = (entryCount + 1) / 2; // ceil(#entries / 2)
            checkArgument(
                    size + 1 /* header */ <= outputSize,
    ...
    private static int setMaxHeight(
        ...
        while (totalCost < 0) { // Sometimes, cost correction overshoot
            if (rankLast[1] == noSymbol) {
                /* special case : no rank 1 symbol (using maxNumberOfBits-1); let's create one from largest rank 0 (using maxNumberOfBits) */
                while (nodeTable.numberOfBits[n] == maxNumberOfBits) {
                    n--;
                }
