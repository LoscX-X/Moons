package com.blanoir.moons.ysm.internal.geckolib3.core.processor;
...
import com.blanoir.moons.ysm.internal.geckolib3.core.controller.BoneTransformProvider;
import com.blanoir.moons.ysm.internal.geckolib3.core.controller.IAnimationController;
...
import com.blanoir.moons.ysm.internal.geckolib3.core.molang.storage.IForeignVariableStorage;
import com.blanoir.moons.ysm.internal.geckolib3.core.molang.storage.VariableStorage;
...
public class AnimationProcessor<TEntity> {
    ...
    private Object2ReferenceMap<String, List<IValue>> initExpressions =
            Object2ReferenceMaps.emptyMap();

    private final Int2ReferenceOpenHashMap<BoneTopLevelSnapshot> boneById =
    ...
    private final ConcurrentLinkedQueue<PendingExpression> pendingExpressions =
    ...
    public void tickAnimation(
            AnimationEvent<AnimatableEntity<TEntity>> event,
        ...
        while (iterator.hasNext()) {
            ...
            if (topLevelSnapshot.isCurrentlyRunningRotationAnimation) {
                ...
                float percentageReset =
                        (seekTime - topLevelSnapshot.mostRecentResetRotationTick)
                                / manager.getResetSpeed();
                if (percentageReset < 1.0f) {
                    runningAnimation = true;
                    MathUtil.nlerpEulerAngles(
                            percentageReset,
                            topLevelSnapshot.prevRotation,
                            MathUtil.ZERO,
            ...
            if (topLevelSnapshot.isCurrentlyRunningPositionAnimation) {
                ...
                float percentageReset =
                        (seekTime - topLevelSnapshot.mostRecentResetPositionTick)
                                / manager.getResetSpeed();
                if (percentageReset < 1.0f) {
                    runningAnimation = true;
                    MathUtil.lerpValues(
                            percentageReset,
                            topLevelSnapshot.prevPosition,
                            MathUtil.ZERO,
                            topLevelSnapshot.position);
                } else {
            ...
            if (topLevelSnapshot.isCurrentlyRunningScaleAnimation) {
                ...
                float percentageReset =
                        (seekTime - topLevelSnapshot.mostRecentResetScaleTick)
                                / manager.getResetSpeed();
                if (percentageReset < 1.0f) {
                    runningAnimation = true;
                    MathUtil.lerpValues(
                            percentageReset,
                            topLevelSnapshot.prevScale,
                            MathUtil.ONE,
                            topLevelSnapshot.scale);
                } else {
    ...
    private static void reportControllerFailure(
            IAnimationController<?> controller, RuntimeException exception) {
        String failureKey =
                controller.getName()
                        + '|'
        ...
        if (REPORTED_CONTROLLER_FAILURES.add(failureKey)) {
            YesSteveModel.LOGGER.warn(
                    "[SM-ANIM] Animation controller '{}' failed; keeping its last valid pose and continuing other controllers",
                    controller.getName(),
                    exception);
        }
    }
    ...
    private void applyTransform(BoneTransformProvider provider) {
        ...
        if (rot != null) {
            ...
            if (this.currentDeprecatedMode) {
                ...
                rot.applyRotationBlendTo(
                        snapshot.rotation, snapshot.bone.getInitialRotation(), this.rotScratch);
    ...
    public void initBones(
            AnimatedGeoModel model, Object2ReferenceMap<String, List<IValue>> object2ReferenceMap) {
        reset();
        if (!model.bones().isEmpty()) {
            this.bones.ensureCapacity(model.bones().size());
            this.bones.add(null);
            int boneId = model.rootBoneId();
            Int2ReferenceMaps.fastForEach(
                    model.bones(),
                    entry -> {
                        BoneTopLevelSnapshot boneTopLevelSnapshot =
                                new BoneTopLevelSnapshot(entry.getValue());
    ...
    private void postProcess(
            PendingExpression value, ExpressionEvaluator<AnimationContext<?>> evaluator) {
        String result;
        try {
            Object ret;
            try {
            ...
            result =
                    ret == null ? "null" : ret instanceof String ? "'" + ret + "'" : ret.toString();
            evaluator.entity().setIsClientSide(false);
    ...
    public void execute(
        ...
        this.pendingExpressions.add(
                new PendingExpression(
                        value, isClientPlayer, executeBeforeAnimation, resultConsumer));
    }
