...
public final class PlayerHitEstimator {
    ...
    public static int hitsToKill(Minecraft client, Player target, float resolvedHealth) {
        ...
        float health = Math.max(0.0F, resolvedHealth)
    ...
    private static float estimatedDamage(Minecraft client, Player target) {
        ...
        boolean maceSmash = weapon.getItem() instanceof MaceItem
                && MaceItem.canSmashAttack(client.player);
        ...
        baseDamage += weapon.getItem()
                .getAttackDamageBonus(target, baseDamage, source);
        ...
        for (EquipmentSlot slot : PLAYER_ARMOR_SLOTS) {
            protection += enchantmentLevel(
                    target.getItemBySlot(slot), Enchantments.PROTECTION);
        }
        ...
        float normalDamage = mitigatedDamage(
                baseDamage + magicBoost, target, source, breach, protection);
        float criticalDamage = mitigatedDamage(
                baseDamage * 1.5F + magicBoost, target, source, breach, protection);
        double criticalRate = HitEstimateSettings.criticalRate();
        return Math.max(0.0F, (float) (normalDamage * (1.0D - criticalRate)
    ...
    private static float currentWeaponAttackDamage(Player player, ItemStack weapon) {
        ...
        return (float) Math.max(0.0D,
                Attributes.ATTACK_DAMAGE.value().sanitizeValue(value));
    }
    ...
    private static float damageAfterArmor(
        ...
        float effectiveArmor = Mth.clamp(
                armor - damage / toughnessFactor,
                armor * 0.2F,
                20.0F);
        float armorEffectiveness = effectiveArmor / 25.0F;
        armorEffectiveness = Mth.clamp(
                armorEffectiveness - 0.15F * breachLevel,
                0.0F,
                1.0F);
