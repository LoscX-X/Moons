...
import com.blanoir.moons.client.module.impl.network.backtrack.BacktrackTiming;
import com.blanoir.moons.client.module.impl.network.backtrack.BacktrackVisual;
...
/**
 * Short, ordered history of target movement with client-thread replay.
 * Other entities are handed off normally; combat input and physics are not delayed.
 */
public final class Backtrack {
    ...
    private static final DoubleSetting RANGE_MIN =
            new DoubleSetting.Builder().name("backtrack.range.min").defaultValue(1.0D).build();

    private static final DoubleSetting RANGE_MAX =
            new DoubleSetting.Builder().name("backtrack.range.max").defaultValue(6.0D).build();

    private static final IntSetting DELAY_MIN =
            new IntSetting.Builder().name("backtrack.delay.min").defaultValue(100).build();

    private static final IntSetting DELAY_MAX =
            new IntSetting.Builder().name("backtrack.delay.max").defaultValue(150).build();
    ...
    private static final IntSetting TRACKING_BUFFER =
            new IntSetting.Builder().name("backtrack.trackingBuffer").defaultValue(500).build();

    private static final DoubleSetting CHANCE =
            new DoubleSetting.Builder().name("backtrack.chance").defaultValue(100.0D).build();
    ...
    private static final IntSetting MAX_QUEUE_SIZE =
            new IntSetting.Builder().name("backtrack.maxQueueSize").defaultValue(256).build();

    private static final DoubleSetting SPEED_FACTOR =
            new DoubleSetting.Builder().name("backtrack.speedFactor").defaultValue(8.0D).build();

    private static final DoubleSetting PING_RATIO =
            new DoubleSetting.Builder().name("backtrack.pingRatio").defaultValue(0.0D).build();

    private static final BooleanSetting ACTION_BAR =
            new BooleanSetting.Builder().name("backtrack.actionbar").defaultValue(false).build();
    ...
    private static volatile int sampledBaseDelay = randomInt(DELAY_MIN.get(), DELAY_MAX.get());
    ...
    public static void init() {
        EventBus.CLIENT_CONTEXT_CHANGED.register(
                "Backtrack.context", event -> clear(false, true, false));
        EventBus.PACKET_SEND_POST.register(
                "Backtrack.attackSent",
                event -> {
                    if (ENABLED.get() && event.packet() instanceof ServerboundAttackPacket attack) {
                        ...
                        client.execute(
                                () -> {
                                    if (connection != null
                                            ...
                                            && client.level != null) {
                                        onAttack(client.level.getEntity(attack.entityId()));
                                    }
                                });
                    }
                });
    ...
    public static boolean handleIncomingPacket(Packet<?> packet, PacketListener listener) {
        if (!ENABLED.get() || !(listener instanceof ClientGamePacketListener)) {
            return false;
        }

        // Login creates the LocalPlayer from which Minecraft.getConnection()
        // is derived. Wrapping it in TrackingPacket would reject it as stale
        // before that player exists, leaving levelData null for the next packet.
        // Preserve vanilla login handling; the context event resets tracking.
        if (packet instanceof ClientboundLoginPacket) return false;

        // PRE is a Netty callback. Use vanilla's packet processor, not the
        // unrelated client task queue, to preserve spawn/move/remove ordering.
        if (!(packet instanceof ClientboundMoveEntityPacket)
                ...
                && !requiresTrackingReset(packet)) return false;
        Minecraft client = Minecraft.getInstance();
        client.packetProcessor()
                .scheduleIfPossible(
                        (ClientGamePacketListener) listener, new TrackingPacket(packet));
        return true;
    }

    private static boolean processIncomingOnClient(Packet<?> packet) {
        ...
        if (packet instanceof ClientboundRemoveEntitiesPacket remove
                && currentTarget != null
                && remove.getEntityIds().contains(currentTarget.getId())) {
            clear(true, false, true);
            return false;
        }
        if (observeTargetAndReleaseIfNeeded(packet, level, player, currentTarget)) {
            return false;
        }
    ...
    private static boolean observeTargetAndReleaseIfNeeded(
            Packet<?> packet, ClientLevel level, LocalPlayer player, Entity currentTarget) {
        ...
        // Never freeze an enemy that is actually closing on us.
        ...
        if (pos != null && isApproachingPlayer(currentTarget, player, previousRealPosition, pos)) {
            intentApproachReleaseUntilTick = gameTick + INTENT_APPROACH_RELEASE_TICKS;
            flushIncoming();
            armed = false;
            return true;
        }
        ...
        if (pos != null
                && squareBoxedDistanceTo(currentTarget, player, pos)
                        <= squaredBoxedDistanceTo(currentTarget, player) + 1.0E-4D) {
    ...
    private static boolean shouldQueueTargetMovement(
        ...
        // Keep one ordered stream while there is useful combat history.
        ...
        synchronized (PACKET_QUEUE_LOCK) {
            ...
            long releaseAt =
                    BacktrackTiming.deadline(
                            timestamp,
                            currentDelay,
                            PACKET_QUEUE.isEmpty() ? 0L : lastQueuedReleaseAt);
            lastQueuedReleaseAt = releaseAt;
            Minecraft client = Minecraft.getInstance();
            PACKET_QUEUE.add(
                    new PacketSnapshot(
                            packet, timestamp, releaseAt, client.getConnection(), client.level));
            return true;
        }
    }

    private static boolean isTargetPositionPacket(
            Packet<?> packet, Entity target, ClientLevel level) {
    ...
    /**
     * Called after the attack packet is dispatched, for both manual and automatic attacks.
     */
    public static void onAttack(Entity entity) {
        if (!ENABLED.get() || entity == null || !shouldBeAttacked(entity)) {
            return;
        }
        // Do not move the clicked entity before vanilla has even sent ATTACK.
        // The short history continues to expire normally while combat continues.
        attackChronometerLastUpdate = nowMillis();
    ...
    private static void releaseOrFlushQueuedPackets() {
        ...
        if (armed && shouldCancelPackets()) {
            // Expired history must never acquire another rate-limit delay.
            releaseDuePackets();
    ...
    /** Show actual queued age instead of diagnostic distances while fighting. */
    public static String hudStats() {
        PacketSnapshot oldest = PACKET_QUEUE.peek();
        return oldest == null ? "Ready" : Math.max(0, nowMillis() - oldest.timestamp()) + "ms";
    }

    private static void processTarget(Entity enemy) {
        shouldPause =
                enemy instanceof LivingEntity living
                        && living.hurtTime >= PAUSE_ON_HURT_TIME_VALUE.get();

        if (enemy != target && hasElapsed(chronometerLastUpdate, 0)) {
            chancePassed = rollChance();
            if (!chancePassed) {
                waitForAtLeast(100, 150);
                return;
            }
        }
    ...
    private static boolean shouldBacktrack(Entity enemy) {
        ...
        if (inRange) {
            trackingBufferChronometerLastUpdate = nowMillis();
        }
        ...
        return (inRange || buffered)
                ...
                && !shouldPause()
                && (isIntentMode()
                        || !hasElapsed(attackChronometerLastUpdate, LAST_ATTACK_TIME.get()))
    ...
    private static void handleSnapshot(PacketSnapshot snapshot) {
        ...
        applyPacket(snapshot.packet(), snapshot.listener());
    }

    @SuppressWarnings("unchecked")
    private static void applyPacket(Packet<?> packet, ClientGamePacketListener listener) {
    ...
    private static boolean isApproachingPlayer(
            Entity currentTarget, Entity player, Vec3 previousPosition, Vec3 currentPosition) {
        ...
        double previousDistance = squareBoxedDistanceTo(currentTarget, player, previousPosition);
        double currentDistance = squareBoxedDistanceTo(currentTarget, player, currentPosition);
        return currentDistance + INTENT_APPROACH_MIN_DISTANCE_SQUARED < previousDistance;
    }

    /**
     * Arms while there is a target, except during the approach-release grace period.
     */
    private static void updateArmState() {
        ...
        // Actual combat intent and distance control the window. Guessing the next
        // click from weapon cooldown made old combat versions repeatedly freeze/snap.
        boolean newArmed = gameTick > intentApproachReleaseUntilTick;
        if (newArmed && !armed) {
            sampleDelayWindow();
        }
        armed = newArmed;
    ...
    /**
     * Releases every expired packet in order; a second throttle would add hidden lag.
     */
    private static void releaseDuePackets() {
        ...
        while (true) {
            ...
            synchronized (PACKET_QUEUE_LOCK) {
                ...
                if (!BacktrackTiming.due(now, snapshot.releaseAt())) {
                    return;
                }
                PACKET_QUEUE.poll();
    ...
    private static void trackTargetSpeed() {
        ...
        TARGET_POSITION_SAMPLES.addLast(new PositionSample(gameTick, POSITION.base()));
     ...
     * movement speed and optionally capped by the player's ping.
     */
    ...
    private static Vec3 rayBoxIntersection(
            Vec3 origin, Vec3 direction, AABB box, double maxDistance) {
        double tMin = 0.0D;
    ...
    private static boolean shouldBeAttacked(Entity entity) {
        ...
        if (living instanceof Player targetPlayer) {
            return Targeting.isEnemyPlayer(client, targetPlayer) && !targetPlayer.isSleeping();
        }
    ...
    private static boolean hasElapsed(long lastUpdate, long ms) {
        return lastUpdate + ms < nowMillis();
    }

    private static void waitForAtLeast(int minDelayMs, int maxDelayMs) {
        chronometerLastUpdate =
                Math.max(chronometerLastUpdate, nowMillis() + randomInt(minDelayMs, maxDelayMs));
    }
    ...
    /** Expires history and updates visual positions once per rendered frame. */
    private static void frame(FrameEvent event) {
        ...
        // A render frame is also a client-thread deadline boundary; avoid 50 ms
        // tick quantization when the renderer is running much faster than 20 Hz.
        if (client.isSameThread()) releaseDuePackets();

        double deltaSeconds = Mth.clamp(event.deltaSeconds(), 1.0D / 500.0D, 1.0D / 20.0D);
        Vec3 base = POSITION.base();
        if (base == null || isZeroPosition(base)) {
            renderPosition = null;
        } else {
            renderPosition = approachFramePosition(renderPosition, base, deltaSeconds, 28.0D);
        }
    }

    private static Vec3 approachFramePosition(
            Vec3 current, Vec3 targetPosition, double deltaSeconds, double responseSpeed) {
        if (current == null || current.distanceToSqr(targetPosition) > 4.0D) {
    ...
    private static EspData getEspData() {
        ...
        if (entity == null || !entity.isAlive() || !hasQueuedIncoming()) {
            return null;
        }
        ...
        if (base.distanceToSqr(entity.position()) < 0.0025D) return null;
    ...
    public static void renderModel(
        ...
        state.lightCoords = scaleLightCoords(state.lightCoords, MODEL_LIGHT_PERCENT.get() * 0.01f);
    ...
    private static void renderBoxEsp(PoseStack matrices, EspData data) {
        ...
        AABB box =
                new AABB(-halfWidth, 0.0D, -halfWidth, halfWidth, dimensions.height(), halfWidth)
                        .inflate(0.015D)
                        .move(data.pos());
        ...
        BacktrackRenderer.renderBox(
                matrices,
                box,
                BacktrackVisual.fill(parseColor(BOX_COLOR.get())),
                BacktrackVisual.outline(parseColor(BOX_OUTLINE_COLOR.get())),
                "backtrack box");
    }
    ...
    private static void renderWireframeEsp(PoseStack matrices, EspData data) {
        ...
        // Keep legacy colors configurable without an opaque second player.
        WIREFRAME.render(
                matrices,
                BacktrackVisual.fill(parseColor(WIREFRAME_COLOR.get())),
                BacktrackVisual.outline(parseColor(WIREFRAME_OUTLINE_COLOR.get())));
    }

    private static int scaleLightCoords(int lightCoords, float scale) {
        int block =
                Math.max(0, Math.min(15, Math.round(LightCoordsUtil.block(lightCoords) * scale)));
    ...
    public static int setRange(Minecraft client, String rawRange) {
        ...
        if (parsed == null) {
            ClientChat.send(
                    client,
                    "Invalid Backtrack range. Use .moons backtrack range x-x, each between 0 and 10.");
            return 0;
        }
        ...
        ClientChat.send(
                client,
                "Backtrack range set to " + formatRange(RANGE_MIN.get(), RANGE_MAX.get()) + ".");
        return 1;
    }
    ...
    public static int setDelay(Minecraft client, String rawRange) {
        ...
        if (parsed == null) {
            ClientChat.send(
                    client,
                    "Invalid Backtrack delay. Use .moons backtrack delay x-x, each between 0 and 1000 ms.");
            return 0;
        }
        ...
        ClientChat.send(
                client,
                "Backtrack delay set to "
                        + formatIntRange(DELAY_MIN.get(), DELAY_MAX.get())
                        + " ms.");
        return 1;
    }
    ...
    public static int setNextBacktrackDelay(Minecraft client, String rawRange) {
        ...
        if (parsed == null) {
            ClientChat.send(
                    client,
                    "Invalid Backtrack nextBacktrackDelay. Use .moons backtrack nextbacktrackdelay x-x, each between 0 and 2000 ms.");
            return 0;
        }
        ...
        ClientChat.send(
                client,
                "Backtrack nextBacktrackDelay set to "
                        + formatIntRange(
                                NEXT_BACKTRACK_DELAY_MIN.get(), NEXT_BACKTRACK_DELAY_MAX.get())
                        + " ms.");
        return 1;
    }
    ...
    public static int setTrackingBuffer(Minecraft client, String rawValue) {
        ...
        if (value == null) {
            ClientChat.send(
                    client,
                    "Invalid Backtrack trackingBuffer. Use .moons backtrack trackingbuffer 0-2000 ms.");
            return 0;
        }

        TRACKING_BUFFER.set(value);
        ClientChat.send(
                client, "Backtrack trackingBuffer set to " + TRACKING_BUFFER.get() + " ms.");
        return 1;
    }
    ...
    public static int setHurtEnabled(Minecraft client, boolean value) {
        ...
        ClientChat.send(
                client,
                "Backtrack pauseOnHurtTime "
                        + (PAUSE_ON_HURT_TIME.get() ? "enabled" : "disabled")
                        + ".");
        return 1;
    }
    ...
    public static int setHurtTime(Minecraft client, String rawValue) {
        ...
        if (value == null) {
            ClientChat.send(
                    client, "Invalid Backtrack hurtTime. Use .moons backtrack hurt time 0-10.");
            return 0;
        }

        PAUSE_ON_HURT_TIME_VALUE.set(value);
        ClientChat.send(
                client, "Backtrack hurtTime set to " + PAUSE_ON_HURT_TIME_VALUE.get() + ".");
        return 1;
    }
    ...
    public static int setLastAttackTime(Minecraft client, String rawValue) {
        ...
        if (value == null) {
            ClientChat.send(
                    client,
                    "Invalid Backtrack lastAttackTimeToWork. Use .moons backtrack lastattacktime 0-5000 ms.");
            return 0;
        }

        LAST_ATTACK_TIME.set(value);
        ClientChat.send(
                client, "Backtrack lastAttackTimeToWork set to " + LAST_ATTACK_TIME.get() + " ms.");
        return 1;
    }
    ...
    public static int setMaxQueueSize(Minecraft client, String rawValue) {
        ...
        ClientChat.send(
                client, "Backtrack maxQueueSize set to " + MAX_QUEUE_SIZE.get() + " packets.");
        return 1;
    }
    ...
    public static int setSpeedFactor(Minecraft client, String rawValue) {
        ...
        if (value == null) {
            ClientChat.send(
                    client,
                    "Invalid Backtrack speedFactor. Use .moons backtrack speedfactor 0-30.");
            return 0;
        }
    ...
    public static int setPingRatio(Minecraft client, String rawValue) {
        ...
        if (value == null) {
            ClientChat.send(
                    client,
                    "Invalid Backtrack pingRatio. Use .moons backtrack pingratio 0-3 (0 disables ping calibration).");
            return 0;
        }
    ...
    public static int setActionBarEnabled(Minecraft client, boolean value) {
        ...
        ClientChat.send(
                client, "Backtrack actionbar " + (ACTION_BAR.get() ? "enabled" : "disabled") + ".");
        return 1;
    }
    ...
    public static int setColor(Minecraft client, String rawColor, boolean outline) {
        ...
        if (color == INVALID_COLOR) {
            ClientChat.send(
                    client,
                    "Invalid color. Use .moons backtrack "
        ...
        if (isBoxEsp()) {
            ...
            ClientChat.send(
                    client,
                    "Backtrack color is only available for the box/wireframe/model esp modes.");
            return 0;
        }
    ...
    public static int setLightPercent(Minecraft client, String rawValue) {
        ...
        if (value == null) {
            ClientChat.send(
                    client,
                    "Invalid Backtrack lightPercent. Use .moons backtrack lightpercent 0-100.");
            return 0;
        }

        MODEL_LIGHT_PERCENT.set(value);
        ClientChat.send(
                client, "Backtrack lightPercent set to " + MODEL_LIGHT_PERCENT.get() + "%.");
        return 1;
    }
    ...
    private static int parseColor(String rawColor) {
        ...
        try {
            if (normalized.length() == 6) {
                ...
                return ColorCodec.parseRgbOrArgb(normalized);
            }
    ...
    private static String formatColor(int argb) {
        return ColorCodec.formatArgb(argb);
    }
    ...
    public static List<String> espModeOptions() {
        return ESP_MODE.optionIds();
    }

    public static boolean espEnabled() {
        return ESP_MODE.get() != EspMode.NONE;
    }

    public static boolean modelEspSelected() {
        return ESP_MODE.get() == EspMode.MODEL;
    }
    ...
    /** Local envelope only; never serialized or sent to a server. */
    private record TrackingPacket(Packet<?> original) implements Packet<ClientGamePacketListener> {
        @Override
        ...
        public void handle(ClientGamePacketListener listener) {
            if (Minecraft.getInstance().getConnection() != listener) return;
            if (!ENABLED.get() || !processIncomingOnClient(original)) {
                applyPacket(original, listener);
            }
        }
