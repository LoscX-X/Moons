...
public final class Animations {
    ...
    private static final BooleanSetting ENABLED = new BooleanSetting.Builder()
            .name("blockanimation.enabled").defaultValue(false).build();
    private static final BooleanSetting SILENT_AURA_ONLY = new BooleanSetting.Builder()
            .name("blockanimation.silentAuraOnly").defaultValue(false).build();
    private static final ModeSetting<Mode> MODE = animationModes();
    private static final DoubleSetting SWING_SPEED = number("blockanimation.swingSpeed", 1.0, .1, 2.0);
    ...
    public static void bindAuraState(BooleanSupplier enabled,
     ...
     */
    public static void apply(PoseStack pose, float vanillaSwingProgress,
                             float equipProgress, Object renderedHand) {
        ...
        switch (mode) {
            ...
            case TAP1 -> {
                ...
                pose.translate(0.56, -0.52, -0.71999997);
                rotate(pose, 45.0F, 0.0F, 1.0F, 0.0F);
                rotate(pose, (progress * 0.8F - progress * progress * 0.8F) * -90.0F,
                        0.0F, 1.0F, 0.0F);
        ...
        rotate(pose, -45.0F, 0.0F, 1.0F, 0.0F);
        pose.scale(MODERN_BLOCK_MODEL_SCALE, MODERN_BLOCK_MODEL_SCALE,
                MODERN_BLOCK_MODEL_SCALE);
    ...
    public static void applyThirdPerson(Avatar avatar, AvatarRenderState state) {
        ...
        boolean active = SILENT_AURA_ONLY.get()
                ? auraOwnsAnimation
                : true;
        if (!active) return;
    ...
    private static float visualSwingProgress(float vanillaSwingProgress) {
        ...
        float explicit = elapsed >= 0L && elapsed < ATTACK_ANIMATION_NANOS
    ...
    private static void rotate(PoseStack pose, float degrees, float x, float y, float z) {
        ...
        pose.mulPose(new Quaternionf().rotationAxis(
    ...
    private static ModeSetting<Mode> animationModes() {
        ModeSetting.Builder<Mode> modes = new ModeSetting.Builder<Mode>()
