...
public final class HudLayoutController {
    ...
    public void render(GuiGraphicsExtractor graphics, double mouseX, double mouseY,
        ...
        if (guideY) graphics.horizontalLine(0, screenWidth, screenHeight / 2, withAlpha(accent, 165));
        ...
        for (HudEditorRegistry.Target target : targets) {
            ...
            SmoothGui.roundedOutline(graphics, left, top, right, bottom, 4, 1,
                    withAlpha(focused ? accent : 0x999999, focused ? 225 : 90), 0x00000000);
        }
    }
    ...
    private void move(double mouseX, double mouseY, int screenWidth, int screenHeight) {
        ...
        if (Math.abs(x + bounds.width() - screenWidth) <= SNAP_DISTANCE) x = screenWidth - bounds.width();
        if (Math.abs(y + bounds.height() - screenHeight) <= SNAP_DISTANCE) y = screenHeight - bounds.height();
    ...
    private void resize(double mouseX, double mouseY, int screenWidth, int screenHeight) {
        ...
        double denominator = resizeStartWidth * resizeStartWidth + resizeStartHeight * resizeStartHeight;
        if (denominator <= 0.0D) return;
        double ratio = (localX * resizeStartWidth + localY * resizeStartHeight) / denominator;
        active.resize(resizeStartScale * ratio, resizeAnchorX, resizeAnchorY, screenWidth, screenHeight);
    }
    ...
    private static int accentColor() {
        try {
            return Integer.parseInt(Settings.getString("clickgui.theme.color", "#b29a65")
                    .replace("#", ""), 16) & 0xFFFFFF;
    ...
    private enum Action { NONE, MOVE, RESIZE }
}
