...
public final class HumanAimSimulator {
    ...
    public static PacketMotion samplePacketMotion(boolean lockMode, double urgency) {
        double yawAcceleration = lockMode
        ...
        double pitchAcceleration = lockMode
                ? RandomMath.between(1.45D + urgency * 0.15D,
                1.85D + urgency * 0.25D)
                : RandomMath.between(1.05D + urgency * 0.15D,
                1.45D + urgency * 0.20D);
        pitchAcceleration = Math.min(
                pitchAcceleration, MATRIX_MAX_PACKET_PITCH_ACCELERATION);
        double yawError = RandomMath.between(lockMode ? -0.12D : -0.18D,
                lockMode ? 0.12D : 0.18D);
    ...
    public static PacketMotion sampleResponsivePacketMotion(
            boolean lockMode, double urgency) {
        double yawAcceleration = lockMode
                ? RandomMath.between(20.0D + urgency * 7.0D,
                29.0D + urgency * 10.0D)
                : RandomMath.between(12.0D + urgency * 4.0D,
                18.0D + urgency * 7.0D);
        double pitchAcceleration = lockMode
                ? RandomMath.between(3.2D + urgency * 0.8D,
                4.6D + urgency * 1.1D)
                : RandomMath.between(2.2D + urgency * 0.5D,
                3.3D + urgency * 0.8D);
        double yawError = RandomMath.between(lockMode ? -0.10D : -0.16D,
                lockMode ? 0.10D : 0.16D);
        double pitchError = RandomMath.between(-0.04D, 0.04D);
        return new PacketMotion(yawAcceleration, pitchAcceleration,
                yawError, pitchError);
    }
    ...
    public static float acceleratedAxisStep(
        ...
        float desiredStep = (float) Math.copySign(
                Math.min(Math.abs(demand), Math.min(maxStep, brakingSpeed)), demand);
        double errorBlend = Mth.clamp(Math.abs(demand) / 5.0D, 0.18D, 1.0D);
        desiredStep = Mth.clamp(
                desiredStep + (float) (constantError * errorBlend), -maxStep, maxStep);
        return previousStep + (float) Mth.clamp(
    ...
    public static float quantizeMouseStep(float lastSent, float desired, double sensitivity) {
        ...
        if (!Double.isFinite(gcd) || gcd <= 1.0E-7D) return desired;
        double difference = MathUtils.wrappedAngleDifference(lastSent, desired);
        return lastSent + (float) (Math.round(difference / gcd) * gcd);
    }
