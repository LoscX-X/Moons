...
public final class ModelOverlayRenderer {
    ...
    /** Reuses uploaded geometry while the marked batch still follows its normal output target. */
    public static boolean captureDraw(
            PreparedRenderType type, StagedVertexBuffer.ExecuteInfo info) {
        ...
        try {
            ...
            new PreparedRenderType(
                            type.pipeline(),
                            capture.layer().outputTarget(),
                            type.dynamicTransforms(),
                            type.scissorState(),
                            type.textures())
                    .drawFromBuffer(info);
