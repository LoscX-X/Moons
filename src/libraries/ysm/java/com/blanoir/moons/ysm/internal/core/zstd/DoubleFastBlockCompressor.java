...
class DoubleFastBlockCompressor implements BlockCompressor {
    ...
    public int compressBlock(
        ...
        // Offsets in hash tables are relative to baseAddress. Hash tables can be reused across
        // calls to compressBlock as long as
        // baseAddress is kept constant.
        // We don't want to generate sequences that point before the current window limit, so we
        // "filter" out all results from looking up in the hash tables
        // beyond that point.
        ...
        final long inputLimit =
                inputEnd - SIZE_OF_LONG; // We read a long at a time for computing the hashes
        ...
        while (input < inputLimit) { // < instead of <=, because repcode check at (input+1)
            ...
            if (offset1 > 0
                    && UNSAFE.getInt(inputBase, input + 1 - offset1)
                ...
                matchLength =
                        count(
                                        ...
                                        input + 1 + SIZE_OF_INT,
                                        inputEnd,
                                        input + 1 + SIZE_OF_INT - offset1)
                                + SIZE_OF_INT;
                input++;
                output.storeSequence(
                        inputBase, anchor, (int) (input - anchor), 0, matchLength - MIN_MATCH);
            } else {
                // check prefix long match
                if (longMatchAddress > windowBaseAddress
                        && UNSAFE.getLong(inputBase, longMatchAddress)
                                == UNSAFE.getLong(inputBase, input)) {
                    matchLength =
                            count(
                                            inputBase,
                                            input + SIZE_OF_LONG,
                    ...
                    while (input > anchor
                            ...
                            && UNSAFE.getByte(inputBase, input - 1)
                                    == UNSAFE.getByte(inputBase, longMatchAddress - 1)) {
                        input--;
                        longMatchAddress--;
                        matchLength++;
                    }
                    ...
                    if (shortMatchAddress > windowBaseAddress
                            && UNSAFE.getInt(inputBase, shortMatchAddress)
                                    == UNSAFE.getInt(inputBase, input)) {
                        int nextOffsetHash =
                                hash8(UNSAFE.getLong(inputBase, input + 1), longHashBits);
                        ...
                        if (nextOffsetMatchAddress > windowBaseAddress
                                && UNSAFE.getLong(inputBase, nextOffsetMatchAddress)
                                        == UNSAFE.getLong(inputBase, input + 1)) {
                            matchLength =
                                    count(
                                                    inputBase,
                            ...
                            while (input > anchor
                                    ...
                                    && UNSAFE.getByte(inputBase, input - 1)
                                            == UNSAFE.getByte(inputBase, shortMatchAddress - 1)) {
                                input--;
                                shortMatchAddress--;
                                matchLength++;
                            }
            ...
            if (input <= inputLimit) {
                ...
                longHashTable[
                                hash8(
                                        UNSAFE.getLong(inputBase, baseAddress + current + 2),
                                        longHashBits)] =
                        current + 2;
                shortHashTable[
                                hash(
                                        inputBase,
                                        baseAddress + current + 2,
                                        shortHashBits,
                                        matchSearchLength)] =
                        current + 2;

                longHashTable[hash8(UNSAFE.getLong(inputBase, input - 2), longHashBits)] =
                ...
                while (input <= inputLimit
                        && offset2 > 0
                        && UNSAFE.getInt(inputBase, input)
                                == UNSAFE.getInt(inputBase, input - offset2)) {
                    int repetitionLength =
                            count(
                                            inputBase,
                                            input + SIZE_OF_INT,
                                            inputEnd,
                                            input + SIZE_OF_INT - offset2)
                                    + SIZE_OF_INT;
                    ...
                    longHashTable[hash8(UNSAFE.getLong(inputBase, input), longHashBits)] =
                            (int) (input - baseAddress);
    ...
    public static int count(
        ...
        while (count < remaining
                && UNSAFE.getByte(inputBase, match) == UNSAFE.getByte(inputBase, input)) {
            count++;
            input++;
            match++;
        }
