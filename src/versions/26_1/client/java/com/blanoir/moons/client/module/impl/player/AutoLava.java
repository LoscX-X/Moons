...
public final class AutoLava {
    ...
    public static void onAttackDispatched(
            Entity entity,
            boolean criticalEligibleBeforeAttack
    ) {
        Minecraft client = Minecraft.getInstance();
        if (!ENABLED.get() || !ready(client) || isBusy() || cooldownTicks > 0
                || AntiWeb.isBusy() || AntiLava.isBusy() || AutoWeb.isBusy()
        ...
        pendingExecuteAtNanos = System.nanoTime()
                + TRIGGER_DELAY_MS.get() * 1_000_000L;
        ...
        if (phase == CyclePhase.IDLE) {
            if (pendingTargetId >= 0) {
                ...
                if (!Targeting.isValidTargetPlayer(client, target) || target.isOnFire()) {
                    ...
                    if (GROUND_ENABLED.get() && landing.expired()
    ...
    private static void tryBeginPendingCycle(
        ...
        if (!pendingWallAttempted) {
            ...
            if (WALL_ENABLED.get()) {
                PlacementPlan wallPlan = planAtTarget(
                        client, target, PlacementRoute.WALL);
                if (wallPlan != null) {
                    beginPendingCycle(
                            client, PlacementRoute.WALL, wallPlan);
                    return;
                }
            }
        }
        ...
        PlacementPlan groundPlan = planAtTarget(
                client, target, PlacementRoute.GROUND);
        if (groundPlan != null) {
    ...
    private static void beginPendingCycle(
        ...
        clearPendingTrigger();
        if (AntiWeb.isBusy() || AntiLava.isBusy() || AutoWeb.isBusy()
                || AutoBed.isBusy() || AutoObsidian.isBusy()) {
            clearMotionSampling();
            return;
        }
        ...
        cycleTicks = 0;
        slotReadyAtNanos = System.nanoTime()
    ...
    private static void tickCycle(Minecraft client) {
        ...
        if (phase == CyclePhase.CLICKING_TO_PICKUP) {
            ...
            boolean bucketRefilled = bucketSlot >= 0
                    && client.player.getInventory().getItem(bucketSlot).is(Items.LAVA_BUCKET);
            if (!bucketRefilled && isLavaSource(client, lavaPos)
    ...
    private static void continueHoldingLava(
            Minecraft client,
            boolean advanceSyncTimeout
    ) {
        if (phase != CyclePhase.HOLDING_LAVA) {
            return;
        }
        boolean bucketEmptied = bucketSlot >= 0
                && client.player.getInventory().getItem(bucketSlot).is(Items.BUCKET);
        if (!bucketEmptied || !isLavaSource(client, lavaPos)) {
            if (!advanceSyncTimeout
                    || ++bucketSyncWaitTicks <= MAX_BUCKET_SYNC_TICKS) {
                return;
            }
            beginReturnRotation(client);
    ...
    private static void beginAfterSlotDelay(Minecraft client) {
        ...
        planningViewDirection = client.player.getLookAngle();
        PlacementPlan plan = planAtTarget(
                client, target, activePlacementRoute);
        if (plan == null) {
            finishCycle(client);
            return;
        }
    ...
    private static void scheduleOriginalSlotRestore() {
        ...
        deferredRestoreAtNanos = System.nanoTime()
                + RETURN_SWITCH_DELAY_MS.get() * 1_000_000L;
    }
    ...
    private static void abortCycle(Minecraft client) {
        ...
        cooldownTicks = Math.max(
                10, (int) Math.ceil(COOLDOWN_SECONDS.get() * 20.0D));
        reset(client);
    }
    ...
    private static void transitionIfCurrent(
            int generation,
            CyclePhase expected,
            CyclePhase next
    ) {
    ...
    private static boolean sentLookReachesLavaSource(
            Minecraft client,
            BlockPos source
    ) {
    ...
    private static boolean sentLookMatchesPlacement(
        ...
        BlockHitResult hit = client.level.clip(new ClipContext(
                ...
                client.player));
        return hit.getType() == HitResult.Type.BLOCK
                && hit.getBlockPos().equals(plannedHit.getBlockPos())
                && hit.getDirection() == plannedHit.getDirection();
    }
    ...
    private static void beginMotionSampling(Minecraft client, Player target) {
        ...
        awaitingAttackVelocity = horizontalDifference(
                anticipatedAttackVelocity, attackBaselineVelocity)
                > KNOCKBACK_SAMPLE_THRESHOLD;
        knockbackWaitTicks = 0;
        smoothedTargetVelocity = awaitingAttackVelocity
                ? anticipatedAttackVelocity : attackBaselineVelocity;
    ...
    private static void sampleTargetMotion(Player target) {
        ...
        if (sampledTargetId != target.getId()
        ...
        Vec3 observed = position.subtract(lastSampledTargetPosition)
                .scale(1.0D / elapsedTicks);
        Vec3 reported = target.getDeltaMovement();
        Vec3 sample = limitHorizontal(
                observed.scale(0.72D).add(reported.scale(0.28D)), 1.5D);
        ...
        if (awaitingAttackVelocity
            ...
            if (impulse >= KNOCKBACK_SAMPLE_THRESHOLD) {
                ...
                smoothedTargetVelocity = previousVelocity.scale(0.55D)
                        .add(sample.scale(0.45D));
                Vec3 observedAcceleration = smoothedTargetVelocity
                        .subtract(previousVelocity);
                smoothedTargetAcceleration = smoothedTargetAcceleration.scale(0.65D)
    ...
    private static Vec3 expectedPostAttackVelocity(
            Minecraft client,
            Player target
    ) {
        ...
        double impulse = levels * 0.5D * (1.0D - Mth.clamp(
    ...
    private static Vec3 predictHorizontalPosition(
        ...
        double velocityX = velocity.x + Mth.clamp(
                acceleration.x, -0.08D, 0.08D) * Math.min(1.0D, remaining);
        double velocityZ = velocity.z + Mth.clamp(
                acceleration.z, -0.08D, 0.08D) * Math.min(1.0D, remaining);
        while (remaining > 1.0E-6D) {
            double fraction = Math.min(1.0D, remaining);
            Vec3 moved = collideHorizontal(
    ...
    private static double clipAxis(
        ...
        AABB full = xAxis
                ? box.move(requested, 0.0D, 0.0D)
                : box.move(0.0D, 0.0D, requested);
    ...
    private static boolean safeForLocalPlayer(
            Minecraft client,
            BlockPos source,
            double leadTicks
    ) {
        Vec3 predicted = predictHorizontalPosition(
                client, client.player, client.player.getDeltaMovement(),
                Vec3.ZERO, Math.min(3.0D, Math.max(0.0D, leadTicks)));
        Vec3 movement = predicted.subtract(client.player.position());
        AABB current = client.player.getBoundingBox().inflate(
                SELF_SAFETY_MARGIN, 0.05D, SELF_SAFETY_MARGIN);
    ...
    private static PlacementPlan planAtTarget(
        ...
        Vec3 velocity = sampledTargetId == target.getId()
                ? smoothedTargetVelocity : target.getDeltaMovement();
        Vec3 acceleration = sampledTargetId == target.getId()
                ? smoothedTargetAcceleration : Vec3.ZERO;
        double predictionDelayTicks = PREDICTION_DELAY_MS.get() / 50.0D;
        double leadTicks = (SMOOTH_TICKS.get() + 1.5D + predictionDelayTicks)
                * strength;
        Vec3 predicted = predictHorizontalPosition(
                client, target, velocity, acceleration, leadTicks);
        BlockPos desired = BlockPos.containing(
                predicted.x, target.getBoundingBox().minY + 1.0E-4D, predicted.z);
        Vec3 eye = client.player.getEyePosition();
        Vec3 look = planningViewDirection != null
                && planningViewDirection.lengthSqr() > 1.0E-9D
        ...
        AABB predictedTarget = currentTarget.move(
                targetMovement.x, 0.0D, targetMovement.z);
        AABB targetSweep = new AABB(
        ...
        PlacementPlan cobwebTopPlan = planAboveTargetCobweb(
                client, desired, targetSweep, predicted, eye, look,
                rayLength, leadTicks);
        ...
        for (int x = -PLACEMENT_SCAN_RADIUS; x <= PLACEMENT_SCAN_RADIUS; x++) {
            for (int z = -PLACEMENT_SCAN_RADIUS; z <= PLACEMENT_SCAN_RADIUS; z++) {
                ...
                double targetDistance = Math.pow(placeCenter.x - predicted.x, 2.0D)
                        + Math.pow(placeCenter.z - predicted.z, 2.0D);
                for (Direction supportDirection : SUPPORT_DIRECTIONS) {
                    if (route == PlacementRoute.GROUND
                    ...
                    Vec3 nearest = AimPointUtils.closest(
                            facePlane, eye, look, rayLength);
                    BlockHitResult visibleHit = visibleFaceHit(
                            client, supportPos, face, nearest);
                    if (visibleHit == null
                            || !withinRange(client, visibleHit.getLocation())
                            || !withinFov(client, visibleHit.getLocation(), look)) {
                        continue;
                    }
                    double rayDistance = squaredDistanceToViewRay(
                            eye, look, visibleHit.getLocation(), rayLength);
                    double eyeDistance = eye.distanceToSqr(visibleHit.getLocation());
                    double score = targetDistance * 8.0D
                            + rayDistance * 0.8D
                            + eyeDistance * 0.015D
        ...
        for (int x = -PLACEMENT_SCAN_RADIUS; x <= PLACEMENT_SCAN_RADIUS; x++) {
            for (int z = -PLACEMENT_SCAN_RADIUS; z <= PLACEMENT_SCAN_RADIUS; z++) {
                ...
                BlockHitResult hit = visibleFaceHit(
                        client, webPos, Direction.UP, nearest);
                if (hit == null
                ...
                double score = Math.pow(webCenter.x - predicted.x, 2.0D)
                        + Math.pow(webCenter.z - predicted.z, 2.0D)
                        + squaredDistanceToViewRay(
                        eye, look, hit.getLocation(), rayLength) * 0.1D;
                if (score < bestScore) {
                    bestScore = score;
    ...
    private static AABB insetFacePlane(BlockPos supportPos, Direction face) {
        ...
        return switch (face.getAxis()) {
            case X -> {
                double x = face == Direction.EAST
                        ? supportPos.getX() + 1.0D : supportPos.getX();
                yield new AABB(x, minY, minZ, x, maxY, maxZ);
            }
            case Y -> {
                double y = face == Direction.UP
                        ? supportPos.getY() + 1.0D : supportPos.getY();
                yield new AABB(minX, y, minZ, maxX, y, maxZ);
            }
            case Z -> {
                double z = face == Direction.SOUTH
                        ? supportPos.getZ() + 1.0D : supportPos.getZ();
    ...
    private static BlockHitResult visibleFaceHit(
        ...
        Vec3 justInsideSupport = planePoint.add(
        ...
        return new BlockHitResult(
                hit.getLocation(), face, supportPos, hit.isInside());
    }
    ...
    private static double squaredDistanceToViewRay(
        ...
        double alongRay = Mth.clamp(
                point.subtract(eye).dot(direction), 0.0D, rayLength);
        return point.distanceToSqr(eye.add(direction.scale(alongRay)));
    }

    private static boolean validatePlacement(Minecraft client) {
        if (lavaPos == null || placementHit == null
                || bucketSlot < 0
        ...
        if (!state.canBeReplaced() || !state.getFluidState().isEmpty()
                || !safeForLocalPlayer(
                client, lavaPos, Math.max(1.0D, activeSmoothTicks + 1.0D))) {
            return false;
        }
    ...
    private static boolean withinRange(Minecraft client, Vec3 point) {
        double range = Math.min(RANGE.get(), client.player.blockInteractionRange());
    ...
    private static boolean withinFov(Minecraft client, Vec3 point, Vec3 look) {
        return MathUtils.withinFov(MathUtils.viewAngle(
                client.player.getEyePosition(), look, point), FOV.get());
    }
    ...
    public static int showStatus(Minecraft client) {
        ClientChat.send(client,
                "AutoLava: " + (ENABLED.get() ? "enabled" : "disabled")
                        + ", critical chance: " + format(CHANCE.get())
                        + ", range: " + format(RANGE.get())
                        + ", fov: " + format(FOV.get())
                        + ", wall: " + (WALL_ENABLED.get() ? "enabled" : "disabled")
                        + ", ground: " + (GROUND_ENABLED.get() ? "enabled" : "disabled")
                        + ", trigger delay: " + TRIGGER_DELAY_MS.get() + "ms"
                        + ", switch delay: " + SWITCH_DELAY_MS.get() + "ms"
                        + ", prediction: " + format(PREDICTION.get()) + "x"
                        + ", prediction delay: " + PREDICTION_DELAY_MS.get() + "ms"
                        + ", pickup delay: " + PICKUP_DELAY_MS.get() + "ms"
                        + ", return switch delay: " + RETURN_SWITCH_DELAY_MS.get() + "ms"
                        + ", smooth: " + SMOOTH_TICKS.get() + "t"
    ...
    public static int setGroundEnabled(Minecraft client, boolean value) {
        ...
        ClientChat.send(client, "AutoLava post-hit landing route "
    ...
    public static int setReturnSwitchDelayMs(Minecraft client, int value) {
        ...
        ClientChat.send(client, "AutoLava return switch delay set to "
                + RETURN_SWITCH_DELAY_MS.get() + " ms.");
        return 1;
    }

    public static int setSmoothTicks(Minecraft client, int value) {
        SMOOTH_TICKS.set(value);
        ClientChat.send(client, "AutoLava smooth turn set to "
                + SMOOTH_TICKS.get() + " ticks.");
        return 1;
    }

    public static int setCooldown(Minecraft client, double value) {
        COOLDOWN_SECONDS.set(value);
        ClientChat.send(client, "AutoLava cooldown set to "
    ...
    public static String debugState() {
        return phase.name().toLowerCase(java.util.Locale.ROOT)
                + " phase=" + phaseTicks + "t"
                + " cycle=" + cycleTicks + "t"
                + " switch=" + (returnSwitchScheduled ? "queued" : "none")
