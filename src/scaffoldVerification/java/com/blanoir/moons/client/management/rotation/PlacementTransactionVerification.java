...
public final class PlacementTransactionVerification {
    ...
    private static void verifyContinuationBoundaries(
        ...
        Method defer = SilentPacketRotation.class.getDeclaredMethod("deferRotation", Runnable.class);
        defer.setAccessible(true);
        RotationLease lease = (RotationLease) field("ROTATION_LEASE").get(null);
        int[] calls = {0};
        boolean[] restored = {false};
        Runnable next = () -> {
            require(!lock.locked() && RotationLease.submission() == null, "continuation sees a closed window");
            require(restored[0], "continuation never reads temporary packet camera angles");
            require(RotationHistory.latest().yaw() == 42, "continuation starts from the closing send");
            calls[0]++;
        };
        ...
        post(look);
        RotationLease.finishMotion();
        require(calls[0] == 3, "a new request accepted after reset survives old-transaction cleanup");
