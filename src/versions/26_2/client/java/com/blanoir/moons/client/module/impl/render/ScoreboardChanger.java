...
public final class ScoreboardChanger {
    ...
    private static final StringSetting CUSTOM_LINES =
            new StringSetting.Builder()
                    ...
                    .defaultValue("<#55c8ff>Moons Client</#55c8ff>|<wave:#55c8ff:#b675ff:0.5>play.example.net</wave>")
                    .build();
    ...
    public static void init() {
        ...
        initialized = true;
        EventBus.HUD_RENDER.register("ScoreboardChanger.hudRender", event -> renderFallback(event.graphics()));
    }
    ...
    public static boolean render(GuiGraphicsExtractor graphics, Objective objective) {
        ...
        Component title = REPLACE_TITLE.get()
                ? DynamicMiniMessage.parse(TITLE.get(), animationTime)
                : objective.getDisplayName();

        List<Line> lines = MODE.get() == Mode.CUSTOM
                ? customLines(animationTime)
                : originalLines(objective);

        if (MODE.get() == Mode.LAST_LINE && !lines.isEmpty()) {
            int last = lines.size() - 1;
            Line original = lines.get(last);
            lines.set(last, new Line(
    ...
    private static Objective currentObjective() {
        ...
        if (team != null) {
            DisplaySlot teamSlot = team.getColor()
                    .map(color -> color.displaySlot())
                    .orElse(null);
            if (teamSlot != null) {
    ...
    private static List<Line> originalLines(Objective objective) {
        ...
        return new ArrayList<>(visibleEntries(objective).stream()
                .map(entry -> new Line(
                        PlayerTeam.formatNameForTeam(
                                scoreboard.getPlayersTeam(entry.owner()), entry.ownerName()),
                        SHOW_SCORES.get() ? entry.formatValue(numberFormat) : Component.empty()))
                .toList());
    }
    ...
    public static void onScoreboardPacketApplied(Packet<?> packet) {
        ...
        if (packet instanceof ClientboundSetScorePacket score) {
            ...
            if (linePatched && dataObjective != null
                ...
                originalLineDisplay = findEntry(dataObjective, lineOwner)
    ...
    private static void syncClientData() {
        ...
        detachLineTeam();
        scoreAccess(objective, lineOwner).display(
                DynamicMiniMessage.parse(LAST_LINE.get(), animationTime));
    }

    private static void syncAddedLine(Objective objective, long animationTime) {
        List<PlayerScoreEntry> serverEntries = objective.getScoreboard()
                .listPlayerScores(objective).stream()
                .filter(entry -> !entry.isHidden())
        ...
        int bottomScore = serverEntries.isEmpty() ? 0
                : serverEntries.get(serverEntries.size() - 1).value();
    ...
    private static void restoreAddedLine() {
        if (addedLine && addedLineObjective != null) {
            addedLineObjective.getScoreboard().resetSinglePlayerScore(
    ...
    private static ScoreAccess scoreAccess(Objective objective, String owner) {
        return objective.getScoreboard().getOrCreatePlayerScore(
                ScoreHolder.forNameOnly(owner), objective, true);
    }
    ...
    private static void restoreLine() {
        if (linePatched && dataObjective != null) {
            boolean stillExists = dataObjective.getScoreboard().listPlayerScores(dataObjective).stream()
    ...
    private static List<Line> customLines(long animationTime) {
        ...
        for (String value : splitLines(CUSTOM_LINES.get())) {
            if (!value.isBlank()) {
                lines.add(new Line(DynamicMiniMessage.parse(value.trim(), animationTime), Component.empty()));
            }
    ...
    private static List<String> splitLines(String value) {
        ...
        for (int offset = 0; offset < normalized.length();) {
            ...
            if (escaped) {
                ...
                if (codePoint != '\r' || offset >= normalized.length()
    ...
    private static void draw(GuiGraphicsExtractor graphics, Font font,
                             Component title, List<Line> lines) {
        int width = font.width(title);
        int colonWidth = font.width(":");
        for (Line line : lines) {
            int scoreWidth = font.width(line.score());
            int lineWidth = font.width(line.name()) + (scoreWidth > 0 ? colonWidth + scoreWidth : 0);
        ...
        graphics.text(font, title, left + width / 2 - font.width(title) / 2, top - 9, 0xFFFFFFFF, false);
