...
public final class BacktrackRenderer {
    private static final RenderPipeline FILLED_PIPELINE = registerPipeline(
            "pipeline/moons_backtrack_filled", PrimitiveTopology.QUADS
    );
    private static final RenderPipeline LINE_PIPELINE = registerPipeline(
            "pipeline/moons_backtrack_lines", PrimitiveTopology.LINES
    );
    ...
    public static void renderBox(PoseStack matrices, AABB box, int fillArgb, int outlineArgb, String label) {
        ...
        if (alpha(fillArgb) > 0) {
            drawBoxMesh(client, FILLED_PIPELINE, List.<Vector4f[]>of(corners), FACES, fillArgb, label + " fill");
        }
        if (alpha(outlineArgb) > 0) {
            drawBoxMesh(client, LINE_PIPELINE, List.<Vector4f[]>of(corners), EDGES, outlineArgb, label + " outline");
        }
    }
    ...
    private static Vector4f[] transformedCorners(Matrix4fc transform, AABB box) {
        ...
        for (int index = 0; index < values.length; index++) {
            result[index] = transform.transform(new Vector4f(
    ...
    private static void drawBoxMesh(Minecraft client, RenderPipeline pipeline, List<Vector4f[]> boxes,
                                    int[][] groups, int color, String label) {
        float red = ((color >>> 16) & 0xFF) / 255.0f;
        float green = ((color >>> 8) & 0xFF) / 255.0f;
        float blue = (color & 0xFF) / 255.0f;
        float opacity = alpha(color) / 255.0f;
        WorldOverlayBuffer.draw(client, pipeline, label, builder -> {
    ...
    private static void vertex(VertexConsumer builder, Vector4f point, float red, float green, float blue, float alpha) {
    ...
    private static RenderPipeline registerPipeline(String name, PrimitiveTopology topology) {
        return GameAccess.registerPipeline(RenderPipeline.builder(GameAccess.debugFilledSnippet())
