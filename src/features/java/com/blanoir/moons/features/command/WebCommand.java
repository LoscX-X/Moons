    ...
    static boolean handle(String tail) {
        ...
        switch (operation) {
            ...
            case "unbind" -> {
                ...
                ClientChat.send(client, "Web server binding removed. Automatic connection is disabled.");
            }
            default -> ClientChat.send(client,
                    "Usage: .web <bind url|status|connect|pair|disconnect|unbind>");
        }
        return true;
    }
