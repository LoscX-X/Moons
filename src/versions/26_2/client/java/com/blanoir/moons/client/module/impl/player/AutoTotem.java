...
public final class AutoTotem {
    ...
    private static void tick(Minecraft client) {
        ...
        boolean offhandHasTotem = client.player.getOffhandItem().getItem() == Items.TOTEM_OF_UNDYING;
    ...
    private static void equipTotem(Minecraft client) {
        ...
        if (inventorySlot == -1) {
            nextActionAtMs = System.currentTimeMillis() + Math.max(SWITCH_DELAY_MS.get(), RETRY_DELAY_MS);
            return;
        }
    ...
    private static void beginInventoryClicks(
            Minecraft client,
    ...
    private static void tickInventoryClicks(Minecraft client) {
        ...
        switch (inventoryClickStep) {
            case EQUIP_PICKUP_TOTEM -> {
                if (sourceMenuSlot < 0
                        || client.player.inventoryMenu.getSlot(sourceMenuSlot)
                        .getItem().getItem() != Items.TOTEM_OF_UNDYING) {
    ...
    private static void abortInventoryClicks(Minecraft client) {
        ...
        nextActionAtMs = System.currentTimeMillis()
                + Math.max(SWITCH_DELAY_MS.get(), RETRY_DELAY_MS);
    }

    private static void closeModuleInventory(Minecraft client) {
        if (inventoryOpenedByModule && MinecraftClientAccess.screen(client) instanceof InventoryScreen) {
    ...
    private static boolean inventoryScreenAvailable(Minecraft client) {
        return MinecraftClientAccess.screen(client) == null || MinecraftClientAccess.screen(client) instanceof InventoryScreen;
    }
    ...
    private static boolean healthBelowThreshold(Minecraft client) {
        ...
        float allowedDamage = health - (safetyOperating ? SAFE_THRESHOLD.get() : HEALTH_THRESHOLD.get());
        ...
        calculatedDamage = Math.max(calculatedDamage, explosionDamageFromBlocks(client, allowedDamage));
        if (calculatedDamage >= allowedDamage) {
            return true;
        }
    ...
    public static int showStatus(Minecraft client) {
        ClientChat.send(
                ...
                "AutoTotem: " + statusText()
                        + ", threshold: " + HEALTH_THRESHOLD.get()
                        + ", safeThreshold: " + SAFE_THRESHOLD.get()
                        + ", missingArmor: " + toggleText(MISSING_ARMOR.get())
                        + ", safety: " + toggleText(SAFETY_ENABLED.get())
                        + ", subtractDamage: " + toggleText(SUBTRACT_CALCULATED_DAMAGE.get())
                        + ", explosionEntities: " + toggleText(PREDICT_EXPLOSION_ENTITIES.get())
                        + ", explosionBlocks: " + toggleText(PREDICT_EXPLOSION_BLOCKS.get())
                        + ", fallDamage: " + toggleText(PREDICT_FALL_DAMAGE.get())
    ...
    public static int setSubtractDamage(Minecraft client, boolean value) {
        ...
        ClientChat.send(client, "AutoTotem subtractDamage " + toggleText(SUBTRACT_CALCULATED_DAMAGE.get()) + ".");
        return 1;
    }

    public static int setExplosionEntities(Minecraft client, boolean value) {
        PREDICT_EXPLOSION_ENTITIES.set(value);
        ClientChat.send(client, "AutoTotem explosionEntities " + toggleText(PREDICT_EXPLOSION_ENTITIES.get()) + ".");
        return 1;
    }

    public static int setExplosionBlocks(Minecraft client, boolean value) {
        PREDICT_EXPLOSION_BLOCKS.set(value);
        ClientChat.send(client, "AutoTotem explosionBlocks " + toggleText(PREDICT_EXPLOSION_BLOCKS.get()) + ".");
        return 1;
    }

    public static int setFallDamage(Minecraft client, boolean value) {
        PREDICT_FALL_DAMAGE.set(value);
        ClientChat.send(client, "AutoTotem fallDamage " + toggleText(PREDICT_FALL_DAMAGE.get()) + ".");
        return 1;
    }
