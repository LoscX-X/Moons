package com.blanoir.moons.ysm.internal.molang.parser;

import static java.util.Objects.requireNonNull;

import com.blanoir.moons.ysm.internal.molang.lexer.MolangLexer;
import com.blanoir.moons.ysm.internal.molang.lexer.Token;
import com.blanoir.moons.ysm.internal.molang.lexer.TokenKind;
import com.blanoir.moons.ysm.internal.molang.parser.ast.*;
...
public final class MolangParserImpl implements MolangParser {
    ...
    Expression parseSingle(final @NotNull MolangLexer lexer) throws IOException {
        ...
        switch (token.kind()) {
            ...
            case LBRACE:
                ...
                while (token.kind() != TokenKind.RBRACE) {
                    ...
                    if (current.kind() == TokenKind.ERROR) {
                        throw new ParseException(
                                "Found an invalid token (error): " + current.value(),
                                lexer.cursor());
                    }
            ...
            case IDENTIFIER:
                ...
                if (lastTarget == null) {
                    throw new ParseException(
                            "Failed to get property: " + token.value(), lexer.cursor());
                }
                ...
                if (token.kind() == TokenKind.DOT) {
                    ...
                    if (token.kind() != TokenKind.IDENTIFIER) {
                        throw new ParseException(
                                "Unexpected token, expected a valid field token", lexer.cursor());
                    }
                    ...
                    if (lastTarget instanceof ObjectBinding) {
                        ...
                        throw new ParseException(
                                "Illegal access to : " + token.value(), lexer.cursor());
                    }
                    if (lastTarget == null) {
                        throw new ParseException(
                                "Failed to get property: " + token.value(), lexer.cursor());
            ...
            case SUB:
                ...
                return new UnaryExpression(
                        ...
                        parseCompoundExpression(
                                lexer, UnaryExpression.Op.ARITHMETICAL_NEGATION.precedence()));
            case BANG:
                lexer.next();
                return new UnaryExpression(
                        UnaryExpression.Op.LOGICAL_NEGATION,
                        parseCompoundExpression(
                                lexer, UnaryExpression.Op.LOGICAL_NEGATION.precedence()));
            case RETURN:
                lexer.next();
                return new UnaryExpression(
    ...
    @NotNull
    Expression parseCompoundExpression(final @NotNull MolangLexer lexer, final int lastPrecedence)
            throws IOException {
    ...
    Expression parseCompound(
        ...
        if (left instanceof CallExpression callExpression) {
            if (current.kind() == TokenKind.LPAREN) {
                ...
                return new CallExpression(
                        callExpression.function(), new Function.ArgumentCollection(arguments));
            }

            if (!callExpression
                    .function()
                    .validateArgumentSize(callExpression.arguments().size())) {
                throw new ParseException("Illegal function arguments size", lexer.cursor());
            }
        }
            ...
            case LPAREN:
                {
                    ...
                    Expression right =
                            parseCompoundExpression(lexer, BinaryExpression.Op.MUL.precedence());
            ...
            case QUES:
                {
                    ...
                    if (lexer.current().kind() == TokenKind.COLON) {
                        // then it's a ternary expression, since there is a ':', indicating the next
                        // expression
                        lexer.next();
                        return new TernaryConditionalExpression(
                                left, trueValue, parseCompoundExpression(lexer, PRECEDENCE_QUES));
                    } else {
                        return new BinaryExpression(
                                BinaryExpression.Op.CONDITIONAL, left, trueValue);
                    }
                }
        ...
        if (current.kind() == TokenKind.DOT) {
            ...
            if (current.kind() == TokenKind.IDENTIFIER) {
                ...
                throw new ParseException(
                        "Expect a identifier after struct access operator", lexer.cursor());
            }
        }
    ...
    public @Nullable Expression current() {
        if (current == UNSET_FLAG) {
            throw new IllegalStateException(
                    "No current parsed expression, call next() at least once!");
        }
