...
import static com.blanoir.moons.ysm.internal.core.zstd.BitInputStream.isEndOfStream;
import static com.blanoir.moons.ysm.internal.core.zstd.BitInputStream.peekBitsFast;
...
import static com.blanoir.moons.ysm.internal.core.zstd.Util.isPowerOf2;
...
class Huffman {
        ...
        if (inputSize >= 128) {
            ...
            input +=
                    reader.readFseTable(
                            ...
                            MAX_FSE_TABLE_LOG);
            outputSize =
                    FiniteStateEntropy.decompress(fseTable, inputBase, input, inputLimit, weights);
        }
        ...
        while (output < fastOutputLimit) {
            BitInputStream.Loader loader =
                    new BitInputStream.Loader(
                            inputBase, inputAddress, currentAddress, bits, bitsConsumed);
    ...
    public void decode4Streams(
        ...
        verify(
                start2 < start3 && start3 < start4 && start4 < inputLimit,
                inputAddress,
                "Input is corrupted");
        ...
        verify(
                output1 <= outputStart2 && output2 <= outputStart3 && output3 <= outputStart4,
                inputAddress,
                "Input is corrupted");
    ...
    private void decodeTail(
        ...
        while (outputAddress < outputLimit) {
            BitInputStream.Loader loader =
                    new BitInputStream.Loader(
                            inputBase, startAddress, currentAddress, bits, bitsConsumed);
            boolean done = loader.load();
            bitsConsumed = loader.getBitsConsumed();
            bits = loader.getBits();
            currentAddress = loader.getCurrentAddress();
        ...
        verify(
                isEndOfStream(startAddress, currentAddress, bitsConsumed),
                startAddress,
