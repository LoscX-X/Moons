...
final class PlacementRotationHistory {
    ...
    boolean wouldRepeatNext(float nextYaw) {
        ...
        return repeatsPlacementDelta(delta)
                || rotatedSincePlacement && delta > 2.0F
                && Math.abs(delta - deltaYaw) < 0.0001F;
    }

    /** Prefer the selected angle, then nearby mouse steps in both directions. */
    Float variedYaw(float desiredYaw, float mouseStep,
                    UnaryOperator<Float> quantize, Predicate<Float> reachable) {
        for (int i = 0; i <= 8; i++) {
            int offset = i == 0 ? 0 : ((i + 1) / 2) * (i % 2 == 1 ? 1 : -1);
            float candidate = offset == 0 ? desiredYaw
                    : quantize.apply(desiredYaw + offset * Math.abs(mouseStep));
            if (Float.isFinite(candidate) && !wouldRepeatNext(candidate) && reachable.test(candidate)) {
                return candidate;
            }
        }
        return null;
