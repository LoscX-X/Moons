...
internal fun matchesSearch(module: Module, search: String): Boolean {
    ...
    return query.isEmpty() || module.name().lowercase(Locale.ROOT).contains(query) ||
...
internal fun jsonRange(low: Double, high: Double) = JsonArray().apply { add(low); add(high) }

internal fun snap(value: Double, min: Double, max: Double, step: Double?): Double {
    val snapped = if (step != null && step > 0.0) {
...
internal fun parseColor(value: String): Color = runCatching {
    ...
    when (raw.length) {
        6 -> Color((0xFF000000L or raw.toLong(16)).toInt())
        8 -> Color(raw.toLong(16).toInt())
        else -> PanelStyle.dim
    }
}.getOrDefault(PanelStyle.dim)

internal fun isCompleteColor(value: String): Boolean =
