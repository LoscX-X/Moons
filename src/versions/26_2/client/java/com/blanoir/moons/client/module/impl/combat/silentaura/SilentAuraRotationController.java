...
public final class SilentAuraRotationController {
    ...
    public void track(Minecraft client, LivingEntity target, Vec3 point, double deltaSeconds) {
        ...
        boolean enteringCrossing = !crossingTarget
                && occupiesCrossingCorridor(client, eye, targetBox,
        ...
        if (crossingTarget) {
            ...
            Vec3 yawCentre = lookahead <= 0.0D ? centre
                    : centre.add(targetVelocity.x * lookahead, 0.0D,
                    targetVelocity.z * lookahead);
            double horizontalSquared = Mth.square(yawCentre.x - yawEye.x)
                    + Mth.square(yawCentre.z - yawEye.z);
            ...
            double crossingYawRate = matrixProfile
            ...
            crossingHeadPitch = approach(
                    crossingHeadPitch, crossingPitchTarget, pitchLimit);
            yaw = crossingHeadYaw;
            ...
            yawVelocity = MathUtils.wrappedAngleDifference(previousCrossingYaw, yaw)
                    / (float) crossingDelta;
            pitchVelocity = (pitch - previousCrossingPitch) / (float) crossingDelta;
            pathJitterBlend = approach(pathJitterBlend, 0.0F,
        ...
        pathJitterBlend = approach(pathJitterBlend, pathDemand,
        ...
        Vec3 anchoredPoint = stabilizeAimHeight(
                target, jitteredPoint, deltaSeconds, targetChanged, bodyFloorFraction);
        Vec3 movingPoint = leadAimPoint(
        ...
        double response = aimType.response(SilentAuraConfig.smooth())
                * responseVariation * visibilityUrgency;
        ...
        Vec3 orbitCandidate = aimType.lock() || lowerBodyFallback ? movingPoint : settledOrbit(
                eye, point, movingPoint, target, time,
        ...
        if (lowerBodyFallback) {
            ...
            if (targetChanged || heldOrbitOffset == null
        ...
        double insetZ = lowerBodyFallback ? 0.002D
        ...
        SilentAimType.Correction correction = aimType.correct(
                desiredRotation, yaw, time, targetChanged);
        desiredRotation = correction.rotation();
        desiredRotation = applyAirbornePitchInertia(
                client, target, eye, desiredPoint, desiredRotation, lowerBodyFallback);
        stepToward(desiredRotation, deltaSeconds,
                response * arriveEase * correction.responseScale(),
                aimType.maxYawSpeed(), aimType.maxPitchSpeed(),
                accelNoise * flickYawAccelScale * visibilityUrgency
                        * (1.0D + assist * aimType.yawAssistScale())
                        * yawAccelerationVariation * correction.accelerationScale(),
                accelNoise * flickPitchAccelScale * visibilityUrgency
                        * (1.0D + assist * aimType.pitchAssistScale())
                        * pitchAccelerationVariation * correction.accelerationScale());
    }
    ...
    public void returnToCamera(Minecraft client, double deltaSeconds) {
        ...
        if (time >= nextReturnMotionSample) {
            ...
            nextReturnMotionSample = time
                    + HumanAimSimulator.sampleVariation(0.045D, 0.110D);
        }
    ...
    public boolean returnPacketReached(float sentYaw, float sentPitch) {
        return returning
                && Math.abs(MathUtils.wrappedAngleDifference(sentYaw, returnYaw)) <= RETURN_DONE_ANGLE
    ...
    private void stepToward(Rotation desired, double rawDelta, double response,
        ...
        double responseFraction = 1.0D - Math.exp(-Math.max(0.01D, response)
                * delta);
        double yawCap = maxYawSpeed * delta
                * Mth.clamp(yawStepScale, 0.62D, 1.42D);
        double pitchCap = maxPitchSpeed * delta
                * Mth.clamp(pitchStepScale, 0.62D, 1.42D);
        float yawStep = Math.abs(yawDifference) <= ANGLE_DEADZONE ? 0.0F
    ...
    private Vec3 leadAimPoint(Minecraft client, LivingEntity target, Vec3 point,
        ...
        return MathUtils.closestPoint(lead, new AABB(
                box.minX, upperBodyFloor, box.minZ,
                box.maxX, upperBodyCeiling, box.maxZ));
    }
    ...
    private Vec3 settledOrbit(Vec3 eye, Vec3 basePoint, Vec3 movingPoint,
        ...
        return MathUtils.closestPoint(orbit, new AABB(
                box.minX + insetX, box.minY + insetY, box.minZ + insetZ,
                box.maxX - insetX, box.maxY - insetY, box.maxZ - insetZ));
    }
    ...
    private Rotation applyAirbornePitchInertia(
        ...
        AABB box = target.getBoundingBox();
        double currentMargin = verticalRayMargin(
                client, eye, desired.yaw(), pitch, box);
        if (currentMargin <= 0.0D) return desired;

        double predictionStrength = Mth.clamp(
                SilentAuraConfig.prediction(), 0.0D, 3.0D);
        double horizonTicks = Mth.clamp(
                SilentAuraConfig.predictionLead() * predictionStrength
        ...
        if (horizonTicks > 1.0E-4D) {
            ...
            futureMargin = verticalRayMargin(
                    client, futureEye, futureRotation.yaw(), pitch, futureBox);

            double safety = aimType.pitchCorridorSafetyFraction();
            double futureUrgency = 1.0D - smoothStep(
                    Mth.clamp(futureMargin / Math.max(safety, 1.0E-4D), 0.0D, 1.0D));
            double predictionBlend = futureUrgency
                    * Mth.clamp(predictionStrength / 1.5D, 0.0D, 1.0D);
            trackingPitch = (float) Mth.lerp(
                    predictionBlend, trackingPitch, futureRotation.pitch());
        }

        double safeFraction = aimType.pitchCorridorSafetyFraction();
        double safeDepth = Mth.clamp(
                Math.min(currentMargin, futureMargin)
                        / Math.max(safeFraction, 1.0E-4D),
                0.0D, 1.0D);
    ...
    private static double crossingLookaheadTicks(
        ...
        if (relativeX * relativeX + relativeZ * relativeZ
                < CROSSING_MIN_RELATIVE_SPEED_SQUARED) return 0.0D;
    ...
    private static boolean occupiesCrossingCorridor(
        ...
        boolean horizontalCentreInside = eye.x >= corridor.minX
                && eye.x <= corridor.maxX
                && eye.z >= corridor.minZ
                && eye.z <= corridor.maxZ;
        boolean bodiesOverlapVertically = localBox.maxY >= corridor.minY
                && localBox.minY <= corridor.maxY;
    ...
    private Vec3 stabilizeAimHeight(
        ...
        if (lowerBodyFallback) {
            ...
            return new Vec3(desired.x,
                    Mth.clamp(stickyAimY, minimum, maximum), desired.z);
        }
    ...
    private static double bodyFloorFraction(Vec3 point, AABB box) {
        double height = Math.max(box.getYsize(), 0.1D);
        double fraction = Mth.clamp((point.y - box.minY) / height,
                LOWER_BODY_FLOOR, UPPER_BODY_CEILING);
        if (fraction >= UPPER_BODY_FLOOR) return UPPER_BODY_FLOOR;
        return Mth.clamp(fraction - 0.025D,
                LOWER_BODY_FLOOR, UPPER_BODY_FLOOR);
    }
    ...
    public boolean crossingTarget() { return crossingTarget; }
    public float bodyYaw() { return crossingTarget ? crossingBodyYaw : yaw; }
    public Vec3 lookVector() { return Vec3.directionFromRotation(pitch, yaw); }
    ...
    private void clearAtCamera(Minecraft client) {
        ...
        // Keep the camera on the closest numerically continuous 360-degree
        // equivalent before releasing silent control. Otherwise the first
        // vanilla look packet can jump from 181 to -179 (Grim AimModulo360).
    ...
    private static float approach(float current, float target, float change) {
        return current + Mth.clamp(target - current, -change, change);
    }
    private static float approachWrapped(float current, float target, float change) {
        return current + Mth.clamp(
                MathUtils.wrappedAngleDifference(current, target), -change, change);
    }
    private static double approach(double current, double target, double change) {
        return current + Mth.clamp(target - current, -change, change);
    }
}
