...
public final class DynamicMiniMessage {
    ...
    public static Component parse(String input, long animationTimeMillis) {
        ...
        for (int offset = 0; offset < input.length();) {
            ...
            if (current == '\\' && offset + 1 < input.length()
            ...
            if (current == '<') {
                ...
                if (end >= 0) {
                    ...
                    TagResult result = applyTag(
                            rawTag, glyphs, frames, style, animationTimeMillis);
                    if (result.handled()) {
                        style = result.style();
                        offset = end + 1;
                        continue;
    ...
    private static TagResult applyTag(
        ...
        if (animation != null) {
            frames.push(new Frame(animation.type().tagName(), current,
                    glyphs.size(), null, animation));
    ...
    private static String colorFrameName(String tag) {
        return tag.startsWith("color:") || tag.startsWith("colour:")
                || tag.startsWith("c:") ? "color" : canonicalTag(tag);
    }
    ...
    private static TextColor parseColorTag(String tag) {
        ...
        try {
            ...
            return formatting.ordinal() <= ChatFormatting.WHITE.ordinal()
                    ? TextColor.fromLegacyFormat(formatting) : null;
    ...
    private static void applyAnimation(
        ...
        for (int index = 0; index < count; index++) {
            int color = switch (animation.type()) {
                case RAINBOW -> rainbowColor(
                        seconds * animation.speed() + (double) index / Math.max(6, count));
                case PULSE -> interpolate(
                        new int[]{animation.firstColor(), animation.secondColor()},
                        (Math.sin(seconds * animation.speed() * Math.PI * 2.0D) + 1.0D) * 0.5D);
                case WAVE -> interpolate(
                        new int[]{animation.firstColor(), animation.secondColor()},
                        (Math.sin(seconds * animation.speed() * Math.PI * 2.0D
                                - index * 0.72D) + 1.0D) * 0.5D);
                case SHINE -> {
                    ...
                    yield interpolate(new int[]{animation.firstColor(), animation.secondColor()}, strength);
                }
                case AURORA -> interpolate(
                        new int[]{0x59FFC5, 0x55C8FF, 0xB675FF, 0x59FFC5},
                        positiveWavePosition(seconds * animation.speed() * 0.32D
                                + (double) index / Math.max(4, count)));
                case FIRE -> {
                    double flicker = (Math.sin(seconds * animation.speed() * 10.0D
                            + index * 1.71D)
                            + Math.sin(seconds * animation.speed() * 17.0D
                            - index * 0.83D) * 0.45D + 1.45D) / 2.9D;
                    yield interpolate(new int[]{0xC81D11, 0xFF7200, 0xFFE66D}, flicker);
                }
                case SPARKLE -> {
                    long sparkleTick = (long) Math.floor(
                            seconds * animation.speed() * 12.0D);
                    long hash = (index + 1L) * 0x9E3779B97F4A7C15L
                            ^ sparkleTick * 0xC2B2AE3D27D4EB4FL;
                    double strength = Long.remainderUnsigned(hash, 9L) == 0L
                            ? 1.0D : 0.12D;
                    yield interpolate(new int[]{animation.firstColor(), animation.secondColor()}, strength);
                }
                case CHASE -> {
            ...
            glyphs.set(start + index, new Glyph(
                    glyph.codePoint(), glyph.style().withColor(color)));
        }
    }
    ...
    private static void applyGradient(List<Glyph> glyphs, int start, Gradient gradient) {
        ...
        for (int index = 0; index < count; index++) {
            ...
            int color = interpolate(gradient.colors(), position);
            Glyph glyph = glyphs.get(start + index);
            glyphs.set(start + index, new Glyph(
                    glyph.codePoint(), glyph.style().withColor(color)));
        }
    ...
    private static int interpolate(int[] colors, double position) {
