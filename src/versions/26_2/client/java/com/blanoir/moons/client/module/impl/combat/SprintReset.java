...
public final class SprintReset {
    ...
    private SprintReset() { }
    public static void init() { EventBus.TICK.register("SprintReset.tick", event -> tick(event.client())); }
    ...
    private static void beginReset(Minecraft client) {
        ...
        if (MODE.get() == Mode.LEGIT) CombatInputController.suppressForward(client, CombatInputController.Owner.SPRINT_RESET);
        resetActive = true;
    }
