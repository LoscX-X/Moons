...
public final class PlacementRotationVerification {
    ...
    public static void main(String[] args) {
        ...
        for (float origin : new float[] {0, 180, -180, 100_000_000, -100_000_000}) {
            ...
            require(
                    Float.isFinite(nextYaw) && nextYaw != origin + 16,
                    "settling changes finite yaw");
        ...
        require(
                varied != null && varied < 90,
                "try opposite direction when first variant misses face");
        require(
                history.variedYaw(90, .15F, yaw -> yaw, yaw -> false) == null,
                "never select an unreachable alternative");
        require(
                history.variedYaw(90, .15F, yaw -> 90F, yaw -> true) == null,
                "quantization collapsing all alternatives cannot bypass repetition guard");
        require(
                history.wouldRepeatNext(90),
                "pending placement delta checked before placement is sent");
        history.placementSent();
        history.rotationSent(java.util.Objects.requireNonNull(varied));
        require(
                !history.needsSettling(),
                "preemptive variation allows next placement without waiting");
