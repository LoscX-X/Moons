...
public final class StructureLabelRenderer {
    ...
    private static final TextFeatureRenderer TEXT_RENDERER = new TextFeatureRenderer();
    ...
    public static void render(
        ...
        List<TextFeatureRenderer.Submit> submits = new ArrayList<>();
    ...
    private static void drawNametag(
        ...
        matrices.rotate(new Quaternionf(camera.rotation()).rotateY((float) Math.PI));
        ...
        submits.add(
                new TextFeatureRenderer.Submit(
                        ...
                        new TextFeatureRenderer.Content.Text(
                                x, y, text.getVisualOrderText(), false, color, background, 0)));
        submits.add(
                new TextFeatureRenderer.Submit(
                        pose,
                        ...
                        FULL_BRIGHT_LIGHT,
                        new TextFeatureRenderer.Content.Text(
                                x, y, text.getVisualOrderText(), false, color, 0, 0)));

        matrices.popPose();
    }

    private static void renderNametagBatch(
            Minecraft client, List<TextFeatureRenderer.Submit> submits) {
        if (submits.isEmpty()) {
            return;
        }
        ...
        TEXT_BUFFER.upload();
        com.mojang.blaze3d.systems.RenderSystem.resizeAllAutoStorageIndexBuffers();
        var target = MinecraftClientAccess.mainRenderTarget(client);
        try (var pass =
                com.mojang.blaze3d.systems.RenderSystem.getDevice()
                        .createCommandEncoder()
                        .createRenderPass(
                                () -> "Moons structure labels",
                                target.getColorTextureView(),
                                java.util.Optional.empty(),
                                target.getDepthTextureView(),
                                java.util.OptionalDouble.empty())) {
            TEXT_RENDERER.executeGroup(context, null, pass, 0, submits, false);
        }
