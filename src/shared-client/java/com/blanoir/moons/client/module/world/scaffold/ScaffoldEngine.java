...
public final class ScaffoldEngine {
    ...
    private static final ModeSetting<ScaffoldMode> MODE = new ModeSetting.Builder<ScaffoldMode>()
    ...
    private static final ModeSetting<TellyRotation> TELLY_ROTATION =
            new ModeSetting.Builder<TellyRotation>()
                    .name("scaffold.tellyRotation").defaultValue(TellyRotation.INSTANT)
                    .option(TellyRotation.INSTANT, "instant")
                    .option(TellyRotation.SMOOTH, "smooth").build();
    private static final ModeSetting<TellyDelay> TELLY_DELAY_MODE =
            new ModeSetting.Builder<TellyDelay>()
                    .name("scaffold.tellyDelayMode").defaultValue(TellyDelay.FIXED)
    ...
    private static final ModeSetting<SprintMode> SPRINT_MODE = new ModeSetting.Builder<SprintMode>()
            ...
            .option(SprintMode.NONE, "none").option(SprintMode.VANILLA, "vanilla").build();
    private static final ModeSetting<TowerMode> TOWER = new ModeSetting.Builder<TowerMode>()
            .name("scaffold.tower").defaultValue(TowerMode.VANILLA)
            .option(TowerMode.NONE, "none")
            .option(TowerMode.VANILLA, "vanilla").build();

    private static final DoubleSetting TELLY_START_SPEED = decimal(
            "scaffold.tellyStartSpeed", 85.0D, 1.0D, 180.0D);
    private static final DoubleSetting TELLY_TRACK_SPEED = decimal(
            "scaffold.tellyTrackSpeed", 45.0D, 1.0D, 180.0D);
    private static final DoubleSetting TELLY_PLACE_ANGLE = decimal(
            "scaffold.tellyPlaceAngle", 6.0D, 0.5D, 20.0D);
    ...
    private static final BooleanSetting TELLY_BPS_LIMIT = bool(
            "scaffold.tellyBlocksPerSecondEnabled", true);
    ...
    private static final IntSetting LEGIT_DELAY_MAX = integer(
            "scaffold.legitDelayMax", 3, 0, 10);
    private static final IntSetting TELLY_MAX_FORWARD_BLOCKS = integer(
            "scaffold.tellyMaxForwardBlocks", 3, 1, 6);
    private static final IntSetting TELLY_PLACE_DELAY = integer(
            "scaffold.tellyPlaceDelay", 4, 0, 8);
    private static final IntSetting TELLY_BLOCKS_PER_SECOND_MIN = integer(
            "scaffold.tellyBlocksPerSecondMin", 3, 1, 20);
    private static final IntSetting TELLY_BLOCKS_PER_SECOND_MAX = integer(
            "scaffold.tellyBlocksPerSecondMax", 4, 1, 20);
    ...
    public static float yaw() { return outgoingYaw; }
    ...
    public static ItemStack spoofedItem(ItemStack original) {
        ...
        if (!enabled() || !tellyMode()
                || !ITEM_SPOOF.get() || client.player == null) return original;
    ...
    public static int showStatus(Minecraft client) {
        ClientChat.send(client, "Scaffold: " + (enabled() ? "enabled" : "disabled")
                + " | " + scaffoldMode().configName()
                + " | rotation " + TELLY_ROTATION.get().configName()
                + " | tower " + towerMode().configName()
                + " | " + countBlocks(client) + " blocks.");
        return 1;
    }
    ...
    public static String statusTag() {
        return scaffoldMode() == ScaffoldMode.LEGIT
                ? "legit"
                : String.format(Locale.ROOT, "%s %.1f BPS %s",
                intaveTellyMode() ? "intave-telly" : "telly",
                currentBps(), towerMode().configName());
    }
    ...
    public static int setMoveFix(Minecraft c, String v) { MOVE_FIX.deserialize(v); return 1; }
    public static int setSprintMode(Minecraft c, String v) { SPRINT_MODE.deserialize(v); return 1; }
    public static int setTower(Minecraft c, String v) { TOWER.deserialize(v); return 1; }
    public static List<String> modeOptions() { return MODE.optionIds(); }
    public static List<String> tellyRotationOptions() { return TELLY_ROTATION.optionIds(); }
    public static List<String> tellyDelayModeOptions() { return TELLY_DELAY_MODE.optionIds(); }
    public static List<String> faceSamplingOptions() { return FACE_SAMPLING.optionIds(); }
    public static List<String> moveFixOptions() { return MOVE_FIX.optionIds(); }
    public static List<String> sprintModeOptions() { return SPRINT_MODE.optionIds(); }
    public static List<String> towerOptions() { return TOWER.optionIds(); }
    public static boolean legitSelected() { return scaffoldMode() == ScaffoldMode.LEGIT; }
    public static boolean tellySelected() { return tellyMode(); }
    ...
    public static int setTellyRotation(Minecraft c, String v) { TELLY_ROTATION.deserialize(v); return 1; }
    public static int setTellyDelayMode(Minecraft c, String v) { TELLY_DELAY_MODE.deserialize(v); return 1; }
    public static int setFaceSampling(Minecraft c, String v) { FACE_SAMPLING.deserialize(v); return 1; }
    public static int setTellyPlaceAngle(Minecraft c, double v) { TELLY_PLACE_ANGLE.set(v); return 1; }
    public static int setTellyMaxForwardBlocks(Minecraft c, int v) { TELLY_MAX_FORWARD_BLOCKS.set(v); return 1; }
    public static int setTellyPlaceDelay(Minecraft c, int v) { TELLY_PLACE_DELAY.set(v); return 1; }
    ...
    public static int setBlockCounter(Minecraft c, boolean v) { BLOCK_COUNTER.set(v); return 1; }
    ...
    private static void tickTelly(Minecraft client) {
        ...
        } else if (scaffoldPath == ScaffoldPath.TELLY && tellyTriggered
                && TELLY_FALL_RESCUE.get() && !client.player.onGround()
                && belowTellyTargetRow(client)) {
            enterBelowRowRescue();
        }

        if (scaffoldPath == ScaffoldPath.TELLY
                && client.player.onGround() && tellyWasAirborne) {
            finishTellyLanding();
        }
        ...
        if (scaffoldPath == ScaffoldPath.TELLY) {
            if (!tellyTriggered) {
                ...
                if (client.player.onGround()
                        || !client.level.getBlockState(belowFeet).isAir()) {
                    clearTellyAim();
                    transitionTelly(TellyPhase.SELECT_POINT,
                            "waiting until feet are over air");
                    return;
                }
                tellyTriggered = true;
                if (TELLY_FLAT.get()) {
            ...
            if (tellyBlocksThisJump == 0
                    && tellyAirTicks < effectiveTellyDelay(client, intent)) {
                clearTellyAim();
    ...
    private static void activateAirborneTelly(Minecraft client) {
        if (!tellyMode() || client.player.onGround() || blockCount <= 0
                || tellyBelowRowRescue || verticalTowerRequested(client)
    ...
    private static void attemptTellyPlacement(
        ...
        PlacementAim aim = verticalTower
                ? findVanillaTowerAim(client, planningPosition, planningEye, intent.desired())
        ...
        transitionTelly(TellyPhase.SELECT_POINT,
                intent.rescue() ? "selected rescue" : "selected point");
        ...
        PlacementStep step = verticalTower
        ...
        double remaining = rotationDistance(
    ...
    private static PlacementStep publishPlacementRotation(
            Minecraft client, PlacementStep step) {
        ...
        if (scaffoldPath != ScaffoldPath.TOWER && !intaveTellyMode()) {
            ...
            float jitteredYaw = SilentPacketRotation.quantizePacketYaw(
                    pubYaw, pubYaw + SilentPacketRotation.packetRotationJitter());
            float jitteredPitch = SilentPacketRotation.quantizePacketPitch(
                    pubPitch, pubPitch + SilentPacketRotation.packetRotationJitter());
            if (jitteredYaw != pubYaw || jitteredPitch != pubPitch) {
                BlockHitResult jitterHit = hit == null ? null : BlockPlacementUtils.traceFace(
                        client, client.player.getEyePosition(), jitteredYaw, jitteredPitch,
                        traceRange, step.target().support(), step.target().face());
                if (jitterHit != null) {
                    pubYaw = jitteredYaw;
    ...
    private static PlacementIntent belowFeetRescueIntent(Vec3 playerPosition) {
        BlockPos belowFeet = new BlockPos(
                Mth.floor(playerPosition.x), Mth.floor(playerPosition.y) - 1,
    ...
    private static BlockPos nextTellyPlacementPos(
            Minecraft client, Vec3 playerPosition) {
    ...
    private static int effectiveTellyDelay(
            Minecraft client, PlacementIntent intent) {
    ...
    private static void moveInput(MoveInputEvent event) {
        ...
        boolean tellyTakeoff = tellyMode()
                && client.player.onGround() && moving(client) && blockCount > 0
                && scaffoldPath != ScaffoldPath.TOWER
                && scaffoldPath != ScaffoldPath.RESCUE;
        if (tellyTakeoff) {
            if (scaffoldPath != ScaffoldPath.TELLY) {
        ...
        if (tellyMode() && TELLY_BPS_LIMIT.get()) {
            ...
            boolean unsupportedFall = !client.player.onGround()
                    && client.player.getDeltaMovement().y <= 0.0D
                    && !hasCollisionBelow(client);
            if (inputScale < 0.999D && tellyTakeoffInputGraceTicks <= 0
                    && !unsupportedFall) {
    ...
    private static double tellyForwardSpeedScale(Minecraft client) {
        ...
        if (tellyTargetBlocksPerSecond <= 0) {
            tellyTargetBlocksPerSecond = randomIntInclusive(
    ...
    private static void inspectSilentMovementInput(Minecraft client) {
        ...
        client.player.input.keyPresses = new Input(
                fixed.forward(), fixed.backward(), fixed.left(), fixed.right(),
                fixed.jump(), fixed.shift(), sprint);
    }
    ...
    private static void updateLegitSneaking(Minecraft client) {
        boolean physicalSneak = CombatInputController.isPhysicallyDown(
                client, client.options.keyShift);
        if (physicalSneak) {
    ...
    private static void continueOrEndLegitSneaking(Minecraft client) {
        ...
        if (legitUnsneakStartTick == -1) {
            ...
            legitUnsneakTicks = randomIntInclusive(
                    LEGIT_DELAY_MIN.get(), LEGIT_DELAY_MAX.get());
        }
    ...
    private static void releaseLegitSneaking(Minecraft client) {
        if (legitSneaking && client != null && client.player != null) {
            setInputSneaking(client, CombatInputController.isPhysicallyDown(
                    client, client.options.keyShift));
        }
        resetLegitSneakState();
    }
    ...
    private static double legitEdgeDistance(Minecraft client, AABB predicted) {
        AABB feet = new AABB(
                predicted.minX, predicted.minY - 0.01D, predicted.minZ,
                predicted.maxX, predicted.minY, predicted.maxZ);
    ...
    private static void updateGroundState(Minecraft client) {
        ...
        if (stage > 0) stage--;
        if (stage < 0) stage++;
        if (stage == 0 && keepYMode() != KeepYMode.NONE
                && (!KEEP_Y_ON_PRESS.get()
    ...
    private static boolean place(Minecraft client, PlacementTarget target, Vec3 hitVec) {
        ...
        if (tellyMode()
                && TELLY_FLAT.get() && tellyFlatStarted
                && !tellyBelowRowRescue && !verticalTowerActive(client)
    ...
    private static void hud(HudRenderEvent event) {
        ...
        if (BLOCK_COUNTER.get()) {
            ...
            event.graphics().text(client.font, text, x, y,
    ...
    private static void render(WorldRenderEvent event) {
        if (!enabled() || !RENDER.get() || renderTarget == null) return;
        Minecraft client = Minecraft.getInstance();
        if (!ready(client) || MinecraftClientAccess.isHudHidden(client)) return;
        long now = System.currentTimeMillis();
        PLACED.removeIf(mark -> now - mark.time() > 700L);
        List<WorldOverlayRenderer.ColoredBox> boxes = new ArrayList<>();
        if (SHOW_SHADE.get()) boxes.add(box(renderTarget, 0.28F));
        for (PlacedMark mark : PLACED) {
            float alpha = OUTLINE_FADE.get()
                    ? (float) Mth.clamp(1.0D - (now - mark.time()) / 700.0D, 0.08D, 0.32D)
                    : 0.25F;
            boxes.add(box(mark.pos(), alpha));
        }
        if (boxes.isEmpty()) boxes.add(box(renderTarget, 0.18F));
    ...
    private static boolean validBlock(Minecraft client, ItemStack stack) {
        if (client == null || client.level == null || stack.isEmpty()
    ...
    private static List<PlacementTarget> collectPlacementTargets(
        ...
        for (int x = -SEARCH_RADIUS; x <= SEARCH_RADIUS; x++) {
            for (int y = -SEARCH_RADIUS; y <= 0; y++) {
                for (int z = -SEARCH_RADIUS; z <= SEARCH_RADIUS; z++) {
                    ...
                    if (state.canBeReplaced() || isInteractable(state)
                            || playerPosition.distanceToSqr(
                            Vec3.atCenterOf(support)) > reachSqr
                    ...
                    for (Direction face : Direction.values()) {
                        ...
                        if (placed.getY() > desired.getY() || !replaceable(client, placed)) continue;
        ...
        targets.sort(Comparator
                .comparingDouble((PlacementTarget target) -> Vec3.atCenterOf(target.placePos())
                        .distanceToSqr(targetCenter))
                .thenComparingDouble(target -> Vec3.atCenterOf(target.support())
                        .distanceToSqr(targetCenter))
    ...
    private static PlacementAim findPlacementAim(
        ...
        for (PlacementTarget target : collectPlacementTargets(client, playerPosition, desired)) {
            double placeDistance = Vec3.atCenterOf(target.placePos())
            ...
            double continuity = lastTellyPlacePos != null
                    && target.support().equals(lastTellyPlacePos) ? -2.0D : 0.0D;
            double score = rotationDistance(
                    candidate.rotation(), baseYaw, basePitch) + continuity;
            if (score < bestScore) {
                best = candidate;
                bestScore = score;
            ...
            if (placeDistance == 0.0D && rotationDistance(
                    candidate.rotation(), baseYaw, basePitch) <= 2.0D) break;
        }
        return best;
    }
    ...
    private static PlacementAim findVanillaTowerAim(
        ...
        targets.sort(Comparator
                .comparingDouble((PlacementTarget target) -> Vec3.atCenterOf(target.support())
                        .distanceToSqr(desiredCenter))
        ...
        for (PlacementTarget target : targets) {
            BlockHitResult hit = BlockPlacementUtils.traceFace(
                    ...
                    client.player.blockInteractionRange(), target.support(), target.face());
            if (hit != null) {
                return new PlacementAim(target,
                        new Rotation(towerRotationYaw, 90.0F), hit);
            }
        }
        return null;
    }
    ...
    private static PlacementAim findFaceAim(
            Minecraft client, PlacementTarget target, Vec3 eye) {
        float baseYaw = placementBaseYaw(client);
        float basePitch = placementBasePitch(client);
        return switch (FACE_SAMPLING.get()) {
            ...
            case DENSE -> scanFaceAim(
                    client, target, eye, baseYaw, basePitch, DENSE_FACE_OFFSETS);
            case ADAPTIVE -> {
                PlacementAim primary = scanFaceAim(
                        client, target, eye, baseYaw, basePitch, PRIMARY_FACE_OFFSETS);
                yield primary != null ? primary : scanFaceAim(
                        client, target, eye, baseYaw, basePitch, DENSE_FACE_OFFSETS);
            }
        };
    }
    ...
    private static PlacementAim scanFaceAim(
        ...
        for (double u : offsets) {
            for (double v : offsets) {
                Vec3 point = BlockPlacementUtils.facePoint(
                ...
                float yaw = SilentPacketRotation.quantizePacketYaw(
                        baseYaw, baseYaw + Mth.wrapDegrees(rawYaw - baseYaw));
                float pitch = SilentPacketRotation.quantizePacketPitch(basePitch, rawPitch);

                BlockHitResult hit = BlockPlacementUtils.traceFace(
                        client, eye, yaw, pitch, range,
                        target.support(), target.face());
                if (hit == null) continue;

                double angleScore = Math.abs(Mth.wrapDegrees(yaw - baseYaw))
                        + Math.abs(pitch - basePitch);
                double centerScore = (u - 0.5D) * (u - 0.5D)
                        + (v - 0.5D) * (v - 0.5D);
    ...
    private static double rotationDistance(
            Rotation rotation, float baseYaw, float basePitch) {
    ...
    private static PlacementStep resolvePlacementStep(
        ...
        BlockHitResult hit = BlockPlacementUtils.traceFace(
                client, eye, limitedYaw, limitedPitch,
                client.player.blockInteractionRange(),
                exact.target().support(), exact.target().face());
        return new PlacementStep(exact.target(),
                new Rotation(limitedYaw, limitedPitch), hit);
    }
    ...
    private static boolean isInteractable(BlockState state) {
        ...
        if (path.equals("crafting_table") || path.contains("anvil")
                || path.endsWith("_bed") || path.endsWith("_trapdoor")
                || path.endsWith("_fence_gate") || path.endsWith("_fence")
                || path.endsWith("_button") || path.equals("lever")
    ...
    private static boolean isSolid(Minecraft client, BlockState state, BlockPos pos) {
        ...
        String path = BuiltInRegistries.BLOCK.getKey(state.getBlock()).getPath();
        if (path.contains("stairs") || path.endsWith("_slab")
                || path.equals("end_portal_frame") || path.equals("end_portal")
                || path.equals("vine") || path.contains("pumpkin")
                || path.equals("jack_o_lantern") || path.equals("cactus")
                || path.equals("cobweb") || path.endsWith("_pane")
                || path.endsWith("_carpet") || path.equals("snow")
                || path.endsWith("_fence") || path.endsWith("_fence_gate")
                || path.endsWith("_wall") || path.equals("ladder")
                || path.contains("torch") || path.equals("redstone_wire")
                || path.contains("repeater") || path.contains("comparator")
                || path.contains("pressure_plate") || path.equals("tripwire")
                || path.equals("tripwire_hook") || path.contains("rail")
                || path.equals("slime_block") || path.equals("tnt")) return false;
    ...
    private static MoveFix moveFix() { return MOVE_FIX.get(); }
    private static SprintMode sprintMode() { return SPRINT_MODE.get(); }
    ...
    private static WorldOverlayRenderer.ColoredBox box(BlockPos pos, float alpha) {
        return new WorldOverlayRenderer.ColoredBox(pos.getX(), pos.getY(), pos.getZ(),
                pos.getX() + 1, pos.getY() + 1, pos.getZ() + 1,
                0.1F, 0.7F, 1.0F, alpha);
    }
    ...
    private record PlacementTarget(BlockPos support, Direction face) {
        BlockPos placePos() { return support.relative(face); }
    }
    private record PlacementIntent(BlockPos desired, boolean rescue) { }
    private record PlacementAim(
            PlacementTarget target, Rotation rotation, BlockHitResult hit) { }
    ...
    private enum TellyPhase {
        ...
        TellyPhase(int color) { this.color = color; }
        int color() { return color; }
        String configName() { return name().toLowerCase(Locale.ROOT); }
    }

    private enum ScaffoldMode {
        LEGIT, TELLY, INTAVE_TELLY;
        String configName() { return name().toLowerCase(Locale.ROOT); }
    }
    ...
    public static int setDebugger(Minecraft c, boolean v) { DEBUGGER.set(v); return 1; }

    private static void drawTellyDebugger(
            HudRenderEvent event, Minecraft client) {
        String[] lines = {
                "Scaffold debugger",
                "path=" + scaffoldPath.configName()
                        + " phase=" + tellyPhase.configName(),
                "reason=" + tellyDebugReason,
                "point=" + debugPos(renderTarget),
                "triggered=" + tellyTriggered
                        + " rescue=" + tellyBelowRowRescue
                        + " ground=" + client.player.onGround(),
                String.format(Locale.ROOT,
                        "air=%d placed=%d vy=%.3f",
                        tellyAirTicks, tellyBlocksThisJump,
        ...
        for (int index = 0; index < lines.length; index++) {
            ...
            event.graphics().text(client.font, lines[index], x,
                    y + index * (client.font.lineHeight + 1), color, true);
        }
    }
