...
object ClickGuiWarmup {
    ...
    fun start() {
        ...
        worker = Thread.ofPlatform().daemon().name("moons-clickgui-warmup").unstarted {
            try {
    ...
    private fun renderPreview() {
        ...
        val modules = ModuleCategories.ordered().flatMap { category ->
            List(16) { index ->
                ModuleRegistry.Module("warmup-$category-$index", "Module ${index + 1}", category,
                    { index % 3 == 0 }, { _, _ -> 1 }, { "" }, emptyList())
            }
        }
        if (cancelled) return
        ...
        System.out.println("[client] ClickGUI CPU warmup complete in ${(System.nanoTime() - start) / 1_000_000} ms")
    }
}
