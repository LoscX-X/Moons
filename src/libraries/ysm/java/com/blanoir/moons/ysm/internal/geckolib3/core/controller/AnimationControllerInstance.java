...
import com.blanoir.moons.ysm.internal.YesSteveModel;
import com.blanoir.moons.ysm.internal.core.config.ConfigPolicies;
...
import com.blanoir.moons.ysm.internal.geckolib3.core.enums.AnimationState;
import com.blanoir.moons.ysm.internal.geckolib3.core.event.InstructionKeyFrameExecutor;
import com.blanoir.moons.ysm.internal.geckolib3.core.event.SoundKeyFrameExecutor;
import com.blanoir.moons.ysm.internal.geckolib3.core.keyframe.*;
import com.blanoir.moons.ysm.internal.geckolib3.core.keyframe.bone.BoneKeyFrame;
import com.blanoir.moons.ysm.internal.geckolib3.core.keyframe.bone.TransitionKeyFrame;
import com.blanoir.moons.ysm.internal.geckolib3.core.molang.context.AnimationContext;
import com.blanoir.moons.ysm.internal.geckolib3.core.snapshot.BoneSnapshot;
import com.blanoir.moons.ysm.internal.geckolib3.core.snapshot.BoneTopLevelSnapshot;
import com.blanoir.moons.ysm.internal.geckolib3.util.IInterpolable;
import com.blanoir.moons.ysm.internal.geckolib3.util.InterpolationLookup;
import com.blanoir.moons.ysm.internal.geckolib3.util.TicksInterpolator;
import com.blanoir.moons.ysm.internal.molang.runtime.ExpressionEvaluator;
import com.blanoir.moons.ysm.internal.runtime.Pair;
...
public class AnimationControllerInstance {
    ...
    @Nullable private Float animationTickOverride;

    public AnimationControllerInstance(
            AnimatableEntity<?> animatable, float transitionLengthTicks) {
    ...
    public AnimationControllerInstance(
            ...
            boolean isScaleTransitionSpecial) {
    ...
    public void initBoneQueues(List<BoneTopLevelSnapshot> list) {
        ...
        for (BoneTopLevelSnapshot boneTopLevelSnapshot : list) {
            this.boneAnimationQueues.put(
                    boneTopLevelSnapshot.boneId, new BoneAnimationQueue(boneTopLevelSnapshot));
        }
    }
    ...
    public void setAnimation(@Nullable String animationName, @Nullable ILoopType loopType) {
        ...
        if (this.lastRequestedAnimation != null
                && this.lastRequestedAnimation.getSecond().equals(animationName)
                && this.lastRequestedAnimation.getFirst() == loopType) {
            return;
        }
        ...
        if (animation == null) {
            if (this.reportedMissingAnimations.add(animationName)) {
                YesSteveModel.LOGGER.warn(
                        "[SM-ANIM] ignored missing animation={}, keeping the current animation",
                        animationName);
            }
            return;
        }
    ...
    public void process(float tick, ExpressionEvaluator<AnimationContext<?>> evaluator, boolean z) {
        ...
        if (this.animationState == AnimationState.RUNNING
                && this.currentAnimationLoop == ILoopType.EDefaultLoopTypes.PLAY_ONCE
        ...
        if (this.animationState == AnimationState.RUNNING) {
            ...
            float runningTick =
                    useSampleTickOverride ? Math.max(sampleTickOverride, 0.0f) : adjustedTick;
            if (runningTick > this.currentAnimation.animationLength) {
                ...
                } else if (this.currentAnimationLoop
    ...
    private void executeRemainingEvents(
            ExpressionEvaluator<AnimationContext<?>> animationTick, boolean z) {
    ...
    private void executeTimelineEvents(
            ExpressionEvaluator<AnimationContext<?>> evaluator,
            float currentTick,
            boolean isActive) {
        if (this.soundExecutor != null) {
    ...
    private void startEndingTransition(float tick) {
        if (this.animationState == AnimationState.RUNNING
            ...
            for (BoneAnimationQueue animationQueue : this.activeBoneAnimationQueues) {
                if (animationQueue.rotationQueue != null
                        && animationQueue.rotationQueue.cachedValue != null) {
                    animationQueue.rotationOutput =
                            new Vector3f(animationQueue.rotationQueue.cachedValue);
                }
                if (animationQueue.positionQueue != null
                        && animationQueue.positionQueue.cachedValue != null) {
                    animationQueue.positionOutput =
                            new Vector3f(animationQueue.positionQueue.cachedValue);
                }
                if (animationQueue.scaleQueue != null
                        && animationQueue.scaleQueue.cachedValue != null) {
                    animationQueue.scaleOutput =
                            new Vector3f(animationQueue.scaleQueue.cachedValue);
                }
            }
            this.tickOffset = tick;
            if (this.animationState == AnimationState.RUNNING) {
    ...
    private void processBeginningTransition(
            ExpressionEvaluator<AnimationContext<?>> evaluator, float tick) {
        TransitionPoint transitionPoint;
        float blendWeight =
                this.currentAnimation.blendWeight != null
                        ? this.currentAnimation.blendWeight.evalAsFloat(evaluator)
                        : 1.0f;
        ...
        for (BoneAnimationQueue boneAnimationQueue : this.activeBoneAnimationQueues) {
            ...
            if (boneAnimationQueue.rotationTimeline != null) {
                boneAnimationQueue.rotationQueue =
                        getTransitionPointAtTick(
                                boneAnimationQueue.rotationTimeline,
                                tick,
                                lerpFactor,
            ...
            if (boneAnimationQueue.positionTimeline != null) {
                boneAnimationQueue.positionQueue =
                        getTransitionPointAtTick(
                                boneAnimationQueue.positionTimeline,
                                tick,
                                lerpFactor,
            ...
            if (boneAnimationQueue.scaleTimeline != null) {
                if (this.isScaleTransitionSpecial) {
                    transitionPoint =
                            getTransitionPointAtTick(
                                    boneAnimationQueue.scaleTimeline,
                                    this.transitionInterpolator.getProgress(),
                                    1.0f,
                                    boneSnapshot.scale);
                } else {
                    transitionPoint =
    ...
    private void processRunningAnimation(
            ExpressionEvaluator<AnimationContext<?>> evaluator, float tick) {
        float blendWeight =
                this.currentAnimation.blendWeight != null
                        ? this.currentAnimation.blendWeight.evalAsFloat(evaluator)
        ...
        for (BoneAnimationQueue boneAnimationQueue : this.activeBoneAnimationQueues) {
            ...
            if (boneAnimationQueue.rotationTimeline != null) {
                boneAnimationQueue.rotationQueue =
                        getKeyFramePointAtTick(boneAnimationQueue.rotationTimeline, tick);
            }
            if (boneAnimationQueue.positionTimeline != null) {
                boneAnimationQueue.positionQueue =
                        getKeyFramePointAtTick(boneAnimationQueue.positionTimeline, tick);
            }
            if (boneAnimationQueue.scaleTimeline != null) {
                boneAnimationQueue.scaleQueue =
                        getKeyFramePointAtTick(boneAnimationQueue.scaleTimeline, tick);
            }
        }
    }

    private void processEndingTransition(
            ExpressionEvaluator<AnimationContext<?>> evaluator, float f) {
        float fMo1906xaffeef43 =
                this.currentAnimation.blendWeight != null
                        ? this.currentAnimation.blendWeight.evalAsFloat(evaluator)
                        : 1.0f;
        for (BoneAnimationQueue boneAnimationQueue : this.activeBoneAnimationQueues) {
            ...
            if (boneAnimationQueue.rotationOutput != null) {
                boneAnimationQueue.rotationQueue =
                        getConstantPointAtTick(
                                f,
                                boneAnimationQueue.rotationOutput,
                                boneAnimationQueue.overrideMode);
            }
            if (boneAnimationQueue.positionOutput != null) {
                boneAnimationQueue.positionQueue =
                        getConstantPointAtTick(
                                f,
                                boneAnimationQueue.positionOutput,
                                boneAnimationQueue.overrideMode);
            }
            if (boneAnimationQueue.scaleOutput != null) {
                boneAnimationQueue.scaleQueue =
                        getConstantPointAtTick(
                                f, boneAnimationQueue.scaleOutput, boneAnimationQueue.overrideMode);
            }
        }
    }
     ...
     * 当前关键帧播放进度
     **/
    private KeyFramePoint getKeyFramePointAtTick(
            InterpolationLookup<BoneKeyFrame> frames, float tick) {
     ...
     * 返过渡进度
     **/
    private TransitionPoint getTransitionPointAtTick(
            InterpolationLookup<BoneKeyFrame> frames,
            float tick,
            float lerpFactor,
            Vector3f offsetPoint) {
        return new TransitionPoint(
                tick,
                lerpFactor,
                this.transitionInterpolator.getProgress(),
                offsetPoint,
                (TransitionKeyFrame) frames.getAtTime(0.0f),
                this.context);
    }
     ...
     * 静态动画点
     */
    private ConstantPoint getConstantPointAtTick(
            float tick, Vector3f offsetPoint, boolean isInstant) {
        return new ConstantPoint(
                tick, isInstant ? 0.0f : defaultTransitionTick, offsetPoint, this.context);
    }
    ...
    private boolean applyPendingAnimation() {
        ...
        if (missingTargets > 0 && ConfigPolicies.diagnostics().animationDebugLog()) {
            YesSteveModel.LOGGER.warn(
                    "[SM-ANIM] animation={} source={} boundChannels={} missingChannels={}",
        ...
        this.instructionExecutor =
                new InstructionKeyFrameExecutor(this.currentAnimation.customInstructionKeyframes);
        this.soundExecutor =
                new SoundKeyFrameExecutor(
                        this.currentAnimation.soundKeyFrames, this.context.getAudioPlayerManager());
        return true;
    }
    ...
    private void reportMissingAnimationTarget(Animation animation, BoneAnimation boneAnimation) {
        ...
        String key =
                animation.sourceKey + "|" + animation.animationName + "|" + boneAnimation.boneName;
        if (this.reportedMissingAnimationTargets.add(key)) {
            YesSteveModel.LOGGER.warn(
                    "[SM-ANIM] missing target bone animation={} source={} bone={} boneId={}",
                    animation.animationName,
                    animation.sourceKey,
                    boneAnimation.boneName,
                    boneAnimation.boneId);
        }
    }
