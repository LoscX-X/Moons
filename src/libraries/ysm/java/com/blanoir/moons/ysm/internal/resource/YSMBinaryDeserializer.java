...
public class YSMBinaryDeserializer implements AutoCloseable {
    ...
    private void deserializeLegacyV1() {
        ...
        for (int i = 0; i < animationBlobCount; ++i) {
            ...
            if (animationId == 5) {
                RawYsmModel.RawSubEntity arrowEntity =
                        model.projectiles.computeIfAbsent(
                                "minecraft:arrow",
                                k -> {
                                    RawYsmModel.RawSubEntity newSub =
                                            new RawYsmModel.RawSubEntity();
                                    newSub.identifier = k;
                                    return newSub;
                                });
        ...
        for (int i = 0; i < customTextureCount; ++i) {
            ...
            else model.mainEntity.textures.put(tex.name, tex);
    ...
    private void deserializeLegacyV15() {
        ...
        if (format > 9) {
            ...
            for (int i = 0; i < animationControllerTableSize; ++i) {
                ...
                for (RawYsmModel.RawAnimationControllerFile file :
                        model.mainEntity.animationControllerFiles) {
    ...
    private void deserializeModern() {
        ...
        for (int i = 0; i < animationCount; ++i) {
            ...
            model.mainEntity.animationFiles.put(
                    YSMFolderDeserializer.getAnimKeyFromType(type), animRef);
    ...
    private void parseSubEntity(
            Map<String, RawYsmModel.RawSubEntity> targetMap, String categoryName, int index) {
        ...
        if (format > 26) {
            ...
            if (identifierCount <= 0 || identifierCount > 128) {
                throw new RuntimeException(
                        "Invalid SubEntity identifier count: " + identifierCount);
            }
    ...
    private void parseAnimationControllers(
            List<RawYsmModel.RawAnimationControllerFile> targetList, boolean readName) {
        int controllerCount = reader.readVarInt();
        for (int i = 0; i < controllerCount; i++) {
            RawYsmModel.RawAnimationControllerFile file =
    ...
    private void parseAnimationControllerBody(
