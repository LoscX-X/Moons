...
import com.blanoir.moons.client.module.impl.player.nofall.AutoMlgNoFall;
import com.blanoir.moons.client.chat.ClientChat;
import com.blanoir.moons.client.utils.client.ClientReady;
import net.minecraft.client.Minecraft;

public final class NoFall {
    private static final AutoMlgNoFall AUTO_MLG = new AutoMlgNoFall();
    private static final BooleanSetting ENABLED = new BooleanSetting.Builder()
            .name("nofall.enabled").defaultValue(false).build();
    private static final DoubleSetting THRESHOLD = new DoubleSetting.Builder()
            .name("nofall.threshold").defaultValue(3.0D).range(1.0D, 10.0D).build();
    private static final IntSetting PREDICT_TICKS = new IntSetting.Builder()
            .name("nofall.predictTicks").defaultValue(2).range(1, 5).build();
    private static final BooleanSetting SOLID_CHECK = new BooleanSetting.Builder()
            .name("nofall.solidCheck").defaultValue(true).build();
    private static final BooleanSetting RECOVERY = new BooleanSetting.Builder()
            .name("nofall.recovery").defaultValue(true).build();
    ...
    public static void init() {
        EventBus.PLAYER_UPDATE.register("NoFall.playerUpdate", event -> {
            ...
            AUTO_MLG.tick(client, THRESHOLD.get(), PREDICT_TICKS.get(),
                    SOLID_CHECK.get(), RECOVERY.get());
        });
    }

    public static int showStatus(Minecraft client) {
        ClientChat.send(client, "AutoMLG: " + statusText()
                + ", fall distance: " + THRESHOLD.get()
                + ", predict ticks: " + PREDICT_TICKS.get()
                + ", solid check: " + SOLID_CHECK.get()
    ...
    public static String statusTag() { return ""; }
    private static String statusText() { return ENABLED.get() ? "enabled" : "disabled"; }
