...
public class Interpolations {
     ...
     * Yaw 的 Hermite 三次插值
     */
    public static double cubicHermiteYaw(
            double y0, double y1, double y2, double y3, double position) {
        y0 = MathHelper.wrapDegrees(y0);
        y1 = MathHelper.wrapDegrees(y1);
        y2 = MathHelper.wrapDegrees(y2);
        y3 = MathHelper.wrapDegrees(y3);
    ...
    public static double envelope(
            double x, double lowIn, double lowOut, double highIn, double highOut) {
        if (x < lowIn || x > highOut) {
            return 0;
        }
        if (x < lowOut) {
