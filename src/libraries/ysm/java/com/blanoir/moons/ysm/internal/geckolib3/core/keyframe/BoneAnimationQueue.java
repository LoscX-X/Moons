package com.blanoir.moons.ysm.internal.geckolib3.core.keyframe;
...
public class BoneAnimationQueue {
    ...
    @Nullable public InterpolationLookup<BoneKeyFrame> rotationTimeline;

    @Nullable public InterpolationLookup<BoneKeyFrame> positionTimeline;

    @Nullable public InterpolationLookup<BoneKeyFrame> scaleTimeline;
    ...
    public void applyAnimation(BoneAnimation animation, boolean z) {
        if (!animation.rotationKeyFrames.isEmpty()) {
            this.rotationTimeline =
                    new InterpolationLookup<>(
                            animation.rotationKeyFrames, 0.0f, BoneKeyFrame::getEndTick);
        } else {
        ...
        if (!animation.positionKeyFrames.isEmpty()) {
            this.positionTimeline =
                    new InterpolationLookup<>(
                            animation.positionKeyFrames, 0.0f, BoneKeyFrame::getEndTick);
        } else {
        ...
        if (!animation.scaleKeyFrames.isEmpty()) {
            this.scaleTimeline =
                    new InterpolationLookup<>(
                            animation.scaleKeyFrames, 0.0f, BoneKeyFrame::getEndTick);
        } else {
