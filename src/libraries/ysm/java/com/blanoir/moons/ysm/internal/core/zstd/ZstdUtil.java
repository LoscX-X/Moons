...
public final class ZstdUtil {
    ...
    public static byte[] compress(byte[] input, int inputOffset, int inputLength, int level) {
        ...
        output +=
                ZstdFrameCompressor.writeFrameHeader(
                        buffer, output, outputLimit, inputLength, parameters.getWindowSize());
        output +=
                ZstdFrameCompressor.compressFrame(
                        input, inputAddress, inputLimit, buffer, output, outputLimit, parameters);
        output +=
                ZstdFrameCompressor.writeChecksum(
                        buffer, output, outputLimit, input, inputAddress, inputLimit);
    ...
    public static byte[] decompress(byte[] input, int offset, int length) {
        ...
        try (ZstdInputStream in =
                new ZstdInputStream(new ByteArrayInputStream(input, offset, length))) {
            TrimmableByteArrayOutputStream baos =
                    new TrimmableByteArrayOutputStream(Math.max(64 * 1024, length * 2));
