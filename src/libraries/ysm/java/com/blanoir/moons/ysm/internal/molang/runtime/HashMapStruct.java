...
public class HashMapStruct implements Struct {
    ...
    public void forEach(java.util.function.BiConsumer<String, Object> consumer) {
        for (var entry : properties.int2ReferenceEntrySet())
            consumer.accept(StringPool.getString(entry.getIntKey()), entry.getValue());
    }
    ...
    public String toString() {
        ...
        for (Int2ReferenceMap.Entry<Object> entry : properties.int2ReferenceEntrySet()) {
            ...
            builder.append(
                    String.format(
                            "%s=%s",
                            StringPool.getString(entry.getIntKey()),
