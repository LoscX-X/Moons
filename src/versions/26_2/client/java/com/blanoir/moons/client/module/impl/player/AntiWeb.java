...
public final class AntiWeb {
    ...
    private static BlockHitResult waterSourceHit(BlockPos waterPos) {
        ...
        Minecraft client = Minecraft.getInstance();
        Vec3 nearest = client != null && client.player != null
    ...
    private static Vec3 closestCellPointToSentRay(
            Minecraft client,
            BlockPos cell
    ) {
        Vec3 eye = client.player.getEyePosition();
        ...
        return new Vec3(
                Mth.clamp(point.x,
                        cell.getX() + BOX_EPSILON,
                        cell.getX() + 1.0D - BOX_EPSILON),
                Mth.clamp(point.y,
                        cell.getY() + BOX_EPSILON,
                        cell.getY() + 1.0D - BOX_EPSILON),
                Mth.clamp(point.z,
                        cell.getZ() + BOX_EPSILON,
                        cell.getZ() + 1.0D - BOX_EPSILON));
    }
    ...
    private static WaterPlacementPlan findPlayerWaterPlan(Minecraft client) {
        ...
        BlockPos trappedWeb = findCobwebInBox(
                client,
    ...
    private static WaterPlacementPlan planWaterPlacement(
        ...
        if (eye.y <= topPoint.y + BOX_EPSILON
                || !withinInteractionRange(client, topPoint)) {
            return null;
        }
        ...
        BlockHitResult placementHit = new BlockHitResult(
                visibleHit.getLocation(), Direction.UP, webPos, false);
        return new WaterPlacementPlan(
                webPos.immutable(), waterPos.immutable(), placementHit, 0.0D);
    }

    /** Top-face point with the smallest angular change from the real camera. */
    private static Vec3 closestTopFacePointToCameraRay(
            Minecraft client,
            BlockPos webPos
    ) {
        Vec3 eye = client.player.getEyePosition();
        ...
        double planeIntersection = Math.abs(look.y) > 1.0E-6D
                ? (topY - eye.y) / look.y : -1.0D;
        double distanceAlongRay = planeIntersection >= 0.0D
                ? planeIntersection
                : Math.max(0.0D, center.subtract(eye).dot(look));
        Vec3 point = eye.add(look.scale(distanceAlongRay));
        return new Vec3(
                Mth.clamp(point.x,
                        webPos.getX() + BOX_EPSILON,
                        webPos.getX() + 1.0D - BOX_EPSILON),
                webPos.getY() + 1.0D,
                Mth.clamp(point.z,
                        webPos.getZ() + BOX_EPSILON,
                        webPos.getZ() + 1.0D - BOX_EPSILON));
    }
