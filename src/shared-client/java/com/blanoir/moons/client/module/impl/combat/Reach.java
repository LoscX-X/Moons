...
public final class Reach {
    ...
    public static void init() {
        ...
        initialized = true;
        EventBus.TICK.register("Reach.sampleExpansionChance", event -> {
    ...
    public static boolean handleManualAttack(Minecraft client) {
        ...
        boolean vanillaHit = client.hitResult instanceof EntityHitResult hit
    ...
    public static void applyNormalPick(Minecraft client) {
        ...
        EntityHitResult extendedHit = RaytraceUtils.findEntity(
                client,
                start,
    ...
    public static void applyAdvancedRender(
            Entity entity,
            EntityRenderState state
    ) {
        Vec3 renderPosition = TARGET_LAG.renderPosition();
        if (!ENABLED.get() || !advancedMode() || renderPosition == null
    ...
    private static boolean withinPendingRange(Minecraft client) {
        ...
        return range > 0.0D
                ...
                client.player.getEyePosition(), laggedBox) <= range * range;
    }
    ...
    public static String statusTag() {
        ...
        if (distanceDisplayTicks > 0
            ...
            return "real:" + formatDistance(displayedRealDistance)
                    + " current:" + formatDistance(displayedCurrentDistance);
        }
        return "Advanced";
    }
    ...
    public static int showStatus(Minecraft client) {
        ClientChat.send(client, "Reach: " + (ENABLED.get() ? "enabled" : "disabled")
