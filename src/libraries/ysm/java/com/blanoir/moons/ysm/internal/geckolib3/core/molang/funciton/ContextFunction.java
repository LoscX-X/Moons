package com.blanoir.moons.ysm.internal.geckolib3.core.molang.funciton;
...
public abstract class ContextFunction<TEntity> implements Function {
    ...
    public final Object evaluate(
            @NotNull ExecutionContext<?> context, @NotNull ArgumentCollection arguments) {
    ...
    public final float evaluateFloat(
            @NotNull ExpressionEvaluator<?> context, @NotNull ArgumentCollection arguments) {
        Object entity = context.entity();
        if (entity instanceof IContext && validateContext((IContext<?>) entity)) {
    ...
    protected abstract Object eval(
            ExecutionContext<IContext<TEntity>> context, ArgumentCollection arguments);

    /**
     * 原始 float 求值钩子，默认回退到 {@link #eval}。返回数值的子类应重写此方法直接返回 float。
     */
    protected float evalFloat(
            ExpressionEvaluator<IContext<TEntity>> context, ArgumentCollection arguments) {
