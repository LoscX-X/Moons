...
import static com.blanoir.moons.ysm.internal.core.zstd.Constants.MAX_OFFSET_CODE_SYMBOL;
...
import static com.blanoir.moons.ysm.internal.core.zstd.FiniteStateEntropy.optimalTableLog;
...
final class SequenceEncoder {
    ...
    public static int compressSequences(
        ...
        // We can only use the basic table if max <= DEFAULT_MAX_OFFSET_CODE_SYMBOL, otherwise the
    ...
    private static int buildCompressionTable(
        ...
        // this is a minor optimization. The last symbol is embedded in the initial FSE state, so
        // it's not part of the bitstream. We can omit it from the
        // statistics (but only if its count is > 1). This makes the statistics a tiny bit more
        // accurate.
    ...
    private static int encodeSequences(
        ...
        if (sequenceCount >= 2) {
            for (int n = sequenceCount - 2; n >= 0; n--) {
                ...
                matchLengthState =
                        matchLengthTable.encode(
                                blockStream, matchLengthState, matchLengthCode); // 24
                literalLengthState =
                        literalLengthTable.encode(
                                blockStream, literalLengthState, literalLengthCode); // 33
    ...
    private static int selectEncodingType(
        ...
        if (strategy.ordinal()
                < CompressionParameters.Strategy.LAZY
                        .ordinal()) { // TODO: more robust check. Maybe encapsulate in strategy
            // objects
