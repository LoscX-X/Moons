...
public final class AntiLava {
    ...
    public static void onBlockUpdate(BlockPos pos, BlockState state) {
        if (!ENABLED.get() || pos == null || state == null
    ...
    private static void tickPlacement(Minecraft client) {
        ...
        if (placementPhase == PlacementPhase.WAITING_FOR_RETURN_ROTATION) {
            ...
            float yawDifference = Math.abs(Mth.wrapDegrees(
                    client.player.getYRot() - SilentPacketRotation.getSentYaw()));
            float pitchDifference = Math.abs(
        ...
        if (currentHit != null
                && canPlace(client, activeHand, activeHotbarSlot, currentHit)) {
            InteractionResult result = useOnSilently(
                    client, activeHand, currentHit);
    ...
    private static PlacementMaterial findPlacementMaterial(Minecraft client) {
        ...
        return bestSlot == -1
                ? null : new PlacementMaterial(InteractionHand.MAIN_HAND, bestSlot);
    }
    ...
    private static boolean canPlace(
        ...
        ItemStack stack = hand == InteractionHand.OFF_HAND
                ...
                : hotbarSlot >= 0 ? client.player.getInventory().getItem(hotbarSlot)
                : ItemStack.EMPTY;
        ...
        for (Direction face : SUPPORT_FACES) {
            ...
            for (double u : FACE_SAMPLES) {
                for (double v : FACE_SAMPLES) {
                    Vec3 requested = BlockPlacementUtils.facePoint(
                            client, supportPos, face, u, v);
                    if (eye.distanceToSqr(requested) > range * range) continue;
                    Rotation rotation = RotationUtils.rotationTo(eye, requested);
                    BlockHitResult traced = BlockPlacementUtils.traceFace(
                            client, eye, rotation.yaw(), rotation.pitch(),
                            range, supportPos, face);
                    if (traced == null) continue;
                    double score = eye.distanceToSqr(traced.getLocation())
    ...
    private static boolean isUnderPlayerFeet(Minecraft client, BlockPos source) {
        ...
        boolean overlapsFootprint = playerBox.maxX > source.getX() + 1.0E-4D
    ...
    private static boolean withinFov(Minecraft client, Vec3 point) {
        return MathUtils.withinFov(MathUtils.viewAngle(client.player.getEyePosition(),
                client.player.getLookAngle(), point), FOV.get());
    }
    ...
    public static int showStatus(Minecraft client) {
        ClientChat.send(client,
                "AntiLava: " + (ENABLED.get() ? "enabled" : "disabled")
                        + ", delay: " + DELAY_MIN.get() + "-" + DELAY_MAX.get() + " ticks"
                        + ", range: " + format(RANGE.get())
                        + ", fov: " + format(FOV.get()) + " degrees"
