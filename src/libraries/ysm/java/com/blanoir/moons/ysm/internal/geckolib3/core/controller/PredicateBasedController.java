...
import com.blanoir.moons.ysm.internal.client.animation.IAnimationPredicate;
...
public class PredicateBasedController<T extends AnimatableEntity<?>>
    ...
    @Nullable private IValue soundIValue;

    private boolean needsReset;

    public PredicateBasedController(
            T animatable,
            String name,
    ...
    public PredicateBasedController(
        ...
        this.transitionInterpolator =
                new AnimationControllerInstance(animatable, transitionLengthTicks, true);
    ...
    public void process(
        ...
        PlayState playState;
        try {
            boolean forceSwingPredicate =
                    PLAYER_SWING_CONTROLLER.equals(name) && event.getAnimatable().isSwinging();
        ...
        event.getAnimatable()
                .setAnimationState(this.name, this.transitionInterpolator.getAnimationState());
    }

    @Nullable
    private PlayState handleSoundExpression(
            ExpressionEvaluator<AnimationContext<?>> expressionEvaluator) {
        ...
        expressionEvaluator
                .entity()
                .setAnimationControllerContext(this.transitionInterpolator.getContext());
    ...
    public void init(
        ...
        this.transitionInterpolator.setTransitionInterpolator(
                new TicksInterpolator(this.transitionLengthTicks));
        this.soundIValue = null;
        List<IValue> list2 =
                object2ReferenceMap.get(
    ...
    public boolean isDeprecatedMode() {
        return this.deprecatedMode
                && this.transitionInterpolator.getAnimationState() == AnimationState.RUNNING;
    }
    ...
    public static final class TransformProviderRecord implements BoneTransformProvider {
        ...
        public TransitionVector3f getRotation(ExpressionEvaluator<AnimationContext<?>> evaluator) {
            ...
            if (point instanceof ConstantPoint) {
                ...
                Vector3f vector3fMul =
                        transition.evaluateRaw(evaluator).mul(this.data.getBlendWeight());
                MathUtil.nlerpEulerAngles(
                        transition.getLerpFactor(),
                        transition.getOffsetPoint(),
                        vector3fMul,
                        this.data.topLevelSnapshot.bone.getInitialRotation(),
                        vector3fMul,
                        this.rotScratch);
        ...
        public TransitionVector3f getPosition(ExpressionEvaluator<AnimationContext<?>> evaluator) {
            ...
            if (point instanceof ConstantPoint) {
                ...
                if (point instanceof TransitionPoint) {
                    blendWeight =
                            MathUtil.lerpValues(
                                    ((TransitionPoint) point).getLerpFactor(), 1.0f, blendWeight);
                }
                mutableVector.setPercentCompleted(0.0f);
            }
            if (blendWeight != 1.0f) {
        ...
        public String toString() {
            return "TransformProviderRecord[" + "data=" + data + ']';
        }
    }
}
