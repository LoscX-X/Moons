...
import com.blanoir.moons.client.utils.inventory.InventoryTransfer;
...
public final class AutoArmor {
    ...
    private static final InventoryTransfer TRANSFER = new InventoryTransfer();
    ...
    public static int setEnabled(Minecraft client, boolean value) {
        ENABLED.set(value);
        if (TRANSFER.active()) TRANSFER.cancel();
        else reset();
        return 1;
    }
    ...
    private static void tick(Minecraft client) {
            ...
            if (!enabled()
                    || client.player != null
                            && (moving(client)
                                    ...
                                    || now < combatUntil)) TRANSFER.cancel();
            boolean allow = now >= nextActionAt && mouseHeld.isEmpty() && client.isWindowActive();
            TRANSFER.tick(client, allow);
            status = TRANSFER.result();
            if (!TRANSFER.active())
                stalled =
                        !status.equals("Armor equipped")
                                && !status.equals("Armor action cancelled");
            if (allow || !TRANSFER.active()) nextActionAt = now + nextDelay() * 1_000_000L;
            return;
        }
        ...
        TRANSFER.tick(client, true);
        status = TRANSFER.result();
        if (!TRANSFER.active())
            stalled = !status.equals("Armor equipped") && !status.equals("Armor action cancelled");
        nextActionAt = now + nextDelay() * 1_000_000L;
    }
