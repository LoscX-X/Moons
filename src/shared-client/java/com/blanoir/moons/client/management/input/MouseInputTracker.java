...
public final class MouseInputTracker {
    ...
    public static Motion currentMotion(long nowNanos) {
        while (!samples.isEmpty() && nowNanos - samples.peekFirst().timeNanos() > STALE_AFTER_NANOS) {
            samples.removeFirst();
        }
        ...
        boolean recentlyMoved = nowNanos - lastSampleNanos <= STALE_AFTER_NANOS
    ...
    public record Motion(double velocityPxPerSecond, double accelerationPxPerSecondSquared, boolean recentlyMoved) {
    }
