...
public final class SilentAuraVerification {
    ...
    private static void noise() {
        ...
        check(
                AimJitter.insideHitbox(eye, edge, box, 0, 7L, 1.0E-5D, 1).distanceTo(edge)
                        < 1.0E-5D,
                "near-zero strength cannot snap a surface point inward");
            ...
            for (double speed : new double[] {0.1D, 0.85D, 1, 3}) {
                for (long seed = 1; seed <= 16; seed++) {
                    ...
                    for (int frame = 0; frame < 240; frame++) {
                        ...
                        Vec3 point =
                                AimJitter.insideHitbox(eye, base, box, time, seed, strength, speed);
                        check(box.contains(point), "noise remains inside the hitbox");
                        check(point.distanceTo(base) <= 0.07D, "noise amplitude is bounded");
                        if (strength == 0)
                            check(point.equals(base), "zero strength is exactly neutral");
                        if (previous != null)
                            check(
                                    point.distanceTo(previous) < 2.0D / 120.0D,
        ...
        check(
                AimMotionNoise.sample(0.41D, 1L) != AimMotionNoise.sample(0.41D, 2L),
    ...
    private static void balance() throws Exception {
        ...
        for (int fps : new int[] {30, 60, 144, 1000, 2000}) {
            ...
            step.invoke(
                    controller, new Rotation(-90, 30), 0.0D, 21.28D, 660.0D, 440.0D, 1.0D, 1.0D);
            near(controller.yaw(), before, 0, "zero elapsed time cannot move aim");
            controller.clear();
            check(
                    !controller.active() && controller.targetId() == -1,
                    "clear releases controller state");
        }
    }
        ...
        for (boolean lock : new boolean[] {false, true}) {
            for (boolean matrix : new boolean[] {false, true}) {
                ...
                for (int tick = 0; tick < 60; tick++) {
                    ...
                    check(
                            smoother.sample(tick, 40, 30, -80, 35, 0.5D, true, matrix)
                                    .equals(sample),
