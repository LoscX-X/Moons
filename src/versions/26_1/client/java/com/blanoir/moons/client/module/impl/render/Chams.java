...
public final class Chams {
    ...
    private static final BooleanSetting CHAMS_ENABLED =
            new BooleanSetting.Builder()
                    .name("chams.players.enabled")
                    .defaultValue(true)
                    .build();

    private static final ChamsRenderTarget RENDER_TARGET = new ChamsRenderTarget(ClientBranding.name() + " Chams");
    ...
    public static RenderType remapHeldItemRenderType(RenderType renderType) {
        if (!CHAMS_ENABLED.get() || renderingItemSubmit == null || !HELD_ITEM_SUBMITS.contains(renderingItemSubmit)
    ...
    public static void compositeIfNeeded() {
        ...
        try {
            ...
            RENDER_TARGET.composite(MinecraftClientAccess.mainRenderTarget(Minecraft.getInstance()));
        } finally {
            dirty = false;
            HELD_ITEM_SUBMITS.clear();
        }
