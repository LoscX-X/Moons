...
public final class CombatDecisionEngine {
    ...
    public static Decision evaluate(
        ...
        return evaluate(client, target, lookaheadTicks, 0,
                stopSprintForCritical, manualIntent, SyncPolicy.disabled());
    }
    ...
    public static Decision evaluate(
        ...
        return evaluate(client, target, lookaheadTicks, earliestAttackTick,
                stopSprintForCritical, manualIntent, SyncPolicy.disabled());
    }
    ...
    public static Decision evaluate(
        ...
        if (currentReach && currentDamageable && canCriticalNow(client, target)) {
            return new Decision(AttackKind.CRITICAL, 0,
                    estimateDamage(client, livingTarget, currentCharge, true),
                    1.0D, "vanilla-critical-now");
        }

        AttackKind currentKind = client.player.isSprinting()
                ? AttackKind.SPRINT : AttackKind.NORMAL;
        Decision current = earliest == 0 && currentReach && currentDamageable
                ? new Decision(currentKind, 0,
                estimateDamage(client, livingTarget, currentCharge, false),
                1.0D, "vanilla-normal-now")
                : Decision.abort("normal-not-ready");

        if (current.attackNow() && wouldCurrentNormalKill(client, target)) {
            return new Decision(current.attackKind(), 0, current.score(), 1.0D,
                    "normal-lethal");
        }

        Decision futureCritical = findFutureCritical(
                client, livingTarget, horizon, earliest, stopSprintForCritical,
        ...
        return current.attackNow()
                ? current
                : Decision.abort("no-near-critical");
    }
    ...
    private static Decision findFutureCritical(
        ...
        double currentCharge = client.player.getAttackStrengthScale(0.5F);
        boolean physicalJumpHeld = CombatInputController.isPhysicallyDown(
                client, client.options.keyJump);
        boolean jumpPhaseActive = !client.player.onGround()
                || physicalJumpHeld
        ...
        if (syncPolicy.enabled() && physicalJumpHeld && syncPolicy.cycles() > 1) {
            searchHorizon = Math.min(MAX_FORECAST_TICKS,
                    horizon + (int) Math.ceil(attackDelay)
                            * (syncPolicy.cycles() - 1)
                            + Math.max(0, syncPolicy.maxOverchargeTicks()) + 8);
        }
        int forecastHorizon = Math.min(MAX_FORECAST_TICKS + CRITICAL_LANDING_MARGIN_TICKS,
        ...
        for (int tick = Math.max(1, earliest); tick <= searchHorizon; tick++) {
            ...
            int projectedOvercharge = projectedOverchargeTicks(
                    currentCharge, attackDelay, tick,
                    Math.max(0, syncPolicy.currentOverchargeTicks()));
            if (syncPolicy.enabled() && jumpPhaseActive
            ...
            Vec3 futureEye = client.player.getEyePosition().add(
            ...
            double confidence = Mth.clamp(1.0D
                    - targetVelocity.horizontalDistance() * tick * 0.035D,
                    0.55D, 1.0D);
            int syncedFollowUps = syncPolicy.enabled() && physicalJumpHeld
                    ? countSyncedFollowUps(states, tick, attackDelay,
            ...
            return new Decision(AttackKind.CRITICAL, tick,
                    damage * confidence + continuityBonus, confidence,
                    syncedFollowUps > 0
                            ? "synced-critical-chain"
                            : vertical.jumpCycle() > 1
                            ? "next-jump-critical" : "predicted-vanilla-critical");
        }
        return Decision.abort("no-near-critical");
    }

    private static boolean hasCriticalLandingMargin(Minecraft client) {
        VerticalState[] states = forecastVerticalStates(
                client, CRITICAL_LANDING_MARGIN_TICKS);
        return hasForecastLandingMargin(states, 0);
    }

    private static boolean hasForecastLandingMargin(
    ...
    private static int projectedOverchargeTicks(
        ...
        int ticksUntilFull = Math.max(0,
                (int) Math.ceil((1.0D - currentCharge) * attackDelay));
    ...
    private static int countSyncedFollowUps(
        ...
        for (int cycle = 1; cycle < cycles; cycle++) {
            ...
            int latestTick = Math.min(states.length - 1,
                    fullyChargedTick + maxOverchargeTicks);
            int matchedTick = -1;
    ...
    private static boolean worthWaitingForCritical(
        ...
        double waitPenalty = futureCritical.ticksAhead()
                * (manualIntent ? 0.055D : 0.035D);
    ...
    private static VerticalState[] forecastVerticalStates(Minecraft client, int count) {
        ...
        boolean physicalJumpHeld = CombatInputController.isPhysicallyDown(
                client, client.options.keyJump);
        boolean effectiveJumpHeld = physicalJumpHeld
                || CombatInputController.isDown(client, client.options.keyJump);
        int jumpCycle = onGround ? 0 : 1;

        states[0] = new VerticalState(
                yOffset,
                fallDistance > 0.0D
        ...
        for (int tick = 1; tick <= count; tick++) {
            ...
            if (!onGround) {
                if (client.player.hasEffect(MobEffects.LEVITATION)) {
                    ...
                    double gravity = client.player.hasEffect(MobEffects.SLOW_FALLING)
                            && velocityY <= 0.0D ? SLOW_FALLING_GRAVITY : GRAVITY;
    ...
    private static double estimateDamage(
        ...
        double attackDamage = Math.max(0.0D,
                client.player.getAttributeValue(Attributes.ATTACK_DAMAGE));
        ...
        double toughness = Math.max(0.0D,
                target.getAttributeValue(Attributes.ARMOR_TOUGHNESS));
        double armorPoints = Math.min(20.0D,
                Math.max(armor / 5.0D,
