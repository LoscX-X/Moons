...
public final class AutoObsidian {
    ...
    private static final double[] FACE_SAMPLES = {-0.36D, -0.18D, 0.0D, 0.18D, 0.36D};
    ...
    private static final BooleanSetting ENABLED =
            new BooleanSetting.Builder().name("autoobsidian.enabled").defaultValue(false).build();
    ...
    private static void begin(Minecraft client) {
        ...
        if (target == null) {
            cleanup(client, true, "AutoObsidian disabled: no target inside scan range and FOV.");
            return;
        }
        ...
        plan = foundPlan;
        finishMessage = null;
        CombatInputController.suppressAttack(client, CombatInputController.Owner.AUTO_OBSIDIAN);
    ...
    private static void confirmObsidian(Minecraft client) {
        if (client.level.getBlockState(plan.lavaPos()).is(Blocks.OBSIDIAN)) {
            ...
            if (COLLECT_WATER.get()
                ...
                beginPickup(
                        client,
                        new PickupPlan(plan.waterPos(), waterSlot, Items.WATER_BUCKET, false));
            } else {
                beginReturn(client);
            }
            return;
    ...
    private static void switchAndRotate(Minecraft client, int slot, Phase turningPhase) {
        if (selectSlot(client, slot)) {
            ...
            switchReadyAtNanos = System.nanoTime() + SWITCH_DELAY_MS.get() * 1_000_000L;
    ...
    private static void beginRotation(Minecraft client, Phase turningPhase) {
        ...
        SilentPacketRotation.beginRotation(
                client, target, SMOOTH_TICKS.get(), () -> transitionIf(turningPhase, waitingPhase));
    }
    ...
    private static void fail(Minecraft client, String reason) {
        ...
        if (plan != null
            ...
            beginPickup(client, new PickupPlan(plan.lavaPos(), lavaSlot, Items.LAVA_BUCKET, true));
            return;
        }
        if (plan != null
                && isSource(client, plan.waterPos(), FluidTags.WATER)
                ...
                && client.player.getInventory().getItem(waterSlot).is(Items.BUCKET)) {
            beginPickup(
                    client, new PickupPlan(plan.waterPos(), waterSlot, Items.WATER_BUCKET, false));
            return;
        }
        beginReturn(client);
    }
    ...
    private static TrapPlan findPlan(Minecraft client, Player target) {
        ...
        BlockHitResult lavaHit = bestVisibleFace(client, ceiling, Direction.DOWN);
        ...
        for (Direction opening : HORIZONTAL) {
            ...
            if (!validBedCell(client, target, near) || !validBedCell(client, target, far)) {
                continue;
            }
            ...
            for (boolean footNear : new boolean[] {true, false}) {
                ...
                BlockHitResult bedHit = findBedPlacementHit(client, foot, facing);
                if (bedHit == null) {
                    continue;
                }
                Set<BlockPos> forbiddenSupports =
                        Set.of(foot.immutable(), head.immutable(), lavaPos.immutable());
                BlockHitResult waterHit = findPlacementHit(client, waterPos, forbiddenSupports);
                if (waterHit == null) {
                    continue;
                }
                int headWalls = 0;
                ...
                double score =
                        -wallCount * 1_000.0D
                                ...
                                + client.player
                                        .getEyePosition()
                                        .distanceToSqr(bedHit.getLocation());
                if (score < bestScore) {
                    bestScore = score;
                    best =
                            new TrapPlan(
                                    targetFoot.immutable(),
                                    foot.immutable(),
                                    head.immutable(),
                                    near.immutable(),
                                    waterPos.immutable(),
    ...
    private static BlockHitResult findBedPlacementHit(
        ...
        for (double x : FACE_SAMPLES) {
            for (double z : FACE_SAMPLES) {
                ...
                BlockHitResult hit = visibleFaceHit(client, floor, Direction.UP, requested);
                if (hit == null
                        || !withinReach(client, hit.getLocation())
                        || Direction.fromYRot(yawTo(eye, hit.getLocation())) != requiredFacing) {
                    continue;
                }
    ...
    private static BlockHitResult findPlacementHit(
        ...
        for (Direction supportDirection : SUPPORT_DIRECTIONS) {
            ...
            double distance = client.player.getEyePosition().distanceToSqr(hit.getLocation());
            if (distance < bestDistance) {
                bestDistance = distance;
                best = hit;
            }
    ...
    private static BlockHitResult bestVisibleFace(
        ...
        for (double first : FACE_SAMPLES) {
            for (double second : FACE_SAMPLES) {
                BlockHitResult hit =
                        visibleFaceHit(
                                client, support, face, pointOnFace(support, face, first, second));
                if (hit == null) {
                    continue;
                }
                double distance = client.player.getEyePosition().distanceToSqr(hit.getLocation());
    ...
    private static BlockHitResult visibleFaceHit(
        ...
        return BlockPlacementUtils.visibleFaceHit(
                client, client.player.getEyePosition(), support, face, requested, RAY_EPSILON);
    }
    ...
    private static boolean validLavaAction(Minecraft client) {
        return plan != null
                ...
                && visibleFaceHit(
                                client,
                                plan.lavaHit().getBlockPos(),
                                plan.lavaHit().getDirection(),
                                plan.lavaHit().getLocation())
                        != null;
    }
    ...
    private static boolean validWaterAction(Minecraft client) {
        return plan != null
                ...
                && visibleFaceHit(
                                client,
                                plan.waterHit().getBlockPos(),
                                plan.waterHit().getDirection(),
                                plan.waterHit().getLocation())
                        != null;
    }
    ...
    private static boolean sentLookMatches(Minecraft client, BlockHitResult planned) {
        ...
        return BlockPlacementUtils.matchesFace(actual, planned);
    }
    ...
    private static boolean validBedCell(Minecraft client, Player target, BlockPos pos) {
        if (target == null || !replaceableDry(client, pos) || !solid(client, pos.below())) {
            return false;
        }
    ...
    private static boolean isSource(
        ...
        BlockState state = client.level.getBlockState(pos);
        return FluidQueries.isSource(state.getFluidState(), tag);
    }
    ...
    private static boolean validTarget(Minecraft client, Player target) {
        if (!Targeting.isValidTargetPlayer(client, target)
                || EntityDistance.squaredToEntity(client, target) > RANGE.get() * RANGE.get()) {
            return false;
        }
        Vec3 eye = client.player.getEyePosition();
    ...
    private static void cleanup(Minecraft client, boolean disable, String message) {
        ...
        CombatInputController.releaseAttack(client, CombatInputController.Owner.AUTO_OBSIDIAN);
        if (owned) {
    ...
    private record PickupPlan(
            BlockPos pos, int slot, net.minecraft.world.item.Item refilledItem, boolean lava) {}
