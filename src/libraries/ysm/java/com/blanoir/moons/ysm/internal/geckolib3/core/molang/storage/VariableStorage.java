...
import com.blanoir.moons.ysm.internal.geckolib3.core.molang.util.PooledStringHashMap;
import com.blanoir.moons.ysm.internal.geckolib3.core.molang.util.PooledStringHashSet;
...
public class VariableStorage implements IScopedVariableStorage, IForeignVariableStorage {
    ...
    private final PooledStringHashMap<VariableValueHolder> scopedMap =
            new PooledStringHashMap<>(SCOPED_INIT_CAPACITY);

    private PooledStringHashMap<VariableValueHolder> publicMap = new PooledStringHashMap<>();

    @Override
    public Object getScoped(int name) {
        VariableValueHolder valueHolder =
                scopedMap.computeIfAbsent(name, n -> new VariableValueHolder());
    ...
    public void initialize(@Nullable PooledStringHashSet publicVariableNames) {
        ...
        if (publicVariableNames != null && !publicVariableNames.isEmpty()) {
            PooledStringHashMap<VariableValueHolder> newPublicMap =
                    new PooledStringHashMap<>(publicVariableNames.size());
