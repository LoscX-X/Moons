...
public final class ClickGuiWarmupVerification {
    public static void main(String[] args) throws Exception {
        Path config = Path.of(System.getProperty("java.io.tmpdir"), "moons-gui-check-" + UUID.randomUUID());
        Settings.configure(config);
        Settings.load();
        var preview = ClickGuiWarmup.class.getDeclaredMethod("renderPreview");
        preview.setAccessible(true);
        AtomicReference<Throwable> failure = new AtomicReference<>();
        Thread worker = Thread.ofPlatform().daemon().name("clickgui-verification").start(() -> {
            try {
                for (int run = 0; run < 2; run++) {
                    long start = System.nanoTime();
                    preview.invoke(ClickGuiWarmup.INSTANCE);
                    System.out.println("CLICKGUI_CPU_PREVIEW run=" + run
