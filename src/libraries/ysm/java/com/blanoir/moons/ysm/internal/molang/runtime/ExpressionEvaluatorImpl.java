...
public final class ExpressionEvaluatorImpl<TEntity>
    ...
    private static final Evaluator[] BINARY_EVALUATORS = {
            ...
            if (a instanceof AssignableVariableExpression) {
                ...
                if (value instanceof Struct) {
                    ...
                    AssignableVariable variable =
                            ((AssignableVariableExpression) exp.left()).target();
        ...
        (evaluator, a, b) -> {
            ...
            if (left instanceof StringExpression) return left.equals(right);
            if (right instanceof StringExpression) return right.equals(left);
            return false;
        }, // eq
        (evaluator, a, b) -> {
            Object left = a.visit(evaluator);
    ...
    @Nullable private StatementExpression.Op op;
    ...
    private float evalFloat(@NotNull Expression expr) {
        ...
        if (expr instanceof BinaryExpression be) {
            switch (be.op()) {
                case ADD:
                    return evalFloat(be.left()) + evalFloat(be.right());
                case SUB:
                    return evalFloat(be.left()) - evalFloat(be.right());
                case MUL:
                    return evalFloat(be.left()) * evalFloat(be.right());
                ...
                case LT:
                    return evalFloat(be.left()) < evalFloat(be.right()) ? 1.0f : 0.0f;
                case LTE:
                    return evalFloat(be.left()) <= evalFloat(be.right()) ? 1.0f : 0.0f;
                case GT:
                    return evalFloat(be.left()) > evalFloat(be.right()) ? 1.0f : 0.0f;
                case GTE:
                    return evalFloat(be.left()) >= evalFloat(be.right()) ? 1.0f : 0.0f;
                case AND:
                    return (evalBool(be.left()) && evalBool(be.right())) ? 1.0f : 0.0f;
                case OR:
                    return (evalBool(be.left()) || evalBool(be.right())) ? 1.0f : 0.0f;
                default:
                    break;
        ...
        if (expr instanceof UnaryExpression ue) {
            switch (ue.op()) {
                case ARITHMETICAL_NEGATION:
                    return -evalFloat(ue.expression());
                case PLUS:
                    return evalFloat(ue.expression());
                case LOGICAL_NEGATION:
                    return evalBool(ue.expression()) ? 0.0f : 1.0f;
                case RETURN:
                    return evalFloat(ue.expression());
                default:
                    break;
    ...
    private boolean evalBool(@NotNull Expression expr) {
        ...
        if (expr instanceof BinaryExpression be) {
            switch (be.op()) {
                case AND:
                    return evalBool(be.left()) && evalBool(be.right());
                case OR:
                    return evalBool(be.left()) || evalBool(be.right());
                case LT:
                    return evalFloat(be.left()) < evalFloat(be.right());
                ...
                case GTE:
                    return evalFloat(be.left()) >= evalFloat(be.right());
                case ADD:
                    return (evalFloat(be.left()) + evalFloat(be.right())) != 0.0f;
                case SUB:
                    return (evalFloat(be.left()) - evalFloat(be.right())) != 0.0f;
                case MUL:
                    {
    ...
    @NotNull
    public <TNewEntity> ExpressionEvaluatorImpl<TNewEntity> createChild(
            @Nullable TNewEntity tnewentity) {
    ...
    public void forEachFunction(
            @NotNull ExecutionScopeExpression executionScope,
    ...
    public Object visitTernaryConditional(@NotNull TernaryConditionalExpression expression) {
        ...
        obj =
                ValueConversions.asBoolean(obj)
