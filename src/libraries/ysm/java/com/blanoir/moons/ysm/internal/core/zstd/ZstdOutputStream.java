...
import static com.blanoir.moons.ysm.internal.core.zstd.CompressionParameters.DEFAULT_COMPRESSION_LEVEL;
...
public class ZstdOutputStream extends OutputStream {
    ...
    private void compressIfNecessary() throws IOException {
        // only flush when the buffer if is max size, full, and the buffer is larger than the window
    ...
    private void writeChunk(boolean lastChunk) throws IOException {
        ...
        if (partialHash == null) {
            ...
            // if this is also the last chunk we know the exact size, otherwise, this is traditional
            // streaming
        ...
        if (lastChunk) {
            ...
            System.arraycopy(
                    ...
                    0,
                    context.parameters.getWindowSize()
                            + (uncompressedPosition - uncompressedOffset));
