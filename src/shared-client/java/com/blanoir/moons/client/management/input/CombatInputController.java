...
public final class CombatInputController {
    ...
    public static void init() {
        ...
        initialized = true;
        EventBus.TICK.register("CombatInputController.tickSyntheticAttack",
        ...
        if (valid(client) && jumpForcers.isEmpty()) {
            ...
            if (key.getType() == InputConstants.Type.MOUSE) {
                ...
                if (!isPhysicallyDown(client, mapping)) {
                    GameAccess.invokeMouseButton(client.mouseHandler,
    ...
    public static boolean isPhysicallyDown(Minecraft client, KeyMapping mapping) {
        ...
        if (key.getType() == InputConstants.Type.MOUSE) {
            return GLFW.glfwGetMouseButton(
                    client.getWindow().handle(), key.getValue()) == GLFW.GLFW_PRESS;
        }
    ...
    public static boolean attackTargetNow(
        ...
        boolean cameraAlreadyTargetsEntity = client.hitResult instanceof EntityHitResult hit
                && hit.getEntity() == target;
        pendingAttackTarget = forceTargetOverride || !cameraAlreadyTargetsEntity
                ? target : null;
    ...
    private static boolean valid(Minecraft client) {
        ...
        return client != null && MinecraftClientAccess.screen(client) == null && client.player != null && client.level != null && client.gameMode != null;
    }
}
