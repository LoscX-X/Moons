...
final class SilentAuraRotationRouter {
    private final SilentAuraRotationController lock = new SilentAuraRotationController(true);
    private final SilentAuraRotationController balance = new SilentAuraRotationController(false);
    ...
    void completeReturn() {
        activeController().completeReturn();
    }

    void cancelReturn(Minecraft client) {
        activeController().cancelReturn(client);
    }
    ...
    void beginFrom(float yaw, float pitch) {
        activeController().beginFrom(yaw, pitch);
    }

    boolean returning() {
        return activeController().returning();
    }

    int targetId() {
        return activeController().targetId();
    }

    float yaw() {
        return activeController().yaw();
    }

    float pitch() {
        return activeController().pitch();
    }

    boolean crossingTarget() {
        return activeController().crossingTarget();
    }

    float bodyYaw() {
        return activeController().bodyYaw();
    }

    Vec3 lookVector() {
        return activeController().lookVector();
    }
