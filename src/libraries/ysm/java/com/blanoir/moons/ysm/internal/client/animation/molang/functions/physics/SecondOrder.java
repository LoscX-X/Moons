package com.blanoir.moons.ysm.internal.client.animation.molang.functions.physics;
...
public class SecondOrder implements IPhysics {
    ...
    public SecondOrder(float input, float frequency, float coefficient, float response) {
        this.input = input;
        this.frequency = ScalarMath.clamp(frequency, 0, 5);
        this.coefficient = ScalarMath.clamp(coefficient, 0, 1);
    ...
    public void update(float timeStep) {
        ...
        float frequency = ScalarMath.clamp(finiteOrZero(this.frequency), MIN_FREQUENCY, 5.0f);
        float coefficient = ScalarMath.clamp(finiteOrZero(this.coefficient), 0.0f, 1.0f);
        float response = finiteOrZero(this.response);

        float k1 = coefficient / ScalarMath.PI / frequency;
        float k2 = 1.0f / (2.0f * ScalarMath.PI * frequency) / (2.0f * ScalarMath.PI * frequency);
        float k3 = response * coefficient / 2.0f / ScalarMath.PI / frequency;
        ...
        int cycleTime = ScalarMath.clamp((int) Math.ceil(timeStep / maxTimeStep), 1, MAX_CYCLES);
        timeStep = timeStep / cycleTime;

        float lastSimulationDot =
                Float.isFinite(this.lastSimulationDot) ? this.lastSimulationDot : 0.0f;
