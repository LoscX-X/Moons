...
public class CompositeAnimationController<T extends AnimatableEntity<?>>
    ...
    public CompositeAnimationController(
        ...
        this.controller =
                new PredicateBasedController<>(
                        animatable, name, transitionLengthTicks, predicate, deprecatedMode);
        this.animationRuntime =
                new AnimationControllerRuntime<>(animatable, name, transitionLengthTicks);
    ...
    public String getCurrentAnimation() {
        if (this.initialized) {
            return this.animationRuntime.isBuiltinAnimation()
                    ? "[builtin] " + this.controller.getCurrentAnimation()
        ...
        if (this.initialized) {
            ...
            if (this.animationRuntime.isBuiltinAnimation()) {
                if (this.activeController != this.controller) {
                    this.controller.setInterpolator(
                            this.animationRuntime
                                    .getCurrentEntry()
                                    .getBlendTransition()
                                    .asInterpolator());
