...
public final class AutoSword {
    ...
    public static void init() {
        EventBus.TICK.register("AutoSword.tick", event -> {
    ...
    public static void onAttack(Entity target) {
        ...
        if (!ENABLED.get() || !ready(client) || AntiWeb.isBusy()
    ...
    private static int findSwordSlot(Minecraft client) {
        ...
        for (int slot = 0; slot < 9; slot++) {
            if (isSword(inventory.getItem(slot))) {
                return slot;
            }
        }
    ...
    public static int showStatus(Minecraft client) {
        ClientChat.send(
                ...
                "AutoSword: " + statusText()
                        + ", slot: " + SWORD_SLOT.get()
                        + ", switchBack: " + toggleText(SWITCH_BACK.get())
                        + ", switchBackDelay: " + SWITCH_BACK_TICKS.get() + "t"
    ...
    public static int setSwitchBackDelay(Minecraft client, int value) {
        ...
        ClientChat.send(client, "AutoSword switchBackDelay set to " + SWITCH_BACK_TICKS.get() + " ticks.");
        return 1;
    }
