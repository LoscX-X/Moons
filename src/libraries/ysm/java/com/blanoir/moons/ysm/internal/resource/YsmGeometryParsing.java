...
public final class YsmGeometryParsing {
     ...
     */
    public static RawYsmModel.RawGeometry parseGeometry(
            byte[] data, int modelType, String identifier) {
        return parseGeometry(data, modelType, identifier, null);
    }

    public static RawYsmModel.RawGeometry parseGeometry(
            byte[] data, int modelType, String identifier, LegacyMetadataSink legacyMetadataSink) {
        String json = new String(data, StandardCharsets.UTF_8);
        JsonObject root = JsonParser.parseString(json).getAsJsonObject();
        JsonArray geometries =
                root.has("minecraft:geometry") ? root.getAsJsonArray("minecraft:geometry") : null;
        ...
        if (geoObj.has("description")) {
            ...
            geo.visibleBoundsWidth =
                    (float) YsmJsonSupport.getDouble(desc, "visible_bounds_width", 0);
            geo.visibleBoundsHeight =
                    (float) YsmJsonSupport.getDouble(desc, "visible_bounds_height", 0);
            if (desc.has("visible_bounds_offset")
                    && desc.get("visible_bounds_offset").isJsonArray()) {
            ...
            if (modelType == 1 && desc.has("ysm_extra_info")) { // legacy
                if (legacyMetadataSink != null)
                    legacyMetadataSink.accept(desc.getAsJsonObject("ysm_extra_info"), false);
            }
        }
            ...
            for (JsonElement boneElem : geoObj.getAsJsonArray("bones")) {
                ...
                if (bObj.has("cubes") && bObj.get("cubes").isJsonArray()) {
                    for (JsonElement cElem : bObj.getAsJsonArray("cubes")) {
                        ...
                        float inflate =
                                cObj.has("inflate")
                                        ? cObj.get("inflate").getAsFloat()
                                        : boneInflate;
                        boolean mirror =
                                cObj.has("mirror") ? cObj.get("mirror").getAsBoolean() : boneMirror;
                        ...
                        if (cObj.has("uv")) {
                            ...
                            if (uvElem.isJsonObject()) {
                                ...
                                fakeUvObj.add(
                                        "north", createFaceUVNode(uvX + dz, uvY + dz, dx, dy));
                                fakeUvObj.add(
                                        "south",
                                        createFaceUVNode(uvX + dz + dx + dz, uvY + dz, dx, dy));
                                fakeUvObj.add("east", createFaceUVNode(uvX, uvY + dz, dz, dy));
                                fakeUvObj.add(
                                        "west", createFaceUVNode(uvX + dz + dx, uvY + dz, dz, dy));
                                fakeUvObj.add("up", createFaceUVNode(uvX + dz, uvY, dx, dz));
                                fakeUvObj.add(
                                        "down", createFaceUVNode(uvX + dz + dx, uvY + dz, dx, -dz));
    ...
    private static JsonObject selectGeometry(JsonArray geometries, String identifier) {
        if (identifier != null && !identifier.isEmpty()) {
            ...
            for (JsonElement element : geometries) {
                ...
                if (!candidate.has("description") || !candidate.get("description").isJsonObject())
                    continue;
                String id =
                        YsmJsonSupport.getJsonString(
                                candidate.getAsJsonObject("description").get("identifier"));
                if (id == null || id.isEmpty()) continue;
                if (id.equalsIgnoreCase(want)
                        || id.equalsIgnoreCase(wantWithPrefix)
                        || id.endsWith("." + want)) {
                    return candidate;
                }
            }
    ...
    private static void bakeFaceToRaw(
            RawYsmModel.RawCube cube,
            JsonObject uvObj,
            String faceType,
            String uvFaceName,
            boolean mirror,
            ...
            float th,
            Vector3f rawNormal,
            Matrix4f cubeBakeMat,
            Matrix3f cubeNormalMat) {
        ...
        for (int i = 0; i < 4; i++) {
            tempPos.set(positions[i].x(), positions[i].y(), positions[i].z(), 1.0f)
                    .mul(cubeBakeMat);
