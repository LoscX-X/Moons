...
public final class RandomFakeLag {
    ...
    private static void tick(TickEvent event) {
        ...
        synchronized (LOCK) {
            ...
            if (queueing) {
                Player target = client.level.getEntity(targetId) instanceof Player player
                        ? player : null;
                if (now - startedAtMs >= durationMs
                        || !isHeadOnSprint(client, target)) {
    ...
    public static boolean handleOutgoing(Connection connection, Packet<?> packet) {
        if (PACKETS.isReplaying() || LowHealthFakeLag.isReplaying()
                || FakeLag.isReplaying()) {
            return false;
        }
    ...
    public static void handleIncoming(Packet<?> packet) {
        synchronized (LOCK) {
            ...
            if (FakeLagPacketPolicy.mustFlushOnIncoming(
                    Minecraft.getInstance(), packet)) {
    ...
    private static boolean isHeadOnSprint(Minecraft client, Player target) {
        ...
        if (playerLook == null || targetLook == null
    ...
    public static int showStatus(Minecraft client) {
        ClientChat.send(client,
                "RandomFakeLag: " + (ENABLED.get() ? "enabled" : "disabled")
                        + ", chance: " + format(CHANCE.get())
                        + ", delay: " + DELAY_MIN.get() + "-" + DELAY_MAX.get() + "ms"
                        + ", range: " + format(RANGE_MIN.get()) + "-" + format(RANGE_MAX.get())
                        + ", cooldown: " + format(COOLDOWN_MS.get() / 1000.0D) + "s"
                        + ", trigger: sprinting toward a mutually-facing player"
                        + ", state: " + (queueing ? "active" : "idle") + "."
