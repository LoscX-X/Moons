...
import com.blanoir.moons.ysm.internal.geckolib3.core.builder.AnimationController;
import com.blanoir.moons.ysm.internal.geckolib3.core.builder.AnimationState;
import com.blanoir.moons.ysm.internal.geckolib3.core.event.predicate.AnimationEvent;
import com.blanoir.moons.ysm.internal.geckolib3.core.keyframe.AnimationPoint;
import com.blanoir.moons.ysm.internal.geckolib3.core.keyframe.BoneAnimationQueue;
import com.blanoir.moons.ysm.internal.geckolib3.core.keyframe.ConstantPoint;
import com.blanoir.moons.ysm.internal.geckolib3.core.keyframe.TransitionPoint;
...
import com.blanoir.moons.ysm.internal.geckolib3.core.util.EulerNlerpScratch;
import com.blanoir.moons.ysm.internal.geckolib3.core.util.MathUtil;
...
public class AnimationControllerRuntime<T extends AnimatableEntity<?>>
        implements IAnimationController<T> {
    ...
    @Nullable private List<BoneTopLevelSnapshot> boneTargets;

    @Nullable private AnimationController animationEntries;

    @Nullable private AnimationState currentEntry;

    @Nullable private String displayName;

    @Nullable private AnimationControllerRuntime<T> childController;

    private final ReferenceArrayList<AnimationSlot> animationSlots = new ReferenceArrayList<>(8);

    private int activeSlotCount = 0;

    private final Int2ReferenceOpenHashMap<BoneBlendState> boneTransformMap =
            new Int2ReferenceOpenHashMap<>(64);

    private final ReferenceArrayList<BoneBlendState> activeBoneTransforms =
            new ReferenceArrayList<>(16);

    private boolean needsRebuild = false;

    private final IntOpenHashSet visitedEntries = new IntOpenHashSet(4);

    @Nullable private String parentName = null;
    ...
    public void process(
        ...
        if (this.currentEntry != null
                && this.currentEntry.getSubName() != null
                && this.depth != MAX_DEPTH) {
            if (transitioned) {
                String subControllerName =
                        this.depth > 1
                                ? String.format(
                                        "%s.%s", this.parentName, this.currentEntry.getSubName())
                                : this.currentEntry.getSubName();
                AnimationController childController =
                        this.animatable.getAnimationEntries(
                                String.format("%s.%s", this.name, subControllerName));
                if (childController != null) {
                    if (this.childController == null) {
                        this.childController =
                                new AnimationControllerRuntime<>(
                                        this.animatable, this.name, this.transitionLengthTicks);
        ...
        for (int slotIndex = 0; slotIndex < this.activeSlotCount; slotIndex++) {
            ...
            slot.getResampler()
                    .process(currentTick, evaluator, isMoving && slot.getCondition().isActive());
        }
        if (this.needsRebuild) {
            for (int i2 = 0; i2 < this.activeSlotCount; i2++) {
                AnimationSlot slot2 = this.animationSlots.get(i2);
                ObjectListIterator it =
                        slot2.animationControllerInstance.getActiveBoneAnimationQueues().iterator();
                while (it.hasNext()) {
                    BoneAnimationQueue boneQueue = (BoneAnimationQueue) it.next();
                    BoneBlendState blendState =
                            this.boneTransformMap.get(boneQueue.topLevelSnapshot.boneId);
    ...
    public void initWithBones(
            List<BoneTopLevelSnapshot> list, @NotNull AnimationController controller) {
        reset();
    ...
    private boolean evaluateTransitions(ExpressionEvaluator<AnimationContext<?>> evaluator) {
        if (this.currentEntry == null) {
            AnimationState nextState =
                    this.animationEntries.getStates().get(this.animationEntries.getStateId());
            if (nextState == null) {
                return false;
            }
        ...
        for (IntReferenceImmutablePair<IValue> transition : this.currentEntry.getTransitions()) {
            ...
            if (!condition) {
                if (soundTransition) {
                    this.soundTransitionGate.allow(
                            this.currentEntry.getHashId(), nextState2.getHashId(), false);
                }
                continue;
            }
            if (soundTransition
    ...
    private void transitionToEntry(
            @Nullable AnimationState nextState,
            ExpressionEvaluator<AnimationContext<?>> evaluator) {
        ...
        int size =
                (nextState == null || nextState.isBuiltinEntry() || nextState.getSubName() != null)
                        ? 0
                        : nextState.getAnimations().size();
        ...
        for (int i = size; i < this.activeSlotCount; i++) {
            AnimationControllerInstance animInstance =
                    this.animationSlots.get(i).animationControllerInstance;
        ...
        for (int i = 0; i < size; i++) {
            ...
            slot.getResampler()
                    .setTransitionInterpolator(nextState.getBlendTransition().asInterpolator());
    ...
    private static class ConditionalEvaluator {
        ...
        @Nullable private IValue IValue;
    ...
    private static class BoneBlendState implements BoneTransformProvider {
        ...
        private final ReferenceArrayList<
                        it.unimi.dsi.fastutil.Pair<ConditionalEvaluator, BoneAnimationQueue>>
                blendSources = new ReferenceArrayList<>(4);
        ...
        public TransitionVector3f getRotation(ExpressionEvaluator<AnimationContext<?>> evaluator) {
            final ReferenceArrayList<
                            it.unimi.dsi.fastutil.Pair<ConditionalEvaluator, BoneAnimationQueue>>
                    sources = this.blendSources;
            ...
            for (int i = 0; i < size; i++) {
                it.unimi.dsi.fastutil.Pair<ConditionalEvaluator, BoneAnimationQueue> pair =
                        sources.get(i);
                if (pair.left().isActive()) {
                    BoneAnimationQueue boneQueue = pair.right();
                    if (boneQueue.isActive()
                            && (animationPoint = boneQueue.rotationQueue) != null) {
                        hasData = true;
                        if (isFirst) {
                            isFirst = false;
                            if (animationPoint instanceof TransitionPoint transition) {
                                ...
                                initialRotaiton =
                                        boneQueue.topLevelSnapshot.bone.getInitialRotation();
                            }
                        }
                        if (!isTransition) {
                            ...
                            if (animationPoint instanceof TransitionPoint transition) {
                                transitionVector3f.fma(
                                        ...
                                        transition.evaluateRaw(evaluator));
                            } else {
                                transitionVector3f.fma(
                                        boneQueue.getBlendWeight(),
                                        animationPoint.getLerpPoint(evaluator));
                            }
                        } else {
                    ...
                    if (boneQueue.isActive() && (point = boneQueue.positionQueue) != null) {
                        ...
                        if (!isTransition) {
                            ...
                            if (point instanceof TransitionPoint transition) {
                                ...
                                result.fma(
                                        boneQueue.getBlendWeight(), point.getLerpPoint(evaluator));
                            }
                        } else {
