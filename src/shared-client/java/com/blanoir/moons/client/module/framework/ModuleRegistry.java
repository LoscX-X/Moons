...
public final class ModuleRegistry {
    ...
    private static final Set<String> HUD_HIDDEN_MODULES = Set.of(
            "clickgui", "hud", "xraytargets", "chatprefix");
    ...
    public static JsonObject setEnabled(String id, boolean enabled) {
        ...
        try {
            ClientChat.withoutNoticesOnCurrentThread(() -> module.toggle().apply(Minecraft.getInstance(), enabled));
    ...
    public static JsonObject setValue(String moduleId, String settingId, JsonElement value) {
        ...
        try {
            ClientChat.withoutNoticesOnCurrentThread(() -> setting.apply().apply(Minecraft.getInstance(), value));
            return success(module.toJson());
        } catch (RuntimeException exception) {
            return error(messageOf(exception));
        }
    ...
    public static Module module(String id, String name, String category, BooleanSupplier enabled,
                                 Toggle toggle, Supplier<String> tag, Setting... settings) {
        String normalized = normalizeId(id);
        List<Setting> allSettings = new ArrayList<>(List.of(settings));
        allSettings.add(bool("hide", "Hide from HUD", "module." + normalized + ".hide",
                false, (client, value) -> { Settings.setBoolean("module." + normalized + ".hide", value); return 1; }));
        return new Module(normalized, name, category, enabled, toggle, tag, List.copyOf(allSettings));
    }
    ...
    public static synchronized void add(Module module) {
        if (!catalogInitializing) {
            throw new IllegalStateException("Modules may only be added while the catalog initializes");
        }
    ...
    public static Setting bool(String id, String name, String key, boolean fallback, BoolSetter setter) {
        return customBool(id, name, () -> Settings.getBoolean(key, fallback), setter);
    }

    public static Setting customBool(String id, String name, BooleanSupplier getter, BoolSetter setter) {
        return new Setting(normalizeId(id), name, "boolean", () -> new JsonPrimitive(getter.getAsBoolean()),
                null, null, null, List.of(), (client, value) -> setter.apply(client, value.getAsBoolean()));
    }

    public static Setting number(String id, String name, String key, double fallback,
                                  double min, double max, double step, DoubleSetter setter) {
        return numeric(id, name, "number", () -> Settings.getDouble(key, fallback), min, max, step,
                (client, value) -> setter.apply(client, value));
    }

    public static Setting numberText(String id, String name, String key, double fallback,
                                      double min, double max, double step, TextSetter setter) {
        return numeric(id, name, "number", () -> Settings.getDouble(key, fallback), min, max, step,
                (client, value) -> setter.apply(client, format(value)));
    }

    public static Setting integer(String id, String name, String key, int fallback,
                                   int min, int max, int step, IntSetter setter) {
        return numeric(id, name, "integer", () -> Settings.getInt(key, fallback), min, max, step,
                (client, value) -> setter.apply(client, (int) Math.round(value)));
    }

    public static Setting integerText(String id, String name, String key, int fallback,
                                       int min, int max, int step, TextSetter setter) {
        return numeric(id, name, "integer", () -> Settings.getInt(key, fallback), min, max, step,
                (client, value) -> setter.apply(client, Integer.toString((int) Math.round(value))));
    }

    public static Setting numeric(String id, String name, String type, Supplier<Number> getter,
                                   double min, double max, double step, NumericSetter setter) {
        return new Setting(normalizeId(id), name, type, () -> new JsonPrimitive(getter.get()),
                min, max, step, List.of(), (client, value) -> {
    ...
    public static Setting range(String id, String name, String minKey, String maxKey,
                                 double minFallback, double maxFallback, double min, double max, double step,
                                 TextSetter setter) {
        return rangeDoubles(id, name, minKey, maxKey, minFallback, maxFallback, min, max, step,
                (client, low, high) -> setter.apply(client, format(low) + "-" + format(high)));
    }

    public static Setting rangeInts(String id, String name, String minKey, String maxKey,
                                     int minFallback, int maxFallback, int min, int max, int step,
                                     IntRangeSetter setter) {
        return rangeValue(id, name,
                () -> Settings.getInt(minKey, minFallback), () -> Settings.getInt(maxKey, maxFallback),
                min, max, step, (client, low, high) -> setter.apply(client, (int) low, (int) high));
    }

    public static Setting rangeDoubles(String id, String name, String minKey, String maxKey,
                                        double minFallback, double maxFallback, double min, double max, double step,
                                        DoubleRangeSetter setter) {
        return rangeValue(id, name,
                () -> Settings.getDouble(minKey, minFallback), () -> Settings.getDouble(maxKey, maxFallback),
                min, max, step, setter::apply);
    }

    static Setting rangeValue(String id, String name, Supplier<Number> lowGetter, Supplier<Number> highGetter,
                                      double min, double max, double step, DoubleRangeSetter setter) {
        return new Setting(normalizeId(id), name, "range", () -> {
    ...
    public static Setting choice(String id, String name, String key, String fallback,
                                  List<String> choices, TextSetter setter) {
        return customChoice(id, name,
                () -> Settings.getString(key, fallback).toLowerCase(Locale.ROOT), choices, setter);
    }

    public static Setting customChoice(String id, String name, Supplier<String> getter,
                                        List<String> choices, TextSetter setter) {
        return new Setting(normalizeId(id), name, "choice",
                () -> new JsonPrimitive(getter.get().toLowerCase(Locale.ROOT)),
                null, null, null, choices, (client, value) -> {
    ...
    public static Setting text(String id, String name, String key, String fallback, TextSetter setter) {
        return new Setting(normalizeId(id), name, "text", () -> new JsonPrimitive(Settings.getString(key, fallback)),
                null, null, null, List.of(), (client, value) -> setter.apply(client, value.getAsString()));
    }

    public static Setting text(String id, String name, Supplier<String> getter, TextSetter setter) {
        return new Setting(normalizeId(id), name, "text", () -> new JsonPrimitive(getter.get()),
                null, null, null, List.of(), (client, value) -> setter.apply(client, value.getAsString()));
    }

    public static Setting colorText(String id, String name, String key, String fallback, TextSetter setter) {
        return new Setting(normalizeId(id), name, "color", () -> new JsonPrimitive(Settings.getString(key, fallback)),
                null, null, null, List.of(), (client, value) -> setter.apply(client, value.getAsString()));
    }

    public static Setting color(String id, String name, Supplier<String> getter, ColorSetter setter) {
        return new Setting(normalizeId(id), name, "color", () -> new JsonPrimitive(toHex(getter.get())),
                null, null, null, List.of(), (client, value) -> setter.apply(client, parseColor(value.getAsString())));
    }
    ...
    public static String rangeText(String minKey, String maxKey, double min, double max) {
        return format(Settings.getDouble(minKey, min)) + "-" + format(Settings.getDouble(maxKey, max));
    }
    ...
    private static String toHex(String raw) {
        ...
        if (parts.length == 3) {
            try {
                return String.format("#%02x%02x%02x", Integer.parseInt(parts[0]), Integer.parseInt(parts[1]), Integer.parseInt(parts[2]));
    ...
    public record Module(String id, String name, String category, BooleanSupplier enabled,
                         Toggle toggle, Supplier<String> tag, List<Setting> settings) {
    ...
    public record Setting(String id, String name, String type, Supplier<JsonElement> value,
                          Double min, Double max, Double step, List<String> options,
                          BooleanSupplier visibleWhen, BooleanSupplier enabledWhen, SettingApply apply) {
        public Setting(String id, String name, String type, Supplier<JsonElement> value,
                       Double min, Double max, Double step, List<String> options, SettingApply apply) {
            this(id, name, type, value, min, max, step, options, () -> true, () -> true, apply);
        }

        public Setting visibleWhen(BooleanSupplier condition) {
            return new Setting(id, name, type, value, min, max, step, options,
                    condition, enabledWhen, apply);
        }

        public Setting enabledWhen(BooleanSupplier condition) {
            return new Setting(id, name, type, value, min, max, step, options,
                    visibleWhen, condition, apply);
        }

        public boolean isVisible() { return safeEnabled(visibleWhen); }
        public boolean isEnabled() { return isVisible() && safeEnabled(enabledWhen); }
