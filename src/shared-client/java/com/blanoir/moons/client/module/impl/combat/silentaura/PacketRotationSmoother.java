...
public final class PacketRotationSmoother {
    ...
    public Rotation sample(int tick, float cameraYaw, float cameraPitch,
                           float desiredYaw, float desiredPitch,
                           double sensitivity,
                           boolean overlappingTarget,
                           boolean matrixCompatibility) {
        return sample(tick, cameraYaw, cameraPitch, desiredYaw, desiredPitch,
                sensitivity, overlappingTarget, matrixCompatibility, 90, 0.0D);
    }
    ...
    public Rotation sample(int tick, float cameraYaw, float cameraPitch,
        ...
        if (matrixCompatibility && overlappingTarget) {
            yawStep = Mth.clamp(yawStep,
                    -OVERLAP_MAX_YAW_STEP, OVERLAP_MAX_YAW_STEP);
        }
