...
public final class FakeLag {
    ...
    public static void handleIncoming(Packet<?> packet) {
        synchronized (LOCK) {
            if (constantActive() && (queueing || !PACKETS.isEmpty())
    ...
    public static int showStatus(Minecraft client) {
        ClientChat.send(client, "FakeLag: " + (MASTER_ENABLED.get() ? "enabled" : "disabled")
                + ", mode " + modeName() + ", constant " + DELAY_MIN.get() + "-"
                + DELAY_MAX.get() + "ms, recoil " + RECOIL_MS.get() + "ms.");
        return 1;
    }
    ...
    public static boolean constantMode() { return MODE.get() == Mode.CONSTANT; }
    public static boolean lowHealthMode() { return MODE.get() == Mode.LOW_HEALTH; }
    public static boolean randomMode() { return MODE.get() == Mode.RANDOM; }
    ...
    private enum Mode { CONSTANT, LOW_HEALTH, RANDOM }
}
