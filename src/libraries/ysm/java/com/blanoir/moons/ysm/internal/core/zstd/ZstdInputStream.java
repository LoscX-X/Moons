...
public class ZstdInputStream extends InputStream {
    ...
    private final ZstdIncrementalFrameDecompressor decompressor =
            new ZstdIncrementalFrameDecompressor();
    ...
    private boolean fillInputBufferIfNecessary(int requiredSize) throws IOException {
        ...
        while (inputBufferLimit < inputBuffer.length) {
            int readSize =
                    inputStream.read(
                            inputBuffer, inputBufferLimit, inputBuffer.length - inputBufferLimit);
            if (readSize < 0) {
                break;
            }
