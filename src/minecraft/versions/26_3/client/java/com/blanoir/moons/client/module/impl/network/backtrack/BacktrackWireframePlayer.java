...
public final class BacktrackWireframePlayer {
    ...
    public void render(PoseStack matrices, int color, int outlineColor) {
        ...
        matrices.pushPose();
        try {
            matrices.rotate(new Quaternionf().rotationY((float) Math.toRadians(bodyYaw)));
    ...
    private void renderSwimming(PoseStack matrices, BacktrackRenderer.BoxBatch batch) {
        ...
        matrices.rotate(new Quaternionf().rotationX((float) Math.toRadians(SWIM_PART_ROTATION)));
    ...
    private void renderPart(
        ...
        if (xRot != 0f || yRot != 0f || zRot != 0f) {
            ...
            if (zRot != 0f) {
                matrices.rotate(new Quaternionf().rotationZ((float) Math.toRadians(zRot)));
            }
            if (yRot != 0f) {
                matrices.rotate(new Quaternionf().rotationY((float) Math.toRadians(yRot)));
            }
            if (xRot != 0f) {
                matrices.rotate(new Quaternionf().rotationX((float) Math.toRadians(xRot)));
            }
