...
public final class SilentAura {
    ...
    public static boolean isActivationHeld(Minecraft client) { return SilentAuraRuntime.activationHeld(client); }
    public static boolean shouldApplyRotation() { return SilentAuraRuntime.shouldApplyRotation(); }
    public static boolean shouldApplyManualUseRotation() { return SilentAuraRuntime.shouldApplyManualUseRotation(); }
    public static float getManualUseYaw() { return SilentAuraRuntime.manualUseYaw(); }
    public static float getManualUsePitch() { return SilentAuraRuntime.manualUsePitch(); }
    public static boolean shouldCorrectMovement() { return SilentAuraRuntime.shouldCorrectMovement(); }
    public static boolean shouldSuppressBlockBreaking() { return SilentAuraRuntime.shouldSuppressBlockBreaking(); }
    public static boolean shouldSuppressUseAction(Minecraft client) {
        return SilentAuraRuntime.shouldSuppressUseAction(client);
    }
    public static boolean isLockMode() { return SilentAuraConfig.lockMode(); }
    public static boolean isFullLockMode() { return SilentAuraConfig.fullLockMode(); }
    public static float getYaw() { return SilentAuraRuntime.yaw(); }
    public static float getPitch() { return SilentAuraRuntime.pitch(); }
    public static float getPacketYaw() { return SilentAuraRuntime.packetYaw(); }
    public static float getPacketPitch() { return SilentAuraRuntime.packetPitch(); }
    ...
    public static boolean shouldRenderBlock(Minecraft client) {
        return SilentAuraConfig.block()
                && isActivationHeld(client)
                && hasLockedTarget(client);
    }
    ...
    public static int setEnabled(Minecraft client, boolean value) {
        if (value) CombatModuleCoordinator.beforeEnable(client, CombatModuleCoordinator.Role.SILENT_AURA);
    ...
    public static int showTargetStatus(Minecraft client) {
        ClientChat.send(client, "SilentAura targets: " + SilentAuraConfig.targetStatus() + "."); return 1;
    }
    ...
    private static void drawDebugger(HudRenderEvent event) {
        ...
        String candidateRay = rayState(client, target, candidate.valid(), candidate.eye(),
                candidate.look(), candidate.targetId(), reach);
        String sentRay = rayState(client, target, sent.valid(), sent.eye(),
                sent.look(), sent.targetId(), reach);
        ...
        String[] lines = {
                ...
                "gate=" + gate,
                "critical=" + Critical.modeName()
                        + " enabled=" + Critical.isEnabled()
                        + " linked=" + SilentAuraConfig.criticalIntegration()
                        + " aimingWindow=" + Critical.isAimingWindowActive(),
                "decision=" + decision.attackKind().name().toLowerCase(Locale.ROOT)
                        + "+" + decision.ticksAhead() + " " + decision.reason()
        };
        int x = 6;
        int y = Math.max(6, event.graphics().guiHeight() / 2 - 42);
        for (int index = 0; index < lines.length; index++) {
            int color = index == 0 ? 0xFFBFA3FF
                    : index == 3 && (gate.contains("critical wait")
                    || gate.contains("critical block")) ? 0xFFFF7777
                    : 0xFFEDE9FF;
        ...
        for (int index = 0; index < timingLines.length; index++) {
            ...
            int color = line.contains("MISMATCH")
                    ? 0xFFFF6666 : index == 0 ? 0xFFFFCC66 : 0xFFD8E8FF;
            event.graphics().text(client.font, line, x,
