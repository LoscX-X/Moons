...
public final class OreHighlighter {
    ...
    private static final RenderPipeline FILLED_THROUGH_WALLS = GameAccess.registerPipeline(
            RenderPipeline.builder(GameAccess.debugFilledSnippet())
                    .withLocation(Identifier.fromNamespaceAndPath(
                            MoonsConfig.MOD_ID, "pipeline/diamond_highlight_through_walls"
                    ))
    ...
    private static final BooleanSetting DISPLAY_ENABLED = new BooleanSetting.Builder()
    ...
    private static void render(WorldRenderEvent context) {
        ...
        WorldOverlayBuffer.draw(client, FILLED_THROUGH_WALLS, "ore highlighter rendering", builder -> {
            for (OreCache.CachedXrayBlock entry : visible) {
                BlockPos pos = entry.pos();
                XrayTarget target = entry.target();
                renderFilledBox(pose, builder, pos.getX(), pos.getY(), pos.getZ(),
                        pos.getX() + 1, pos.getY() + 1, pos.getZ() + 1,
                        target.red() / 255.0f, target.green() / 255.0f,
                        target.blue() / 255.0f, BOX_ALPHA);
            }
        });
    }

    private static void renderFilledBox(Matrix4fc pose, VertexConsumer b,
                                        float x1, float y1, float z1, float x2, float y2, float z2,
                                        float red, float green, float blue, float alpha) {
        vertex(b, pose, x1, y1, z2, red, green, blue, alpha); vertex(b, pose, x2, y1, z2, red, green, blue, alpha);
        vertex(b, pose, x2, y2, z2, red, green, blue, alpha); vertex(b, pose, x1, y2, z2, red, green, blue, alpha);
        vertex(b, pose, x2, y1, z1, red, green, blue, alpha); vertex(b, pose, x1, y1, z1, red, green, blue, alpha);
        vertex(b, pose, x1, y2, z1, red, green, blue, alpha); vertex(b, pose, x2, y2, z1, red, green, blue, alpha);
        vertex(b, pose, x1, y1, z1, red, green, blue, alpha); vertex(b, pose, x1, y1, z2, red, green, blue, alpha);
        vertex(b, pose, x1, y2, z2, red, green, blue, alpha); vertex(b, pose, x1, y2, z1, red, green, blue, alpha);
        vertex(b, pose, x2, y1, z2, red, green, blue, alpha); vertex(b, pose, x2, y1, z1, red, green, blue, alpha);
        vertex(b, pose, x2, y2, z1, red, green, blue, alpha); vertex(b, pose, x2, y2, z2, red, green, blue, alpha);
        vertex(b, pose, x1, y2, z2, red, green, blue, alpha); vertex(b, pose, x2, y2, z2, red, green, blue, alpha);
        vertex(b, pose, x2, y2, z1, red, green, blue, alpha); vertex(b, pose, x1, y2, z1, red, green, blue, alpha);
        vertex(b, pose, x1, y1, z1, red, green, blue, alpha); vertex(b, pose, x2, y1, z1, red, green, blue, alpha);
        vertex(b, pose, x2, y1, z2, red, green, blue, alpha); vertex(b, pose, x1, y1, z2, red, green, blue, alpha);
    }
