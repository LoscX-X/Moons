...
import com.blanoir.moons.ysm.internal.molang.lexer.Cursor;
...
public /* sealed */ interface MolangParser /* permits MolangParserImpl */ extends Closeable {
    ...
    static @NotNull MolangParser parser(
            @NotNull MolangLexer molangLexer, @NotNull ObjectBinding objectBinding)
            throws IOException {
        return new MolangParserImpl(molangLexer, objectBinding);
    }

    static @NotNull MolangParser parser(
            @NotNull Reader reader, @NotNull ObjectBinding objectBinding) throws IOException {
        return parser(MolangLexer.lexer(reader), objectBinding);
    }

    static @NotNull MolangParser parser(@NotNull String str, @NotNull ObjectBinding objectBinding)
            throws IOException {
        return parser(MolangLexer.lexer(str), objectBinding);
    }

    static @NotNull List<Expression> parseExpressions(
            @NotNull Reader reader, @NotNull ObjectBinding objectBinding) throws IOException {
    ...
    static @NotNull List<Expression> parseExpressions(
            @NotNull String str, @NotNull ObjectBinding objectBinding) throws IOException {
