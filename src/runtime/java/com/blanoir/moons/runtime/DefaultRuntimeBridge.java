...
public final class DefaultRuntimeBridge implements RuntimeBridge {
        ...
        if (!closed.get()) {
            if (versionVerified.compareAndSet(false, true)) {
                try {
                    ...
                    throw new IllegalStateException("Minecraft version verification failed", failure);
                }
            }
            startAndReloadModules();
            if (firstTickReported.compareAndSet(false, true)) {
                System.out.println(Branding.prefix() + " First client tick received from "
                        + minecraft.getClass().getName());
            }
            events.clientTick().publish(new RuntimeEvents.ClientTick(minecraft, RuntimeEvents.Phase.START));
        }
    }
    ...
    public void onClientTickEnd(Object minecraft) {
        if (!closed.get()) {
            events.clientTick().publish(new RuntimeEvents.ClientTick(minecraft, RuntimeEvents.Phase.END));
    ...
    public void onHudRender(Object gui, Object extractor, Object deltaTracker) {
        if (!closed.get()) events.hud().publish(new RuntimeEvents.Hud(gui, extractor, deltaTracker));
    }
    ...
    public void onWorldRender(
        ...
        if (!closed.get()) events.worldRender().publish(
                new RuntimeEvents.WorldRender(renderer, poseStack, levelRenderState, stage));
    }
    ...
    public boolean onKey(Object handler, long window, int action, Object event) {
        ...
        if (!closed.get()) events.key().publish(new RuntimeEvents.Key(handler, window, action, event, control));
    ...
    public boolean onMouse(Object handler, long window, Object button, int action) {
        RuntimeEvents.Control control = new RuntimeEvents.Control();
        if (!closed.get()) events.mouse().publish(new RuntimeEvents.Mouse(handler, window, button, action, control));
        return !control.isCancelled();
    }
    ...
    public boolean onMouseScroll(Object handler, long window, double xOffset, double yOffset) {
        RuntimeEvents.Control control = new RuntimeEvents.Control();
        if (!closed.get()) events.mouseScroll().publish(
                new RuntimeEvents.MouseScroll(handler, window, xOffset, yOffset, control));
        return !control.isCancelled();
    }
    ...
    public boolean onPacketSend(Object connection, Object packet) {
        RuntimeEvents.Control control = new RuntimeEvents.Control();
        if (!closed.get()) events.packet().publish(new RuntimeEvents.Packet(
                connection, packet, RuntimeEvents.PacketPhase.SEND_PRE, control));
        return !control.isCancelled();
    }
    ...
    public void onPacketSent(Object connection, Object packet) {
        if (!closed.get()) events.packet().publish(new RuntimeEvents.Packet(
                connection, packet, RuntimeEvents.PacketPhase.SEND_POST, new RuntimeEvents.Control()));
    }
    ...
    public boolean onPacketReceive(Object packet, Object listener) {
        RuntimeEvents.Control control = new RuntimeEvents.Control();
        if (!closed.get()) events.packet().publish(new RuntimeEvents.Packet(
                listener, packet, RuntimeEvents.PacketPhase.RECEIVE_NETWORK, control));
        return !control.isCancelled();
    }
    ...
    public void onPacketApply(Object packet, Object listener) {
        if (!closed.get()) events.packet().publish(new RuntimeEvents.Packet(
                listener, packet, RuntimeEvents.PacketPhase.RECEIVE_APPLY, new RuntimeEvents.Control()));
    }

    @Override
    public void onBlockUpdate(Object level, Object position, Object newState, boolean applied) {
        if (!closed.get()) events.blockUpdate().publish(
    ...
    public void onMouseMoveStart(Object handler) {
        if (!closed.get()) events.mouseMove().publish(
                new RuntimeEvents.MouseMove(handler, RuntimeEvents.Phase.START));
    }

    @Override
    public void onMouseMoveEnd(Object handler) {
        if (!closed.get()) events.mouseMove().publish(
                new RuntimeEvents.MouseMove(handler, RuntimeEvents.Phase.END));
    }

    @Override
    public void onPlayerPositionStart(Object player) {
        if (!closed.get()) events.playerPosition().publish(
                new RuntimeEvents.PlayerPosition(player, RuntimeEvents.Phase.START));
    }

    @Override
    public void onPlayerPositionEnd(Object player) {
        if (!closed.get()) events.playerPosition().publish(
                new RuntimeEvents.PlayerPosition(player, RuntimeEvents.Phase.END));
    }

    @Override
    public void onRenderState(Object entity, Object state, float partialTick) {
        if (!closed.get()) events.renderState().publish(new RuntimeEvents.RenderState(entity, state, partialTick));
    }

    @Override
    public void onRendererClose(Object renderer) {
        if (!closed.get()) events.rendererClose().publish(new RuntimeEvents.RendererClose(renderer));
    }

    @Override
    public void onVoidHook(String id, Object owner, Object argument) {
        if (!closed.get()) events.methodHook().publish(
                new RuntimeEvents.MethodHook(id, owner, argument, null));
    }
    ...
    private void startAndReloadModules() {
        try {
            ...
            System.err.println(Branding.prefix() + " Module operation failed on client thread: " + failure);
    ...
    private boolean publishAction(Object minecraft, RuntimeEvents.Kind kind) {
        RuntimeEvents.Control control = new RuntimeEvents.Control();
        if (!closed.get()) events.action().publish(new RuntimeEvents.Action(
    ...
    private void publishActionEnd(Object minecraft, RuntimeEvents.Kind kind) {
        if (!closed.get()) events.action().publish(new RuntimeEvents.Action(
                minecraft, kind, RuntimeEvents.Phase.END, new RuntimeEvents.Control()));
    }
}
