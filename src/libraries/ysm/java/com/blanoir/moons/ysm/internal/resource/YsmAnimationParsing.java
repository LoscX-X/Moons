...
public final class YsmAnimationParsing {
    ...
    public static RawYsmModel.RawAnimationFile parseAnimationFile(byte[] data) {
        ...
        if (root.has("animations")) {
            ...
            for (Map.Entry<String, JsonElement> entry : anims.entrySet()) {
                ...
                anim.length =
                        (float)
                                YsmJsonSupport.getDouble(
                                        aObj, "animation_length", Float.POSITIVE_INFINITY);
                ...
                if (aObj.has("sound_effects") && aObj.get("sound_effects").isJsonObject()) {
                    ...
                    for (Map.Entry<String, JsonElement> sfxEntry : sfxObj.entrySet()) {
                        ...
                        sfx.effectName =
                                YsmJsonSupport.getStr(
                                        sfxEntry.getValue().getAsJsonObject(), "effect", "");
    ...
    private static void parseChannelToKeyframes(
            JsonObject bObj, String channel, List<RawYsmModel.RawKeyframe> targetList) {
            ...
            if (valElem.isJsonObject()) {
                ...
                if (obj.has("lerp_mode")) {
                    ...
                    if ("catmullrom".equals(lm))
                        kf.interpolationMode = RawYsmModel.RawKeyframe.INTERPOLATION_CATMULLROM;
                    else if ("step".equals(lm))
                        kf.interpolationMode = RawYsmModel.RawKeyframe.INTERPOLATION_STEP;
                } else kf.interpolationMode = RawYsmModel.RawKeyframe.INTERPOLATION_STEP;
                ...
                if (obj.has("pre") && obj.has("post")) {
                    ...
                    kf.hasPreData = false;
                    kf.postData =
                            jsonElementToMolangArray(
                                    obj.has("post")
                                            ? obj.get("post")
                                            : obj.has("pre") ? obj.get("pre") : obj);
                }
            } else {
                kf.hasPreData = false;
