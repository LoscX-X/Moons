...
import static com.blanoir.moons.ysm.internal.core.zstd.Util.checkArgument;
import static com.blanoir.moons.ysm.internal.core.zstd.Util.checkState;
...
import static com.blanoir.moons.ysm.internal.core.zstd.ZstdFrameDecompressor.MAX_WINDOW_SIZE;
import static com.blanoir.moons.ysm.internal.core.zstd.ZstdFrameDecompressor.decodeRawBlock;
import static com.blanoir.moons.ysm.internal.core.zstd.ZstdFrameDecompressor.decodeRleBlock;
import static com.blanoir.moons.ysm.internal.core.zstd.ZstdFrameDecompressor.readFrameHeader;
import static com.blanoir.moons.ysm.internal.core.zstd.ZstdFrameDecompressor.verifyMagic;
...
public class ZstdIncrementalFrameDecompressor {
    ...
    public void partialDecompress(
        ...
        if (inputRequired > inputLimit - inputAddress) {
            throw new IllegalArgumentException(
                    format(
                            ...
                            inputRequired, inputLimit - inputAddress));
        }
        ...
        while (true) {
            // Flush ready output
            {
                ...
                if (flushableOutputSize > 0) {
                    ...
                    if (freeOutputSize > 0) {
                        ...
                        System.arraycopy(
                                windowBase,
                                toIntExact(windowAddress - ARRAY_BYTE_BASE_OFFSET),
                                outputArray,
                                output,
                                copySize);
                        if (partialHash != null) {
                    ...
                    if (flushableOutputSize > 0) {
                        requestOutput(
                                inputAddress, outputOffset, input, output, flushableOutputSize);
                        return;
                    }
                }
            }
            ...
            checkState(
                    computeFlushableOutputSize(frameHeader) == 0, "Expected output to be flushed");
            ...
            if (state == State.READ_FRAME_HEADER) {
                ...
                verify(
                        frameHeaderSize == frameHeader.headerSize,
                        input,
                        "Unexpected frame header size");
                    ...
                    case RAW_BLOCK:
                        {
                            ...
                            verify(
                                    windowLimit - windowPosition >= blockSize,
                                    input,
                                    "window buffer is too small");
                            decodedSize =
                                    decodeRawBlock(
                                            inputBase,
                                            input,
                    ...
                    case COMPRESSED_BLOCK:
                        {
                            ...
                            verify(
                                    windowLimit - windowPosition >= MAX_BLOCK_SIZE,
                                    input,
                                    "window buffer is too small");
                            decodedSize =
                                    frameDecompressor.decodeCompressedBlock(
                                            inputBase,
                                            input,
                                            blockSize,
                                            windowBase,
        ...
        return max(
                ...
                toIntExact(
                        windowPosition
                                ...
                                - (frameHeader == null
                                        ? 0
                                        : frameHeader.computeRequiredOutputBufferLookBackSize())));
    }

    private void resizeWindowBufferIfNecessary(
            FrameHeader frameHeader, int blockType, int blockSize) {
        int maxBlockOutput;
        ...
        if (windowLimit - windowPosition < MAX_BLOCK_SIZE) {
            ...
            checkState(
                    windowPosition - windowAddress <= requiredWindowSize,
                    "Expected output to be flushed");
            ...
            if (windowLimit - windowPosition < maxBlockOutput) {
                // if content size is set and smaller than the required window size, use the content
                // size
                int newWindowSize;
                if (frameHeader.contentSize >= 0 && frameHeader.contentSize < requiredWindowSize) {
                    ...
                    checkState(
                            windowContentsSize + maxBlockOutput <= newWindowSize,
                            "Computed new window size buffer is not large enough");
                }
            ...
            checkState(
                    windowLimit - windowPosition >= maxBlockOutput, "window buffer is too small");
        }
    }

    private static int determineFrameHeaderSize(
    ...
    private void requestOutput(
            long inputAddress, int outputOffset, long input, int output, int requestedOutputSize) {
    ...
    private void inputRequired(
            long inputAddress, int outputOffset, long input, int output, int inputRequired) {
        updateInputOutputState(inputAddress, outputOffset, input, output);
    ...
    private void updateInputOutputState(
            long inputAddress, int outputOffset, long input, int output) {
