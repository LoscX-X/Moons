...
public final class AntiNick {
    ...
    private static void tick(Minecraft client) {
        if (!ENABLED.get() || !RESOLVE_NAMES.get() || client == null || client.getConnection() == null) return;
        ...
        for (PlayerInfo info : client.getConnection().getListedOnlinePlayers()) {
            ...
            MOJANG.lookupByUuidAsync(uuid).whenComplete((profile, failure) -> client.execute(() -> {
                ...
                if (failure != null) {
                    retryAt.put(uuid, System.currentTimeMillis() + 30_000L);
                } else if (profile == null) {
                    retryAt.put(uuid, System.currentTimeMillis() + 60_000L);
                } else {
                    retryAt.remove(uuid);
    ...
    private static Component decorate(UUID uuid, String visibleName, Component original) {
        ...
        if (MARK_NICK_UUID.get() && uuid.version() == 1 && !plain.contains(SUFFIX.get())) {
            result = result.copy().append(Component.literal(" " + SUFFIX.get())
    ...
    public static int setMarkNickUuid(Minecraft client, boolean value) { MARK_NICK_UUID.set(value); return 1; }
    public static int setResolveNames(Minecraft client, boolean value) { RESOLVE_NAMES.set(value); return 1; }
    public static int setSuffix(Minecraft client, String value) { SUFFIX.set(value); return 1; }
    public static int setRefreshMs(Minecraft client, int value) { REFRESH_MS.set(value); nextRefreshAt = 0L; return 1; }
