...
public final class LowHealthFakeLag {
    ...
    public static boolean handleOutgoing(Connection connection, Packet<?> packet) {
        if (PACKETS.isReplaying() || RandomFakeLag.isReplaying()
                || FakeLag.isReplaying()) {
            return false;
        }
    ...
    private static double healthRatio(Minecraft client) {
        return client.player.getHealth()
                / Math.max(1.0D, client.player.getMaxHealth());
    }
    ...
    public static int showStatus(Minecraft client) {
        ClientChat.send(client,
                "LowHealthFakeLag: " + (ENABLED.get() ? "enabled" : "disabled")
                        + ", health: 25%, cycles: " + CYCLES_MIN.get() + "-" + CYCLES_MAX.get()
                        ...
                        + ", recoil: 250ms, state: " + (queueing ? "active" : "idle") + "."
