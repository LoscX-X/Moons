...
internal fun MoonsPanelClickGui(
    ...
    MaterialTheme(
        ...
        ProvideTextStyle(TextStyle(fontFamily = PanelFontFamily)) {
            ...
            val contentWidth = (logicalWidth - CONTROL_WIDTH).coerceAtLeast(PANEL_WIDTH + PANEL_MARGIN * 2f)
            val columns = min(
                visibleCategories.size.coerceAtLeast(1),
                max(
                    1,
                    floor((contentWidth - PANEL_MARGIN * 2f + PANEL_GAP) / (PANEL_WIDTH + PANEL_GAP)).toInt()
                )
            )
            val occupiedWidth = columns * PANEL_WIDTH + (columns - 1) * PANEL_GAP
            val gridStartX = CONTROL_WIDTH +
            ...
            visibleCategories.forEachIndexed { index, category ->
                ...
                CategoryPanel(
                    ...
                    onMoveBy = { delta ->
                        ...
                        positions[category] = clampPanelPosition(
                            current + delta,
                            logicalWidth,
                            logicalHeight
                        )
                ...
                ControlMenu(
                    ...
                    onSelectSection = {
                        ...
                        when (it) {
                            ...
                            "all" -> {
                                ...
                                val next = if (openSections.containsAll(allIds)) {
                                    emptySet()
                                } else {
                                    allIds
                                }
                                ...
                                settingsOpen = false
                                val next = if (it in openSections) {
                                    openSections - it
                                } else {
                                    openSections + it
                                }
                    ...
                    onClose = onClose,
                    modifier = Modifier.align(Alignment.CenterStart).padding(start = 12.dp).zIndex(100f)
                )
            }
        }
    }
