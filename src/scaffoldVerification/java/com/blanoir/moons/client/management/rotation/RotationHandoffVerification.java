...
public final class RotationHandoffVerification {
    ...
    private static void acquisitionStartsFromRealHistory() {
        ...
        Rotation[] start = {null};
        int[] starts = {0};
        require(aura.acquire(request(70, 10), CAMERA, value -> { start[0] = value; starts[0]++; }), "initial acquire");
        ...
        require(!RotationHistory.latest().valid(), "positionless/rotationless send cannot invent a look");
        var sent = look(70, 10);
        ...
        require(block.acquire(request(120, 40), CAMERA, value -> start[0] = value), "uncommitted preemption");
        require(RotationHistory.same(start[0], new Rotation(70, 10)), "B starts at sent 70, not planned 80 or camera 5");
        ...
        RotationHistory.reset();
        require(!aura.active() && !RotationHistory.latest().valid(), "context reset invalidates owner and history");
    }
    ...
    private static void committedWindowSurvivesDisableAndCancelledSend() {
        ...
        require(aura.commit(new Rotation(100, 50), true, true) == committed, "all readers reuse the same result");
        require(!block.acquire(request(120, 40)), "cannot preempt an action/movement window");
        aura.release();
        require(aura.active() && RotationLease.submission().rotation() == committed, "disable defers release and preserves output");
        var cancelled = look(80, 20);
        RotationHistory.capture(new PacketSendEvent.Pre(null, cancelled, null));
        RotationLease.finishMotion();
        require(!aura.active() && RotationHistory.latest().yaw() == 70, "method completion without POST cannot advance history");
        Rotation[] start = {null};
        require(block.acquire(request(120, 40), CAMERA, value -> start[0] = value), "handoff after cancelled window completes");
        ...
        require(aura.confirmed() && RotationHistory.sentFor(aura, emitted), "matching real send confirms its owner");
        RotationLease.finishMotion();
    ...
    private static void pinnedInteractionSurvivesRelease() {
        ...
        block.release();
        RotationLease.finishMotion();
        require(block.active() && !next.acquire(request(0, 0)), "pin survives disable and method completion");
        block.unpin();
        require(!block.active() && next.acquire(request(0, 0)), "explicit interaction closure completes deferred release");
    }
    ...
    private static void replayCannotConfirmAnotherRequest() {
        ...
        require(!lease.confirmed() && !RotationHistory.sentFor(lease, queued), "same floats cannot confirm a new tenure");
        RotationLease.finishMotion();
        var own = look(20, 10);
        lease.commit(new Rotation(20, 10), true, true);
        ...
        require(RotationHistory.latest().yaw() == 20 && RotationHistory.latest().pitch() == 10,
        ...
        require(!lease.confirmed() && !RotationHistory.sentFor(lease, oldWindow),
    ...
    private static void manualUseBlocksNewOwners() {
        ...
        RotationLease.finishMotion();
        require(RotationLease.manualRotation() != null, "cancel/no packet cannot close manual window");
        RotationHistory.record(look(42, 76), 1);
        require(RotationLease.manualRotation() != null, "similar float is not the exact manual angle");
        RotationHistory.record(look(exact.yaw(), exact.pitch()), 2);
        Rotation[] start = {null};
        require(next.acquire(request(120, 30), CAMERA, value -> start[0] = value), "closing send permits takeover");
    ...
    private static void quantizationPreservesExactPairs() {
        require(RotationQuantizer.yaw(179, -179, .5) == 181, "wrap follows shortest equivalent yaw");
        ...
        require(RotationHistory.same(exact, lease.commit(exact, true, true)), "validated/interaction float bits bypass quantization");
    }
    ...
    private static void nextOwnerResumesAtClosingBoundary() {
        ...
        int[] calls = {0};
        next.whenAvailable(() -> {
            require(next.acquire(request(120, 30), CAMERA, start -> {
                require(start.yaw() == 42 && start.pitch() == 76, "next owner inherits closing angle");
        ...
        RotationLease.finishMotion();
        require(calls[0] == 1 && next.active() && RotationHistory.latest().tick() == 7,
                "release and reacquisition share the same tick without an idle movement");
        require(RotationHistory.latest().sequence() == 1, "handoff creates no synthetic movement packet");
    }
    ...
    private static void releaseBeforeMovementResumesWithoutGap() {
        ...
        int[] calls = {0};
        next.whenAvailable(() -> {
            require(next.acquire(request(120, 30)), "next producer can acquire before this tick's movement");
            calls[0]++;
        });
        require(calls[0] == 0, "ordinary priority remains effective while old owner is active");
        previous.release();
        require(calls[0] == 1, "release directly resumes waiting work without FRAME or finishMotion");
        next.release();
        ...
        previous.release();
        require(calls[0] == 1, "temporary camera window blocks early continuation even without a submission");
        RotationLease.finishMotion();
