...
public final class TargetInfoHud {
    ...
    private static final BooleanSetting ENABLED =
            new BooleanSetting.Builder().name("targetinfo.enabled").defaultValue(true).build();
    private static final IntSetting POSITION_X =
            new IntSetting.Builder().name("targetinfo.x").defaultValue(20).range(0, 10000).build();
    private static final IntSetting POSITION_Y =
            new IntSetting.Builder().name("targetinfo.y").defaultValue(24).range(0, 10000).build();
    private static final DoubleSetting SCALE =
            new DoubleSetting.Builder()
                    .name("targetinfo.scale")
                    .defaultValue(1.0D)
                    .range(0.25D, 2.0D)
                    .build();
    ...
    public record Snapshot(
        ...
        private static final Snapshot HIDDEN =
                new Snapshot(false, new Bounds(0, 0, 0, 0), "", "", "", 0.0D, 0x70D69A, 0, 0.0F);
    }
}
