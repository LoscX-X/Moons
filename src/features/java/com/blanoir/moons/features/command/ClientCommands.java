...
public final class ClientCommands {
    ...
    private static boolean handleMoons(String tail) {
        ...
        if (parts.length == 1 && parts[0].equalsIgnoreCase("list")) {
            String enabled = ModuleRegistry.enabledModules().stream()
        ...
        if (parts.length < 2) {
            boolean known = ModuleRegistry.modules().stream()
                    .anyMatch(candidate -> candidate.id().equalsIgnoreCase(parts[0]));
            ClientChat.send(client, known
        ...
        if (operation.equals("enable") || operation.equals("disable") || operation.equals("toggle")) {
            ModuleRegistry.Module found = ModuleRegistry.modules().stream()
                    .filter(candidate -> candidate.id().equalsIgnoreCase(module))
                    .findFirst().orElse(null);
            if (found == null) {
                ClientChat.send(client, "Unknown module: " + module);
            } else {
                boolean enabled = operation.equals("toggle")
                        ? !found.enabled().getAsBoolean() : operation.equals("enable");
