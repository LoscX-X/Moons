...
public final class MathUtils {
    ...
    public static AABB inset(AABB box, double requestedInset, double maxSizeFraction) {
        ...
        return new AABB(box.minX + x, box.minY + y, box.minZ + z,
    ...
    public static double angleBetween(Vec3 first, Vec3 second) {
        if (first == null || second == null
                || first.lengthSqr() < EPSILON || second.lengthSqr() < EPSILON) {
            return 0.0D;
        }
    ...
    public static double squaredDistanceToRay(
        ...
        Vec3 direction = rayDirection.lengthSqr() > EPSILON
                ? rayDirection.normalize() : new Vec3(0.0D, 0.0D, 1.0D);
        double along = Mth.clamp(
                point.subtract(rayStart).dot(direction), 0.0D, Math.max(0.0D, rayLength));
