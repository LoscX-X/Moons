...
final class FiniteStateEntropy {
    ...
    public static int decompress(
        ...
        BitInputStream.Loader loader =
                new BitInputStream.Loader(inputBase, input, currentAddress, bits, bitsConsumed);
        loader.load();
        bits = loader.getBits();
    ...
    public static int normalizeCounts(
        ...
        for (int symbol = 0; symbol <= maxSymbol; symbol++) {
            if (counts[symbol] == total) {
                throw new IllegalArgumentException(); // TODO: should have been RLE-compressed by
                // upper layers
            }
            if (counts[symbol] == 0) {
        ...
        if (-stillToDistribute >= (normalizedCounts[largest] >>> 1)) {
            ...
            // TODO size_t const errorCode = FSE_normalizeM2(normalizedCounter, tableLog, count,
            // total, maxSymbolValue);
    ...
    private static int normalizeCounts2(
        ...
        int lowThreshold =
                total >>> tableLog; // minimum count below which frequency in the normalized table
        // is "too small" (~ < 1)
        int lowOne =
                (total * 3)
                        >>> (tableLog
                                + 1); // 1.5 * lowThreshold. If count in (lowThreshold, lowOne] =>
        // assign frequency 1
