...
public final class NativeMoonsTransformerVerification {
    ...
    public static void main(String[] arguments) throws Exception {
        ...
        if (minecraft.ticks() != 1L || starts.get() != 1 || ends.get() != 1) {
            throw new AssertionError(
                    "Production tick hook failed: ticks="
                            + minecraft.ticks()
                            + ", starts="
                            + starts.get()
                            + ", ends="
        ...
        System.out.println(
                "MOONS_NATIVE_MOONS_VERIFIED ticks="
                        + minecraft.ticks()
                        + " starts="
                        + starts.get()
                        + " ends="
    ...
    private static void verifyIsolatedGameBridge() throws Exception {
        ...
        agentBridge.getMethod("onClientTickStart", Object.class).invoke(null, new Object[] {null});

        Object failingRuntime =
        ...
        try (PrintStream testError = new PrintStream(capturedError, true, StandardCharsets.UTF_8)) {
            try {
                ...
                agentBridge
                        .getMethod("onClientTickStart", Object.class)
        ...
        if (!report.contains("Runtime failure in client tick start")
            ...
            throw new AssertionError(
                    "Isolated AgentBridge did not report the swallowed LinkageError");
        }
    }
