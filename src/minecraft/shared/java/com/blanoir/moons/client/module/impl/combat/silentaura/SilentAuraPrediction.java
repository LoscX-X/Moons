...
public final class SilentAuraPrediction {
    ...
    public Vec3 displacement(double horizonTicks) {
        double horizon = Math.clamp(horizonTicks, 0.0D, 3.0D);
        ...
        Vec3 travel =
                velocity.scale(horizon)
                        .add(acceleration.scale(0.5D * horizon * horizon / (1.0D + horizon)));
    ...
    private static Vec3 bounded(Vec3 vector, double maximum) {
        if (vector == null
                ...
                || !Double.isFinite(vector.z)) return Vec3.ZERO;
