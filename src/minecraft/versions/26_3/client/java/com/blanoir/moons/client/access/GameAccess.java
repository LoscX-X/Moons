...
public final class GameAccess {
    ...
    /** Preserve this version's complete render setup; the prepared draw is captured separately. */
    public static RenderType copyRenderType(RenderType base, String name) {
        try {
            return (RenderType)
                    CREATE_RENDER_TYPE.invoke(name, (RenderSetup) RENDER_SETUP.get(base));
        } catch (Throwable failure) {
