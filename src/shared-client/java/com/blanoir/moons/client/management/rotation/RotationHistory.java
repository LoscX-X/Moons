...
public final class RotationHistory {
    public record Sent(boolean valid, float yaw, float pitch, int tick, long sequence) {
        private static Sent empty() { return new Sent(false, 0, 0, Integer.MIN_VALUE, 0); }
        public Rotation rotation() { return new Rotation(yaw, pitch); }
    }
    ...
    public static void init() {
        ...
        initialized = true;
        EventBus.CLIENT_CONTEXT_CHANGED.register("RotationHistory.context", EventPriority.HIGHEST,
                event -> reset());
        EventBus.PACKET_SEND_PRE.register("RotationHistory.capture", EventPriority.HIGHEST,
                RotationHistory::capture);
        EventBus.PACKET_SEND_POST.register("RotationHistory.sent", EventPriority.HIGHEST,
                RotationHistory::record);
    }

    public static synchronized Sent latest() { return latest; }
    ...
    public static Rotation start(Minecraft client) {
        ...
        return start(client == null || currentPlayer == null ? new Rotation(0, 0)
    ...
    static synchronized void record(ServerboundMovePlayerPacket movement, int tick) {
        ...
        latest = new Sent(true, movement.getYRot(latest.yaw()), movement.getXRot(latest.pitch()),
                tick, latest.sequence() + 1);
    ...
    public static synchronized boolean sentFor(RotationLease lease, Packet<?> packet) {
        return latest.valid() && lastPacket == packet && lastSubmission != null
                && lastSubmission.lease() == lease && lease.matches(lastSubmission)
    ...
    private static synchronized void synchronizeContext(Minecraft client) {
        ...
        if ((context != null || player != null || level != null)
                && (context != current || player != client.player || level != client.level)) reset();
        context = current;
