...
public final class RotationLease {
    ...
    public record Submission(RotationLease lease, long generation, long requestId,
                             Rotation rotation, boolean correctMovement) { }
    ...
    public boolean acquire(RotationRequest request, Rotation camera, Consumer<Rotation> initialize) {
    ...
    public boolean active() { synchronized (RotationHistory.class) { return holder == this && request != null; } }
    public boolean confirmed() { synchronized (RotationHistory.class) { return active() && confirmed; } }
    public RotationRequest request() { synchronized (RotationHistory.class) { return request; } }
    ...
    public Rotation commit(Rotation candidate, boolean exact, boolean correctMovement) {
        synchronized (RotationHistory.class) {
            ...
            if (submission != null) return submission.lease() == this ? submission.rotation() : null;
            Rotation base = RotationHistory.start(start);
            Rotation result = exact ? candidate : new Rotation(
    ...
    public static Submission submission() { synchronized (RotationHistory.class) { return submission; } }
    ...
    public static void beginMotion() {
        synchronized (RotationHistory.class) { motionWindow = true; }
    }

    public static void beginPacketObservation() {
        synchronized (RotationHistory.class) { packetObservers++; }
    }
    ...
    public void cancelPending() {
        synchronized (RotationHistory.class) { continuations.remove(this); }
    }

    /** Runs ready preparation outside committed, temporary-camera and packet-observer windows. */
    public static void resumePending() {
        synchronized (RotationHistory.class) {
            if (resuming || motionWindow || packetObservers != 0
                    || submission != null || manual != null || continuations.isEmpty()) return;
            resuming = true;
            try {
                var ready = new ArrayList<>(continuations.keySet());
                ready.sort(Comparator.comparingInt((RotationLease lease) -> lease.priority).reversed());
                for (RotationLease lease : ready) {
                    if (submission != null || manual != null) break;
                    if (holder != null && (holder.pinned
                            || holder != lease && holder.priority >= lease.priority)) continue;
                    ...
                    try {
                        ...
                        System.err.println("[RotationLease] Continuation failed for " + lease.owner);
    ...
    boolean matches(Submission value) {
        return active() && value == submission
                && value.generation() == generation && value.requestId() == requestId;
    }
    ...
    public static boolean holdManual(Rotation rotation) {
        synchronized (RotationHistory.class) {
            if (holder != null && holder.pinned
                    || submission != null && !RotationHistory.same(submission.rotation(), rotation)) return false;
    ...
    public static Rotation manualRotation() { synchronized (RotationHistory.class) { return manual; } }
    ...
    public boolean pinned() { synchronized (RotationHistory.class) { return active() && pinned; } }
    ...
    public static boolean busyFor(RotationLease lease) {
        synchronized (RotationHistory.class) {
            return manual != null || submission != null && submission.lease() != lease
                    || holder != null && holder != lease
