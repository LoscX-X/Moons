...
public final class FeatureHooks {
        ...
        switch (hook.id()) {
            ...
            case "movement.sprint-start", "movement.sprint-input" -> {
                if (hook.owner() instanceof net.minecraft.client.player.LocalPlayer player) {
                    ...
                    hook.value(!CombatInputController.isSprintSuppressed()
                            && !Scaffold.shouldSuppressSprint(Minecraft.getInstance())
                            && (Sprint.shouldSprint(player)
                            || NoSlow.shouldKeepSprinting(player) || vanilla));
                }
            }
            ...
            case "combat.player-extra-knockback.end" -> KeepSprint.finishAttackSlowdown(hook.owner());
            ...
            case "render.scoreboard" -> {
                if (hook.argument() instanceof Object[] values && values.length == 2
            ...
            case "render.silent-aura-animation" -> {
                if (hook.argument() instanceof Object[] values && values.length == 4
            ...
            case "render.chams-mark-item" -> {
                Object submit = hook.argument() != null
                        ? hook.argument() : lastItemSubmit(hook.owner());
            ...
            case "render.clip-occlusion" -> {
                ...
                if (Clip.isEnabled() && client.player != null
            ...
            case "render.player-name" -> {
                if (hook.owner() instanceof Player player && hook.value() instanceof Component component) {
                    Component named = AntiNick.applyPlayerName(player,
                            Nickname.replaceLocalPlayerName(player, component));
            ...
            case "render.tab-name" -> {
                if (hook.argument() instanceof PlayerInfo info
                    ...
                    Component named = AntiNick.applyTabName(info, Nickname.replaceOwnName(component));
                    hook.value(PremiumCheckCommand.decorate(info, named));
                } else if (hook.argument() instanceof PlayerInfo info
                        && hook.value() instanceof Component component) {
                    hook.value(PremiumCheckCommand.decorate(info,
                            AntiNick.applyTabName(info, component)));
                }
            }
            ...
            case "render.chat-filter.server-system" ->
                    hook.value(!(hook.argument() instanceof Component component
            ...
            case "xray.block-tessellate.end", "optional.sodium.render-model.end" -> XrayTerrain.endBackground();
            ...
            case "command.client" -> {
                if (hook.argument() instanceof String command
                        && ClientCommands.handle(command)) {
                    hook.value(false);
                }
            }
            default -> { }
    ...
    private static void xrayTessellate(RuntimeEvents.MethodHook hook) {
        ...
        BlockAndTintGetter level = arguments[0] instanceof BlockAndTintGetter supplied
                ? supplied : Minecraft.getInstance().level;
