...
final class XrayCommand {
    ...
    private static final Map<String, CustomXrayTargets.ColorValue> COLORS =
            Map.ofEntries(
                    ...
                    Map.entry("yellow", color(255, 255, 0)),
                            Map.entry("orange", color(239, 128, 2)),
    ...
    private static void add(String blockName, CustomXrayTargets.ColorValue color) {
        ...
        ClientChat.send(
                client,
                "Ore target added: "
                        + result.target().commandName()
                        + " "
                        + result.target().rgbString()
                        + ".");
    }
